...
record-family order. Core comparison consumes comparison metadata for `FLST`, `GMST`, `GLOB`, `MISC`, `KYWD`, `STAT`,
`BOOK`, `DOOR`, `CONT`, `COBJ`, and `CNDF` simple scalar rows. Complex comparison strategies and the actual
game-specific Mutagen mapping remain owned by the existing Core and game-adapter services until later approved work
...
The specification catalog now drives the simple comparison fields for `FLST`, `GMST`, `GLOB`, `MISC`, `KYWD`, `STAT`,
`BOOK`, `DOOR`, `CONT`, `COBJ`, and `CNDF`. The Core comparison service remains the runtime authority for generated
