...
source fields, and strategy-owned child rows. Keyword rows, shared model rows, shared sound rows, and shared scripting
adapter rows for current supported comparison families are selected by comparison child-group metadata. Other `CLAS`,
`FACT`, `AVIF`, `NPC_`, `MGEF`, `PERK`, `STAT`, `BOOK`, `DOOR`, `CONT`, `COBJ`, `CNDF`, `MISC`, and `TERM` child
