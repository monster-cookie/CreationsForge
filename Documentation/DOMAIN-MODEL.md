...
`KYWD`, `STAT`, `BOOK`, `DOOR`, `CONT`, `COBJ`, and `CNDF` simple scalar rows. Complex comparison strategies and the
...
`KYWD`, `STAT`, `BOOK`, `DOOR`, `CONT`, `COBJ`, and `CNDF`. The Core comparison service remains the runtime authority
for generated comparison DTOs, including row state, plugin column ordering, indexed `FLST` item expansion, localized
`GMST` `Data` display, game-dependent `BOOK` body text source fields, and strategy-owned `CLAS`, `STAT`, `BOOK`,
`DOOR`, `CONT`, `COBJ`, `CNDF`, and `MISC` child groups.
