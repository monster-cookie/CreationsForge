...
`MISC`, `KYWD`, `AVIF`, `MGEF`, `STAT`, `BOOK`, `DOOR`, `CONT`, `COBJ`, and `CNDF` simple scalar rows. Complex
comparison strategies and the actual game-specific Mutagen mapping remain owned by the existing Core and game-adapter
services until later approved work makes those paths specification-driven.
...
`MISC`, `KYWD`, `AVIF`, `MGEF`, `STAT`, `BOOK`, `DOOR`, `CONT`, `COBJ`, and `CNDF`. The Core comparison service
remains the runtime authority for generated comparison DTOs, including row state, plugin column ordering, indexed
`FLST` item expansion, localized `GMST` `Data` display, game-dependent `BOOK` body text source fields, and
strategy-owned `CLAS`, `FACT`, `AVIF`, `MGEF`, `STAT`, `BOOK`, `DOOR`, `CONT`, `COBJ`, `CNDF`, and `MISC` child
groups.
