...
The specification catalog now drives simple comparison rows for `FLST`, `GMST`, `GLOB`, `CLAS`, `MISC`, `KYWD`,
`AVIF`, `MGEF`, `STAT`, `BOOK`, `DOOR`, `CONT`, `COBJ`, and `CNDF`.
...
`Data` display, game-dependent `BOOK` body-text source fields, and `CLAS`, `MISC`, `AVIF`, `MGEF`, `STAT`, `BOOK`,
`DOOR`, `CONT`, `COBJ`, and `CNDF` child groups. Complex record families and shared child groups remain on the existing
