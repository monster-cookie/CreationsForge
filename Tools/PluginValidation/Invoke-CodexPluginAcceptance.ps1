...
<#
.SYNOPSIS
Runs one isolated Codex CLI process for plugin MCP acceptance evidence.
.DESCRIPTION
Starts the plugin Codex CLI with command-local configuration for only the
...
try {
    ...
    Write-Output "Started isolated Codex plugin-acceptance process (PID $($child.Id))."
