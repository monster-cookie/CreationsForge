...
/// <content>Emits the deterministic paged Starfield Mutagen-field schema source and constructible defaults.</content>
internal sealed partial class CodecEmitter
{
    ...
    {
        new KeyValuePair<string, Type>("record.component", typeof(AComponent)),
        new KeyValuePair<string, Type>("record.condition", typeof(Condition)),
        new KeyValuePair<string, Type>("record.condition-data", typeof(ConditionData)),
    });

    /// <summary>The stable absolute JSON Schema URI prefix for Starfield v1 type nodes.</summary>
    private const string SchemaTypeUriPrefix = "urn:creationsforge:Mutagen-wire:starfield:starfield:v1:type:";
    ...
    private string EmitSchemaCatalog()
    {
        ...
        output.AppendLine();
        output.AppendLine("/// <summary>Exposes generated Starfield Mutagen-field schemas and actual installed constructor defaults to the game catalog.</summary>");
        output.AppendLine("internal static class StarfieldGeneratedRecordFieldSchema");
        output.AppendLine("{");
        ...
        output.AppendLine();
        output.AppendLine("    /// <summary>Every generated typed-leaf and concrete Mutagen node name in deterministic ordinal order.</summary>");
        ...
        output.AppendLine("    /// <summary>Gets every generated union, typed-leaf, and concrete Mutagen node name in deterministic ordinal order.</summary>");
        ...
        output.AppendLine("    /// <param name=\"recordTypeName\">The exact generated leaf or mutable Mutagen type name.</param>");
        ...
        output.AppendLine("    internal static string GetSchemaJson(string recordTypeName)");
        output.AppendLine("    {");
        output.AppendLine("        ArgumentException.ThrowIfNullOrWhiteSpace(recordTypeName);");
        output.AppendLine("        return recordTypeName switch");
        output.AppendLine("        {");
        foreach (var schema in schemas)
        {
        ...
        output.AppendLine("            _ => throw new NotSupportedException($\"Plugin type '{recordTypeName}' is outside generated Starfield schema coverage.\"),");
        output.AppendLine("        };");
        output.AppendLine("    }");
        output.AppendLine();
        ...
        output.AppendLine("    /// <exception cref=\"ArgumentException\">Thrown when the type name is empty.</exception>");
        output.AppendLine("    /// <exception cref=\"NotSupportedException\">Thrown when the type name is outside generated coverage.</exception>");
        output.AppendLine("    internal static string? CreateDefaultJson(string recordTypeName, CancellationToken cancellationToken = default)");
        output.AppendLine("    {");
        output.AppendLine("        _ = GetSchemaJson(recordTypeName);");
        output.AppendLine("        cancellationToken.ThrowIfCancellationRequested();");
        output.AppendLine("        switch (recordTypeName)");
        output.AppendLine("        {");
        output.AppendLine("            case \"record.component\":");
        output.AppendLine("            case \"record.condition\":");
        output.AppendLine("            case \"record.condition-data\":");
        output.AppendLine("            case \"record.array2d\":");
        output.AppendLine("            case \"record.asset-link\":");
        output.AppendLine("            case \"record.form-link\":");
        output.AppendLine("            case \"record.form-link-or-index\":");
        ...
        output.AppendLine("            case \"record.single\":");
        output.AppendLine("                return \"{\\\"text\\\":\\\"0\\\",\\\"bits\\\":\\\"0x00000000\\\",\\\"number\\\":0}\";");
        output.AppendLine("            case \"record.translated-string\":");
        ...
        output.AppendLine("        {");
        output.AppendLine("            StarfieldNestedFieldCodec.WriteDefaultRecordType(writer, recordTypeName, cancellationToken);");
        output.AppendLine("        }");
        output.AppendLine("        cancellationToken.ThrowIfCancellationRequested();");
    ...
    /// <param name="typeNames">The ordered exact Mutagen type names.</param>
    ...
    /// <param name="polymorphicType">The Mutagen root type represented by the union.</param>
    ...
    private string EmitUnionSchema(string nodeName, Type polymorphicType)
    {
        ...
        schema["description"] = $"Generated exact concrete union for installed Mutagen type {TypeName(polymorphicType)}.";
        schema["oneOf"] = alternatives;
        schema["x-record-polymorphic-type"] = TypeName(polymorphicType);
        schema["x-default-unavailable-reason"] = "A polymorphic union requires selection of one concrete Mutagen type.";
        return schema.ToJsonString();
    }

    /// <summary>Emits one complete concrete indexed type schema from installed metadata.</summary>
    /// <param name="model">The concrete generated Mutagen model.</param>
    /// <returns>Compact deterministic JSON Schema.</returns>
    private string EmitTypeSchema(RecordTypeModel model)
    {
        ...
        var properties = new JsonObject
        {
            ["$type"] = new JsonObject
            {
                ["const"] = RecordTypeDiscriminator(typeName),
                ["description"] = "The exact package-scoped Mutagen concrete type discriminator.",
            },
        };
            ...
            && !model.Fields.Any(static field => string.Equals(field.Name, "Function", StringComparison.Ordinal)))
        {
            var instance = System.Activator.CreateInstance(model.MutableType)
                ?? throw new InvalidOperationException($"Could not construct default Mutagen type {typeName} for derived Function metadata.");
            ...
            properties["Function"] = new JsonObject
            {
                ...
                ["x-record-type"] = TypeName(typeof(Condition.Function)),
                ["x-record-derived"] = true,
            };
        }
        ...
        foreach (var field in model.Fields)
        {
            ...
            fieldSchema["description"] = $"Plugin FieldIndex ordinal {field.Ordinal} for {typeName}.{field.Name}.";
            fieldSchema["x-record-field-ordinal"] = field.Ordinal;
            fieldSchema["x-record-mutable-type"] = TypeName(field.MutableProperty.PropertyType);
            fieldSchema["x-record-getter-type"] = TypeName(field.GetterProperty.PropertyType);
            fieldSchema["x-record-nullability"] = DescribeNullability(field.Nullability);
        ...
        schema["description"] = $"Complete generated construction schema for installed Mutagen type {typeName}. JSON is request-local wire data and is not persisted record authority.";
        schema["type"] = "object";
        schema["properties"] = properties;
        schema["required"] = required;
        schema["additionalProperties"] = false;
        schema["x-record-mutable-type"] = typeName;
        schema["x-record-getter-type"] = TypeName(model.GetterType);
        schema["x-record-field-count"] = model.Fields.Count;
        schema["x-record-construction"] = "public-parameterless-constructor-then-indexed-setters";
    ...
    /// <summary>Creates one recursive JSON Schema value for a generated record field.</summary>
    ...
    private JsonObject CreateValueSchema(RecordValueConstructionModel construction)
    {
        if (construction.Kind == RecordValueConstructionKind.Nullable)
        {
        ...
        {
            RecordValueConstructionKind.String => new JsonObject
            {
                ["type"] = "string",
                ["maxLength"] = 1048576,
            },
            RecordValueConstructionKind.Boolean => new JsonObject { ["type"] = "boolean" },
            RecordValueConstructionKind.Single => ReferenceSchema("record.single"),
            RecordValueConstructionKind.Double => ReferenceSchema("record.double"),
            RecordValueConstructionKind.Enum => CreateEnumSchema(construction.GetterType),
            RecordValueConstructionKind.Numeric => CreateNumericSchema(construction.GetterType),
            RecordValueConstructionKind.Guid => new JsonObject
            {
                ["type"] = "string",
                ["format"] = "uuid",
                ["pattern"] = "^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$",
            },
            RecordValueConstructionKind.Color => ReferenceSchema("record.color"),
            RecordValueConstructionKind.P2Float => ReferenceSchema("record.p2-float"),
            RecordValueConstructionKind.P2Int => ReferenceSchema("record.p2-int"),
            RecordValueConstructionKind.P3Float => ReferenceSchema("record.p3-float"),
            RecordValueConstructionKind.TranslatedString => ReferenceSchema("record.translated-string"),
            RecordValueConstructionKind.Memory => ReferenceSchema("record.byte-sequence"),
            RecordValueConstructionKind.FormLink => CreateFormLinkSchema(construction),
            RecordValueConstructionKind.FormLinkOrIndex => CreateFormLinkOrIndexSchema(construction),
            RecordValueConstructionKind.AssetLink => CreateAssetLinkSchema(construction),
            RecordValueConstructionKind.Collection => CreateCollectionSchema(construction),
            RecordValueConstructionKind.Array2d => CreateArray2dSchema(construction),
            RecordValueConstructionKind.NestedConcrete => ReferenceSchema(construction.MutableType.FullName!),
            RecordValueConstructionKind.NestedPolymorphic => CreatePolymorphicSchema(construction.MutableType),
    ...
    /// <param name="enumType">The installed Mutagen enum type.</param>
    ...
    private static JsonObject CreateEnumSchema(Type enumType)
    {
        ...
        schema["x-record-type"] = TypeName(enumType);
        ...
        schema["x-record-enum-values"] = values;
    ...
    private static JsonObject CreateFormLinkSchema(RecordValueConstructionModel construction)
    {
        return new JsonObject
        {
            ["$ref"] = SchemaTypeUri("record.form-link"),
            ["properties"] = new JsonObject
            {
                ["$type"] = new JsonObject { ["const"] = RecordTypeDiscriminator(TypeName(construction.GetterType)) },
            },
            ["x-record-wrapper-type"] = TypeName(construction.GetterType),
            ["x-record-reference-target"] = TypeName(construction.GetterType.GetGenericArguments()[0]),
        };
    }
    ...
    private static JsonObject CreateFormLinkOrIndexSchema(RecordValueConstructionModel construction)
    {
        return new JsonObject
        {
            ["$ref"] = SchemaTypeUri("record.form-link-or-index"),
            ["properties"] = new JsonObject
            {
                ["$type"] = new JsonObject { ["const"] = RecordTypeDiscriminator(TypeName(construction.GetterType)) },
            },
            ["x-record-wrapper-type"] = TypeName(construction.GetterType),
            ["x-record-reference-target"] = TypeName(construction.GetterType.GetGenericArguments()[0]),
            ["x-record-owner-derived-mode"] = true,
        };
    }
    ...
    private static JsonObject CreateAssetLinkSchema(RecordValueConstructionModel construction)
    {
        return new JsonObject
        {
            ["$ref"] = SchemaTypeUri("record.asset-link"),
            ["properties"] = new JsonObject
            {
                ["$type"] = new JsonObject { ["const"] = RecordTypeDiscriminator(TypeName(construction.GetterType)) },
            },
            ["x-record-wrapper-type"] = TypeName(construction.GetterType),
            ["x-record-asset-type"] = TypeName(construction.GetterType.GetGenericArguments()[0]),
        };
    }
    ...
    private JsonObject CreateCollectionSchema(RecordValueConstructionModel construction)
    {
        return new JsonObject
        {
            ["type"] = "array",
            ...
            ["x-record-collection-type"] = TypeName(construction.MutableType),
        };
    }
    ...
    private JsonObject CreateArray2dSchema(RecordValueConstructionModel construction)
    {
        ...
        return new JsonObject
        {
            ...
            ["x-record-array-type"] = TypeName(construction.MutableType),
            ["x-row-major"] = true,
        };
    }

    /// <summary>Creates one anyOf reference set for an abstract installed record union.</summary>
    /// <param name="polymorphicType">The abstract mutable Mutagen type.</param>
    ...
    private JsonObject CreatePolymorphicSchema(Type polymorphicType)
    {
        ...
        {
            ["oneOf"] = alternatives,
            ["x-record-polymorphic-type"] = TypeName(polymorphicType),
        };
    }
    ...
    private static string EmitLeafSchema(string name)
    {
        ...
        schema["title"] = name;
        schema["description"] = "Generated Mutagen wire leaf used by Starfield indexed-field construction.";
        switch (name)
        {
            case "record.single":
            ...
            case "record.form-link":
                ...
                schema["x-default-unavailable-reason"] = "A generic typed FormLink default requires a concrete reference target.";
                break;
            case "record.form-link-or-index":
                AddClosedObject(schema, new JsonObject
                {
                    ["$type"] = new JsonObject { ["type"] = "string", ["minLength"] = 1 },
                    ...
                    ["link"] = ReferenceSchema("record.form-link"),
                }, "$type", "index", "link");
                schema["x-default-unavailable-reason"] = "A link-or-index default requires a concrete reference target and owner flags.";
                break;
            case "record.translated-string":
                AddClosedObject(schema, new JsonObject
                {
                    ["$type"] = new JsonObject { ["const"] = RecordTypeDiscriminator("Mutagen.Bethesda.Strings.ITranslatedStringGetter") },
                    ["value"] = CreateTranslatedStringValueSchema(),
                }, "$type", "value");
                break;
            case "record.asset-link":
                AddClosedObject(schema, new JsonObject
                {
                    ["$type"] = new JsonObject { ["type"] = "string", ["minLength"] = 1 },
            ...
            case "record.array2d":
                ...
                schema["x-default-unavailable-reason"] = "A generic Array2d default requires a concrete Mutagen element type.";
                break;
            default:
    ...
    /// <param name="recordTypeName">The installed Mutagen or stable leaf type name.</param>
    /// <returns>The ordered schema root.</returns>
    private static JsonObject SchemaObject(string nodeName, string recordTypeName)
    {
        return new JsonObject
        {
            ["$schema"] = "https://json-schema.org/draft/2020-12/schema",
            ["$id"] = SchemaTypeUri(nodeName),
            ["x-record-type"] = recordTypeName,
        };
    }
    ...
    /// <summary>Returns the package-scoped wire discriminator for one Mutagen type identity.</summary>
    /// <param name="recordTypeName">The Mutagen full type identity.</param>
    /// <returns>The exact writer-compatible discriminator.</returns>
    private static string RecordTypeDiscriminator(string recordTypeName)
    {
        return $"Mutagen.Bethesda.Starfield/{PackageVersion}:{recordTypeName}";
    }
    ...
    /// <param name="description">The Mutagen floating-point kind.</param>
    ...
    private static JsonObject FloatVectorProperties(params string[] names)
    {
        ...
        {
            properties[name] = ReferenceSchema("record.single");
        }
    ...
    private static string EmitTranslatedStringDefault()
    {
        ...
        {
            type = RecordTypeDiscriminator("Mutagen.Bethesda.Strings.ITranslatedStringGetter"),
            value = new
            {
