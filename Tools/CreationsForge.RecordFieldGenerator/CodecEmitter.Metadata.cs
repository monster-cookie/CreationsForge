...
/// <content>Emits the coverage manifest and provides deterministic Mutagen metadata and source-file helpers.</content>
internal sealed partial class CodecEmitter
{
    ...
    private string EmitManifest()
    {
        var manifest = new
        {
            format = "creationsforge-starfield-Mutagen-field-manifest-v1",
            ...
            {
                recordType = model.MutableType.FullName,
    ...
    /// <summary>Classifies the statically callable construction surface of one concrete Mutagen model.</summary>
    ...
    private static RecordTypeConstructionModel CreateTypeConstructionModel(Type mutableType)
    {
        ...
        {
            { IsPublic: true } => RecordTypeConstructionKind.PublicParameterlessConstructor,
            not null => RecordTypeConstructionKind.NonPublicParameterlessConstructor,
            _ => RecordTypeConstructionKind.ParameterizedConstructorOnly,
        };
        var requiredMemberNames = mutableType
            ...
            .ToArray();
        return new RecordTypeConstructionModel(kind, parameterlessConstructor, requiredMemberNames);
    }

    /// <summary>Creates the recursive construction category for one mutable/getter value pair.</summary>
    /// <param name="mutableType">The mutable Mutagen API type.</param>
    /// <param name="getterType">The corresponding getter API type.</param>
    /// <param name="nullability">The reflected getter nullability graph.</param>
    /// <param name="location">The record field path used in generation failures.</param>
    /// <returns>The complete recursive value construction model.</returns>
    private static RecordValueConstructionModel CreateConstructionModel(
        Type mutableType,
        Type getterType,
        ...
        string location)
    {
        ...
        if (mutableNullable is not null || getterNullable is not null)
        {
            ...
            return new RecordValueConstructionModel(
                RecordValueConstructionKind.Nullable,
                mutableType,
                getterType,
                nullability,
        ...
        if (IsCollection(getterType) || IsArray2d(getterType))
        {
            var kind = IsCollection(getterType)
                ? RecordValueConstructionKind.Collection
                : RecordValueConstructionKind.Array2d;
            return new RecordValueConstructionModel(
                kind,
                mutableType,
                getterType,
        ...
        return new RecordValueConstructionModel(constructionKind, mutableType, getterType, nullability);
    }
    ...
    /// <summary>Classifies one non-nullable, non-collection Mutagen construction terminal.</summary>
    /// <param name="mutableType">The mutable Mutagen API type.</param>
    /// <param name="getterType">The corresponding getter API type.</param>
    /// <param name="location">The record field path used in generation failures.</param>
    /// <returns>The exact terminal construction category.</returns>
    private static RecordValueConstructionKind GetTerminalConstructionKind(
        Type mutableType,
        Type getterType,
        string location)
    {
        ...
        {
            return RecordValueConstructionKind.String;
        }

        if (getterType == typeof(bool))
        {
            return RecordValueConstructionKind.Boolean;
        }

        if (getterType == typeof(float))
        {
            return RecordValueConstructionKind.Single;
        }

        if (getterType == typeof(double))
        {
            return RecordValueConstructionKind.Double;
        }

        if (getterType.IsEnum)
        {
            return RecordValueConstructionKind.Enum;
        }

        if (getterType.IsPrimitive || getterType == typeof(decimal))
        {
            return RecordValueConstructionKind.Numeric;
        }

        if (getterType == typeof(Guid))
        {
            return RecordValueConstructionKind.Guid;
        }

        if (getterType == typeof(System.Drawing.Color))
        {
            return RecordValueConstructionKind.Color;
        }

        if (getterType.FullName == "Noggog.P2Float")
        {
            return RecordValueConstructionKind.P2Float;
        }

        if (getterType.FullName == "Noggog.P2Int")
        {
            return RecordValueConstructionKind.P2Int;
        }

        if (getterType.FullName == "Noggog.P3Float")
        {
            return RecordValueConstructionKind.P3Float;
        }

        if (getterType.FullName == "Mutagen.Bethesda.Strings.ITranslatedStringGetter")
        {
            return RecordValueConstructionKind.TranslatedString;
        }

        if (IsMemory(getterType))
        {
            return RecordValueConstructionKind.Memory;
        }

        if (IsFormLink(getterType))
        {
            return RecordValueConstructionKind.FormLink;
        }

        if (IsFormLinkOrIndex(getterType))
        {
            return RecordValueConstructionKind.FormLinkOrIndex;
        }

        if (IsAssetLink(getterType))
        {
            return RecordValueConstructionKind.AssetLink;
        }
        ...
        if (getterType.Namespace == typeof(FormList).Namespace && getterType.IsInterface)
        {
            return mutableType.IsAbstract || mutableType.IsInterface
                ? RecordValueConstructionKind.NestedPolymorphic
                : RecordValueConstructionKind.NestedConcrete;
        }
    ...
    private static StringBuilder SourceHeader()
    {
        ...
        output.AppendLine("// Generated from Mutagen.Bethesda.Starfield 0.55.0-alpha.53 FieldIndex and getter metadata.");
        output.AppendLine("// Run: dotnet run --project Tools/CreationsForge.RecordFieldGenerator/CreationsForge.RecordFieldGenerator.csproj -- generate .");
    ...
    /// <returns>The matching mutable Mutagen model class.</returns>
    private Type ResolveMutable(Type getterType)
    {
        ...
        throw new InvalidOperationException($"No mutable Mutagen model maps to getter {getterType.FullName}.");
    }

    /// <summary>
    /// Returns record fields in exact generated FieldIndex order.
    /// </summary>
    /// <param name="type">The mutable Mutagen model class.</param>
    ...
    /// <param name="name">The indexed record field name.</param>
    ...
    /// <summary>
    /// Finds a generated FieldIndex enum for one mutable Mutagen model class.
    /// </summary>
    /// <param name="type">The mutable Mutagen model class.</param>
    ...
    /// </summary>
    /// <param name="type">The Mutagen or framework type to format.</param>
    ...
    /// <summary>Identifies ordinary and nullable Mutagen form-link getter shapes.</summary>
    /// <param name="type">The field type to classify.</param>
    /// <returns>True for supported Mutagen form-link wrappers.</returns>
    ...
    /// <summary>Identifies owner-sensitive record link-or-index getter shapes.</summary>
    /// <param name="type">The field type to classify.</param>
    ...
    /// <summary>Identifies record asset-link getter and mutable shapes.</summary>
    /// <param name="type">The field type to classify.</param>
    /// <returns>True for supported record asset-link wrappers.</returns>
