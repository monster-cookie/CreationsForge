...
/// <content>Emits recursive wire-to-record value construction and shared Starfield reader leaves.</content>
internal sealed partial class CodecEmitter
{
    ...
    private static StringBuilder ReaderGeneratedHeader()
    {
        ...
        output.AppendLine("using CreationsForge.Core.Engine.RecordWire;");
    ...
    private static bool AllowsNull(RecordValueConstructionModel construction)
    {
    ...
    /// <param name="output">The generated source builder.</param>
    /// <param name="enumType">The concrete Mutagen enum type.</param>
    ...
    private static string GetNumericReader(Type numericType)
    {
        ...
        throw new InvalidOperationException($"Numeric type {numericType} has no bounded Mutagen wire reader.");
    }
        ...
        string indent)
    {
        ...
        output.AppendLine($"{indent}{{");
        output.AppendLine($"{indent}    context.Reject({variableName}Data.GivenPathValue, $\"Invalid record asset path: {{exception.Message}}\");");
        output.AppendLine($"{indent}    throw new InvalidOperationException(\"RecordWireReadContext.Reject unexpectedly returned.\", exception);");
        output.AppendLine($"{indent}}}");
        ...
        output.AppendLine($"{indent}{{");
        output.AppendLine($"{indent}    context.Reject({variableName}Data.IsNullValue, \"Asset-link null state does not match the supplied Mutagen path.\");");
        output.AppendLine($"{indent}}}");
        output.AppendLine($"{indent}if (!string.Equals({variableName}.GivenPath, {variableName}Data.GivenPath, StringComparison.Ordinal))");
        output.AppendLine($"{indent}{{");
        output.AppendLine($"{indent}    context.Reject({variableName}Data.GivenPathValue, \"Asset-link Mutagen path does not round-trip exactly.\");");
        output.AppendLine($"{indent}}}");
        ...
        output.AppendLine($"{indent}{{");
        output.AppendLine($"{indent}    context.Reject({variableName}Data.DataRelativePathValue.Value, \"Asset-link data-relative path does not match the supplied Mutagen path.\");");
        output.AppendLine($"{indent}}}");
        ...
        output.AppendLine($"{indent}{{");
        output.AppendLine($"{indent}    context.Reject({variableName}Data.ExtensionValue.Value, \"Asset-link extension does not match the supplied Mutagen path.\");");
        output.AppendLine($"{indent}}}");
    }
    ...
    /// <summary>Emits one bounded row-major two-dimensional Mutagen collection construction.</summary>
    /// <param name="output">The generated source builder.</param>
    ...
    /// <summary>Emits shared closed-object leaf readers used by generated Mutagen construction.</summary>
    ...
    {
        output.AppendLine("    /// <summary>Defines a closed two-dimensional Mutagen array value.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape Array2dReadShape = new(\"Array2d\", new[] { \"width\", \"height\", \"rows\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines a closed two-dimensional float vector.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape P2FloatReadShape = new(\"P2Float\", new[] { \"X\", \"Y\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines a closed two-dimensional integer vector.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape P2IntReadShape = new(\"P2Int\", new[] { \"X\", \"Y\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines a closed three-dimensional float vector.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape P3FloatReadShape = new(\"P3Float\", new[] { \"X\", \"Y\", \"Z\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines the complete redundant Mutagen color representation.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape ColorReadShape = new(\"Color\", new[] { \"argb\", \"a\", \"r\", \"g\", \"b\", \"isEmpty\", \"isKnownColor\", \"isNamedColor\", \"isSystemColor\", \"name\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines a typed ordinary or nullable FormKey link wrapper.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape FormLinkReadShape = new(\"FormLink\", new[] { \"$type\", \"isNull\", \"formKey\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines an owner-sensitive link-or-index wrapper.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape FormLinkOrIndexReadShape = new(\"FormLinkOrIndex\", new[] { \"$type\", \"index\", \"link\" }, new[] { \"usesLink\", \"usesAlias\", \"usesPackageData\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines a typed asset-link wrapper.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape AssetLinkReadShape = new(\"AssetLink\", new[] { \"$type\", \"value\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines the redundant asset-link path payload.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape AssetLinkValueReadShape = new(\"asset-link value\", new[] { \"isNull\", \"givenPath\" }, new[] { \"dataRelativePath\", \"extension\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines a translated-string wrapper.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape TranslatedStringReadShape = new(\"TranslatedString\", new[] { \"$type\", \"value\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines a complete translated-string payload.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape TranslatedStringValueReadShape = new(\"translated-string value\", new[] { \"targetLanguage\", \"value\", \"translations\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Defines one translated-string language entry.</summary>");
        output.AppendLine("    private static readonly RecordWireObjectShape TranslationEntryReadShape = new(\"translation entry\", new[] { \"language\", \"value\" });");
        output.AppendLine();
        output.AppendLine("    /// <summary>Validates one package-scoped wire discriminator against the statically selected Mutagen type.</summary>");
        ...
        output.AppendLine("    /// <param name=\"recordTypeName\">The expected Mutagen full name without package scope.</param>");
        output.AppendLine("    private static void ReadAndValidateType(RecordWireReadContext context, RecordWireValue value, string recordTypeName)");
        output.AppendLine("    {");
        output.AppendLine("        var actual = context.ReadString(value);");
        output.AppendLine("        var expected = RecordTypeNamespace + recordTypeName;");
        output.AppendLine("        if (!string.Equals(actual, expected, StringComparison.Ordinal))");
        output.AppendLine("        {");
        output.AppendLine("            context.Reject(value, $\"Expected plugin type discriminator '{expected}'.\");");
        output.AppendLine("        }");
        output.AppendLine("    }");
        output.AppendLine();
        ...
        output.AppendLine("    private static T RejectValue<T>(RecordWireReadContext context, RecordWireValue value, string message)");
        output.AppendLine("    {");
        output.AppendLine("        context.Reject(value, message);");
        output.AppendLine("        throw new InvalidOperationException(\"RecordWireReadContext.Reject unexpectedly returned.\");");
        output.AppendLine("    }");
        output.AppendLine();
        output.AppendLine("    /// <summary>Reads a nullable record string from an explicitly present required property.</summary>");
        output.AppendLine("    /// <param name=\"context\">The request-local bounded read context.</param>");
        ...
        output.AppendLine("    private static string? ReadNullableString(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        ...
        output.AppendLine("    /// <returns>The decoded FormKey, or null for the Mutagen null-link state.</returns>");
        output.AppendLine("    private static FormKey? ReadFormLinkKey(RecordWireReadContext context, RecordWireValue value, string expectedWrapperType)");
        output.AppendLine("    {");
        ...
        output.AppendLine("    private static (bool? UsesLink, RecordWireValue? UsesLinkValue, bool? UsesAlias, RecordWireValue? UsesAliasValue, bool? UsesPackageData, RecordWireValue? UsesPackageDataValue, uint? Index, FormKey? Link) ReadFormLinkOrIndex(RecordWireReadContext context, RecordWireValue value, string expectedWrapperType, string expectedLinkType)");
        output.AppendLine("    {");
        ...
        output.AppendLine("    private static (bool IsNull, RecordWireValue IsNullValue, string GivenPath, RecordWireValue GivenPathValue, string? DataRelativePath, RecordWireValue? DataRelativePathValue, string? Extension, RecordWireValue? ExtensionValue) ReadAssetLink(RecordWireReadContext context, RecordWireValue value, string expectedWrapperType)");
        output.AppendLine("    {");
        ...
        output.AppendLine("    private static TranslatedString ReadTranslatedString(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        ...
        output.AppendLine("    private static TEnum ReadNamedEnum<TEnum>(RecordWireReadContext context, RecordWireValue value) where TEnum : struct, Enum");
        output.AppendLine("    {");
        ...
        output.AppendLine("    /// <returns>The exact Mutagen vector.</returns>");
        output.AppendLine("    private static P2Float ReadP2Float(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        ...
        output.AppendLine("    /// <param name=\"value\">The context-owned vector object.</param>");
        output.AppendLine("    /// <returns>The exact Mutagen vector.</returns>");
        output.AppendLine("    private static P2Int ReadP2Int(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        ...
        output.AppendLine("    /// <param name=\"value\">The context-owned vector object.</param>");
        output.AppendLine("    /// <returns>The exact Mutagen vector.</returns>");
        output.AppendLine("    private static P3Float ReadP3Float(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        ...
        output.AppendLine("    }");
        output.AppendLine();
        output.AppendLine("    /// <summary>Reads one complete Mutagen color and validates every redundant representation field.</summary>");
        output.AppendLine("    /// <param name=\"context\">The request-local bounded read context.</param>");
        output.AppendLine("    /// <param name=\"value\">The context-owned color object.</param>");
        output.AppendLine("    /// <returns>The exact reconstructible Mutagen color.</returns>");
        output.AppendLine("    private static System.Drawing.Color ReadColor(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        ...
        output.AppendLine("        {");
        output.AppendLine("            context.Reject(value, \"Color metadata is not an exact reconstructible Mutagen color.\");");
        output.AppendLine("        }");
        output.AppendLine("        return result;");
        output.AppendLine("    }");
    }
