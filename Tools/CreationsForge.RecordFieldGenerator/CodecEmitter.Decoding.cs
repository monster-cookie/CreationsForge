...
/// <content>Emits complete static Starfield Mutagen readers and their exact type dispatch.</content>
internal sealed partial class CodecEmitter
{
    ...
    private string EmitReaderEntryPoints()
    {
        ...
        output.AppendLine();
        output.AppendLine("/// <content>Reads complete Starfield component and condition graphs through generated static Mutagen construction.</content>");
        output.AppendLine("internal static partial class StarfieldNestedFieldCodec");
        output.AppendLine("{");
        output.AppendLine("    /// <summary>Reads one complete concrete Starfield component from its closed Mutagen wire object.</summary>");
        ...
        output.AppendLine("    internal static AComponent ReadComponent(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        ...
        output.AppendLine("    internal static Condition ReadCondition(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        output.AppendLine("        ArgumentNullException.ThrowIfNull(context);");
        output.AppendLine("        return ReadPolymorphicCondition(context, value);");
        output.AppendLine("    }");
        output.AppendLine();
        output.AppendLine("    /// <summary>Reads any generated concrete Mutagen type by its exact catalog node name.</summary>");
        output.AppendLine("    /// <param name=\"context\">The request-local bounded read context.</param>");
        output.AppendLine("    /// <param name=\"value\">The context-owned Mutagen object value.</param>");
        output.AppendLine("    /// <param name=\"recordTypeName\">The exact mutable Mutagen full name.</param>");
        output.AppendLine("    /// <returns>The fully validated concrete mutable Mutagen object.</returns>");
        ...
        output.AppendLine("    internal static object ReadRecordType(RecordWireReadContext context, RecordWireValue value, string recordTypeName)");
        output.AppendLine("    {");
        output.AppendLine("        ArgumentNullException.ThrowIfNull(context);");
        ...
        output.AppendLine("            _ => throw new NotSupportedException($\"Plugin type '{recordTypeName}' is outside generated Starfield coverage.\"),");
        output.AppendLine("        };");
        output.AppendLine("    }");
        output.AppendLine();
        output.AppendLine("    /// <summary>Writes any generated concrete Mutagen type selected by its exact catalog node name.</summary>");
        output.AppendLine("    /// <param name=\"writer\">The caller-owned JSON writer.</param>");
        output.AppendLine("    /// <param name=\"recordTypeName\">The exact mutable Mutagen full name.</param>");
        output.AppendLine("    /// <param name=\"value\">The matching concrete record getter.</param>");
        output.AppendLine("    /// <param name=\"cancellationToken\">A token observed throughout traversal.</param>");
        output.AppendLine("    /// <exception cref=\"ArgumentException\">Thrown when the type name is empty.</exception>");
        output.AppendLine("    /// <exception cref=\"ArgumentNullException\">Thrown when the writer or record value is null.</exception>");
        ...
        output.AppendLine("    /// <exception cref=\"NotSupportedException\">Thrown when the type name is outside generated coverage.</exception>");
        output.AppendLine("    internal static void WriteRecordType(Utf8JsonWriter writer, string recordTypeName, object value, CancellationToken cancellationToken)");
        output.AppendLine("    {");
        ...
        foreach (var model in Models)
        {
            ...
            output.AppendLine($"                throw new InvalidOperationException(\"Plugin value does not implement {Escape(getterName)}.\");");
        }

        output.AppendLine("            default:");
        output.AppendLine("                throw new NotSupportedException($\"Plugin type '{recordTypeName}' is outside generated Starfield coverage.\");");
        output.AppendLine("        }");
        output.AppendLine("    }");
        output.AppendLine();
        output.AppendLine("    /// <summary>Constructs and writes the actual installed default for any generated concrete Mutagen type.</summary>");
        output.AppendLine("    /// <param name=\"writer\">The caller-owned JSON writer.</param>");
        output.AppendLine("    /// <param name=\"recordTypeName\">The exact mutable Mutagen full name.</param>");
        output.AppendLine("    /// <param name=\"cancellationToken\">A token observed throughout traversal.</param>");
        output.AppendLine("    /// <exception cref=\"ArgumentException\">Thrown when the type name is empty.</exception>");
        ...
        output.AppendLine("    /// <exception cref=\"NotSupportedException\">Thrown when the type name is outside generated coverage.</exception>");
        output.AppendLine("    internal static void WriteDefaultRecordType(Utf8JsonWriter writer, string recordTypeName, CancellationToken cancellationToken)");
        output.AppendLine("    {");
        output.AppendLine("        ArgumentNullException.ThrowIfNull(writer);");
        output.AppendLine("        ArgumentException.ThrowIfNullOrWhiteSpace(recordTypeName);");
        output.AppendLine("        switch (recordTypeName)");
    ...
    /// <summary>Emits one partition of concrete Mutagen reader methods and closed object shapes.</summary>
    /// <param name="models">The concrete Mutagen models assigned to this partition.</param>
    /// <returns>Complete generated reader source.</returns>
    private string EmitReaders(IEnumerable<RecordTypeModel> models)
    {
        var output = ReaderGeneratedHeader();
        foreach (var model in models)
        {
    ...
    private void EmitPolymorphicReaderDispatcher(StringBuilder output, Type polymorphicType)
    {
        ...
        output.AppendLine($"    private static readonly RecordWireObjectShape Polymorphic{polymorphicType.Name}ReadShape = new(");
        ...
        output.AppendLine("        });");
        output.AppendLine();
        output.AppendLine($"    /// <summary>Reads one concrete Mutagen {polymorphicType.Name} selected by its exact package-scoped discriminator.</summary>");
        output.AppendLine("    /// <param name=\"context\">The request-local bounded read context.</param>");
        output.AppendLine("    /// <param name=\"value\">The context-owned Mutagen object value.</param>");
        output.AppendLine("    /// <returns>The fully validated concrete mutable record value.</returns>");
        output.AppendLine($"    private static {TypeName(polymorphicType)} ReadPolymorphic{polymorphicType.Name}(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        ...
        {
            output.AppendLine($"            RecordTypeNamespace + \"{Escape(model.MutableType.FullName!)}\" => Read{model.MutableType.Name}(context, value),");
        }

        output.AppendLine($"            _ => RejectValue<{TypeName(polymorphicType)}>(context, typeValue, $\"Plugin type '{{typeName}}' is not a generated {Escape(getter.FullName!)} implementation.\"),");
        output.AppendLine("        };");
        output.AppendLine("    }");
        output.AppendLine();
    }

    /// <summary>Emits the exact closed object shape used by one concrete Mutagen reader.</summary>
    /// <param name="output">The generated source builder.</param>
    /// <param name="model">The concrete Mutagen model.</param>
    private static void EmitConcreteReadShape(StringBuilder output, RecordTypeModel model)
    {
        ...
        output.AppendLine($"    /// <summary>Defines the complete required and optional wire property shape for Mutagen {model.MutableType.Name}.</summary>");
        output.AppendLine($"    private static readonly RecordWireObjectShape {model.MutableType.Name}ReadShape = new(");
    ...
    /// <summary>Emits a complete concrete Mutagen reader with owner-linked construction deferred until its owner exists.</summary>
    /// <param name="output">The generated source builder.</param>
    /// <param name="model">The concrete Mutagen model.</param>
    private void EmitConcreteReader(StringBuilder output, RecordTypeModel model)
    {
        if (model.Construction.Kind != RecordTypeConstructionKind.PublicParameterlessConstructor
            || model.Construction.RequiredMemberNames.Count != 0)
        {
            throw new InvalidOperationException($"Plugin type {model.MutableType.FullName} is not directly default constructible.");
        }

        var ownerLinkedFields = model.Fields
            .Where(static field => field.Construction.Kind == RecordValueConstructionKind.FormLinkOrIndex)
            .ToArray();
        ...
        {
            throw new InvalidOperationException($"Plugin owner {model.MutableType.FullName} does not implement IFormLinkOrIndexFlagGetter.");
        }

        if (ownerLinkedFields.Any(static field => IsInitOnly(field.MutableProperty)))
        {
            throw new InvalidOperationException($"Plugin owner {model.MutableType.FullName} exposes an init-only owner-linked field that cannot be bound after construction.");
        }
            ...
            foreach (var prerequisiteName in new[] { "UseAliases", "UsePackageData" })
            {
                ...
                if (prerequisite is null || prerequisite.Ordinal >= ownerLinkedField.Ordinal)
                {
                    throw new InvalidOperationException(
                        $"Plugin owner {model.MutableType.FullName} must expose {prerequisiteName} before owner-linked field {ownerLinkedField.Name}.");
                }
            }
        }

        output.AppendLine($"    /// <summary>Reads every indexed field of Mutagen {model.MutableType.Name} in FieldIndex order.</summary>");
        output.AppendLine("    /// <param name=\"context\">The request-local bounded read context.</param>");
        output.AppendLine("    /// <param name=\"value\">The context-owned Mutagen object value.</param>");
        output.AppendLine("    /// <returns>The fully validated concrete mutable record value.</returns>");
        output.AppendLine($"    private static {TypeName(model.MutableType)} Read{model.MutableType.Name}(RecordWireReadContext context, RecordWireValue value)");
        output.AppendLine("    {");
        ...
        {
            var field = model.Fields[index];
            if (field.Construction.Kind == RecordValueConstructionKind.FormLinkOrIndex)
            {
        ...
        if (validatesDerivedFunction)
        {
            ...
            output.AppendLine("        {");
            output.AppendLine("            context.Reject(functionValue, $\"Derived Function {function.Value} does not match concrete Mutagen type value {(ushort)((IConditionDataGetter)result).Function}.\");");
            output.AppendLine("        }");
        }
    ...
    /// <param name="property">The mutable Mutagen property.</param>
        ...
        string indent)
    {
        ...
        output.AppendLine($"{indent}{{");
        output.AppendLine($"{indent}    context.Reject({valueExpression}.UsesLinkValue!.Value, \"Link-or-index usesLink does not match the decoded Mutagen owner state.\");");
        output.AppendLine($"{indent}}}");
        ...
        output.AppendLine($"{indent}{{");
        output.AppendLine($"{indent}    context.Reject({valueExpression}.UsesAliasValue!.Value, \"Link-or-index usesAlias does not match the decoded Mutagen owner state.\");");
        output.AppendLine($"{indent}}}");
        ...
        output.AppendLine($"{indent}{{");
        output.AppendLine($"{indent}    context.Reject({valueExpression}.UsesPackageDataValue!.Value, \"Link-or-index usesPackageData does not match the decoded Mutagen owner state.\");");
        output.AppendLine($"{indent}}}");
        if (nullable)
        {
            indent = indent[..^4];
