...
internal sealed partial class CodecEmitter
{
    ...
    /// <summary>All concrete indexed types reachable from the owned native roots.</summary>
    private readonly IReadOnlyList<NativeTypeModel> Models;
    ...
    {
        var productionRoot = Path.Combine(repositoryRoot, "CreationsForge.Starfield", "Native", "NativeInspection", "Nested");
        ...
        var artifacts = new List<KeyValuePair<string, string>>
        {
            ...
            new(Path.Combine(generatedRoot, "StarfieldGeneratedNativeFieldSchema.g.cs"), EmitSchemaCatalog()),
        };
        ...
        artifacts.Add(new KeyValuePair<string, string>(
            Path.Combine(productionRoot, "StarfieldNativeFieldManifest.json"),
            EmitManifest()));
    ...
    /// <summary>
    /// Traverses concrete native indexed fields and records abstract dispatch boundaries.
    /// </summary>
    /// <param name="type">The mutable native type to visit.</param>
    /// <param name="visited">Concrete indexed types already included.</param>
    /// <param name="polymorphic">Abstract native types requiring concrete dispatch.</param>
    private void Visit(Type type, HashSet<Type> visited, HashSet<Type> polymorphic)
    {
        ...
        if (type.IsAbstract || type.IsInterface)
        {
            ...
            {
                throw new InvalidOperationException($"No concrete native types implement {type.FullName}.");
            }
        ...
        {
            throw new InvalidOperationException($"Unclassified non-indexed native type {type.FullName}.");
        }
    ...
    /// <summary>
    /// Creates one concrete visitor model from native FieldIndex and getter metadata.
    /// </summary>
    /// <param name="mutableType">The installed concrete mutable native class.</param>
    /// <returns>A closed visitor model with fields in native index order.</returns>
    private static NativeTypeModel CreateModel(Type mutableType)
    {
        ...
        var fields = Enum.GetNames(fieldIndex)
            ...
            .ToArray();
        return new NativeTypeModel(mutableType, getterType, fields, CreateTypeConstructionModel(mutableType));
    }
    ...
    /// <param name="location">The native field location used in failures.</param>
    private void ValidateValueShape(Type mutableType, Type getterType, string location)
    {
        ...
        if (getterType.Namespace == typeof(FormList).Namespace && getterType.IsInterface)
        {
            ...
            {
                throw new InvalidOperationException($"Native getter mapping mismatch at {location}: {mutableType} / {getterType} resolves {resolvedMutable}.");
            }
    ...
    /// <summary>
    /// Emits every root and abstract native concrete-type dispatcher.
    /// </summary>
    ...
    private string EmitVisitors(IEnumerable<NativeTypeModel> models)
    {
        var output = GeneratedHeader();
        foreach (var model in models)
        {
    ...
    /// <summary>
    /// Emits ordered direct type dispatch for one abstract native model boundary.
    /// </summary>
    /// <param name="output">The generated source builder.</param>
    /// <param name="polymorphicType">The abstract mutable native model type.</param>
    private void EmitPolymorphicDispatcher(StringBuilder output, Type polymorphicType)
    {
        ...
        output.AppendLine("    /// <param name=\"value\">The concrete native getter.</param>");
        ...
        output.AppendLine($"    private static void WritePolymorphic{polymorphicType.Name}(Utf8JsonWriter writer, {TypeName(getter)} value, CancellationToken cancellationToken, NativeJsonWriteContext? context)");
        output.AppendLine("    {");
        ...
        output.AppendLine($"        throw new NotSupportedException(\"The installed Mutagen {polymorphicType.Name} implementation is not covered by the generated Starfield native field manifest.\");");
        output.AppendLine("    }");
        output.AppendLine();
        ...
        output.AppendLine("    /// <param name=\"before\">The prior native getter, or null.</param>");
        output.AppendLine("    /// <param name=\"after\">The resulting native getter, or null.</param>");
        output.AppendLine("    /// <param name=\"path\">The stable native field path.</param>");
        ...
        output.AppendLine("    /// <param name=\"value\">The concrete native getter.</param>");
        output.AppendLine("    /// <returns>The canonical package-scoped native type name.</returns>");
        ...
        foreach (var model in derived)
        {
            ...
            output.AppendLine("        {");
            output.AppendLine($"            return NativeTypeNamespace + \"{Escape(model.MutableType.FullName!)}\";");
            output.AppendLine("        }");
            output.AppendLine();
        }
    ...
    /// <summary>
    /// Emits one complete direct typed JSON writer in native FieldIndex order.
    /// </summary>
    /// <param name="output">The generated source builder.</param>
    /// <param name="model">The concrete native visitor model.</param>
    private void EmitConcreteWriter(StringBuilder output, NativeTypeModel model)
    {
        output.AppendLine($"    /// <summary>Writes every indexed field of native {model.MutableType.Name} in FieldIndex order.</summary>");
        output.AppendLine("    /// <param name=\"writer\">The caller-owned JSON writer.</param>");
        output.AppendLine("    /// <param name=\"value\">The concrete native getter.</param>");
        output.AppendLine("    /// <param name=\"cancellationToken\">A token observed throughout traversal.</param>");
        output.AppendLine("    /// <param name=\"context\">The per-call write mode and validation state, or null for read-view output.</param>");
        output.AppendLine($"    private static void Write{model.MutableType.Name}(Utf8JsonWriter writer, {TypeName(model.GetterType)} value, CancellationToken cancellationToken, NativeJsonWriteContext? context)");
        output.AppendLine("    {");
        output.AppendLine("        cancellationToken.ThrowIfCancellationRequested();");
    ...
    /// <summary>
    /// Emits one complete direct typed native comparer in FieldIndex order.
    /// </summary>
    /// <param name="output">The generated source builder.</param>
    /// <param name="model">The concrete native visitor model.</param>
    private void EmitConcreteComparer(StringBuilder output, NativeTypeModel model)
    {
        output.AppendLine($"    /// <summary>Compares every indexed field of native {model.MutableType.Name} without native equality or JSON authority.</summary>");
        output.AppendLine("    /// <param name=\"before\">The prior concrete native getter.</param>");
        output.AppendLine("    /// <param name=\"after\">The resulting concrete native getter.</param>");
        output.AppendLine("    /// <param name=\"path\">The stable native object path.</param>");
        output.AppendLine("    /// <param name=\"changes\">The caller-owned ordered change collection.</param>");
        output.AppendLine("    /// <param name=\"cancellationToken\">A token observed throughout traversal.</param>");
    ...
    /// </summary>
    /// <param name="output">The generated source builder.</param>
    /// <param name="mutableType">The mutable field type used to resolve nested native models.</param>
    ...
    private void EmitWriteValue(StringBuilder output, Type mutableType, Type getterType, string value, string indent)
    {
        ...
        {
            output.AppendLine($"{indent}NativeJsonLeafWriter.WriteString(writer, {value}, context);");
        }
        ...
        {
            output.AppendLine($"{indent}NativeJsonLeafWriter.WriteSingle(writer, {value});");
        }
        else if (getterType == typeof(double))
        {
            output.AppendLine($"{indent}NativeJsonLeafWriter.WriteDouble(writer, {value});");
        }
        ...
        else if (getterType.FullName == "Noggog.P2Float")
        {
            ...
            output.AppendLine($"{indent}NativeJsonLeafWriter.WriteSingle(writer, {value}.X);");
            output.AppendLine($"{indent}writer.WritePropertyName(\"Y\");");
            output.AppendLine($"{indent}NativeJsonLeafWriter.WriteSingle(writer, {value}.Y);");
        ...
        else if (getterType.FullName == "Noggog.P3Float")
        {
            ...
            output.AppendLine($"{indent}NativeJsonLeafWriter.WriteSingle(writer, {value}.Z);");
            output.AppendLine($"{indent}writer.WriteEndObject();");
        }
        ...
        {
            output.AppendLine($"{indent}NativeJsonLeafWriter.WriteBytes(writer, {value}.ToArray(), cancellationToken);");
        }
    ...
    /// <param name="path">The generated expression producing the stable native field path.</param>
    /// <param name="indent">The current source indentation.</param>
    ...
    /// <summary>
    /// Returns the direct exact equality expression for one non-collection native leaf.
    /// </summary>
    /// <param name="getterType">The closed getter leaf type.</param>
    /// <param name="before">The prior static getter expression.</param>
    /// <param name="after">The resulting static getter expression.</param>
    /// <returns>A generated Boolean expression that applies the leaf's native semantic equality.</returns>
    private static string GetLeafEquality(Type getterType, string before, string after)
    {
        if (getterType == typeof(float))
        {
            return $"NativeSemanticComparer.BitwiseEquals({before}, {after})";
        }
        ...
        if (getterType == typeof(string))
        {
            return $"NativeSemanticComparer.StringEquals({before}, {after})";
        }

        if (getterType.FullName == "Mutagen.Bethesda.Strings.ITranslatedStringGetter")
        {
            return $"NativeSemanticComparer.TranslatedStringEquals({before}, {after}, cancellationToken)";
        }

        if (IsMemory(getterType))
        {
            return $"NativeSemanticComparer.BytesEqual({before}.ToArray(), {after}.ToArray(), cancellationToken)";
        }

        if (IsFormLink(getterType))
        {
            return $"NativeSemanticComparer.FormLinkEquals({before}, {after})";
        }

        if (IsFormLinkOrIndex(getterType))
        {
            return $"NativeSemanticComparer.FormLinkOrIndexEquals({before}, {after})";
        }

        if (IsAssetLink(getterType))
        {
            return $"NativeSemanticComparer.AssetLinkEquals({before}, {after})";
        }

        if (getterType.FullName == "Noggog.P2Float")
        {
            return $"NativeSemanticComparer.BitwiseEquals({before}.X, {after}.X) && NativeSemanticComparer.BitwiseEquals({before}.Y, {after}.Y)";
        }
        ...
        {
            return $"NativeSemanticComparer.BitwiseEquals({before}.X, {after}.X) && NativeSemanticComparer.BitwiseEquals({before}.Y, {after}.Y) && NativeSemanticComparer.BitwiseEquals({before}.Z, {after}.Z)";
        }

        if (getterType == typeof(System.Drawing.Color))
        {
            return $"{before}.ToArgb() == {after}.ToArgb() && {before}.A == {after}.A && {before}.R == {after}.R && {before}.G == {after}.G && {before}.B == {after}.B && {before}.IsEmpty == {after}.IsEmpty && {before}.IsKnownColor == {after}.IsKnownColor && {before}.IsNamedColor == {after}.IsNamedColor && {before}.IsSystemColor == {after}.IsSystemColor && NativeSemanticComparer.StringEquals({before}.Name, {after}.Name)";
        }
    ...
    /// <summary>
    /// Emits null-aware concrete or polymorphic nested native comparison.
    /// </summary>
    /// <param name="output">The generated source builder.</param>
    /// <param name="mutableType">The mutable nested type used to select concrete or polymorphic dispatch.</param>
    /// <param name="getterType">The getter nested type used by production code.</param>
