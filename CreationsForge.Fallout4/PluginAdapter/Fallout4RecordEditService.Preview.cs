...
public sealed partial class Fallout4RecordEditService
{
        ...
        foreach (var entry in provenance)
        {
            ...
            if (afterMatches.Count != 1 || afterMatches[0] is not IFormListGetter afterBorrowed)
            {
                return PreviewFailure(
                    sources,
                    $"Staged Fallout 4 FormList '{entry.TargetFormKey}' is missing, duplicated, or belongs to another record family.",
                    allWarnings);
            }
    ...
    /// <param name="sources">The immutable plugin source lifetime.</param>
        ...
        if (provenance.BaselineKind == EditBaselineKind.OriginalOutput)
        {
            ...
            if (matches.Length != 1 || matches[0] is not IFormListGetter formList)
            {
                return EngineResult<PreviewRecord>.Failure(new EngineError(
                    ...
                    $"Original Fallout 4 output baseline for '{provenance.TargetFormKey}' is missing, duplicated, or belongs to another Mutagen family."));
            }
        ...
        for (var index = 0; index < formList.Items.Count; index++)
        {
            ...
            if (!isLive)
            {
                warnings.Add(new EngineWarning(
                    ...
                    $"Reference {formKey} at Items[{index}] does not resolve to a live record in the current workspace view."));
            }
        }
    ...
    /// <summary>Pairs an exact immutable preview context with an optional detached FormList.</summary>
    private sealed class PreviewRecord
    {
        /// <summary>Initializes one detached preview record.</summary>
        /// <param name="context">The exact record context.</param>
        ...
        /// <summary>Gets the exact record context.</summary>
