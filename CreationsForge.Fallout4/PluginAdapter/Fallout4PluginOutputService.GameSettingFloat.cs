...
/// <summary>Owns native Fallout4 GameSettingFloat allocation and exact-context override selection.</summary>
public sealed partial class Fallout4PluginOutputService
{
        ...
        CancellationToken cancellationToken)
    {
        if (!request.EditorId.StartsWith('f') || request.Xalg is not null || request.Data is { } data && !float.IsFinite(data))
        {
            return EngineResult<RecordEditMutationResult>.Failure(new EngineError(
                ...
                "Fallout 4 float game settings require an EditorID beginning with f, a finite value, and no XALG."));
        }
        ...
        if (!found.Succeeded || found.Value?.Record is null)
        {
            return EngineResult<RecordEditMutationResult>.Failure(
                found.Error ?? new EngineError(EngineErrorCode.RecordNotFound, $"Fallout 4 output has no GameSettingFloat {target}."));
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.Role == FormListEditRole.Override)
        {
            ...
            if (!readResult.Succeeded || readResult.Value is null)
            {
                return EngineResult<RecordEditIdentity>.Failure(
                    readResult.Error ?? new EngineError(EngineErrorCode.RecordNotFound, "The Fallout4 override origin could not be read."),
                ...
                readResult.Value.Record is not IGameSettingFloatGetter origin)
            {
                return EngineResult<RecordEditIdentity>.Failure(new EngineError(
                    EngineErrorCode.ValidationFailed,
                    $"Fallout4 override origin {formKey} is not one live GameSettingFloat in the selected context."),
                    warnings: readResult.Warnings);
            }
            ...
            if (current.Value?.Record is null)
            {
                ...
                {
                    return EngineResult<RecordEditIdentity>.Failure(
                        current.Error ?? new EngineError(EngineErrorCode.ValidationFailed, "The Fallout4 override was not retained in output."),
                        warnings: readResult.Warnings);
                }
            }

            var identity = new RecordEditIdentity(Guid.NewGuid(), formKey, formKey, request.Role, request.RecordType);
            candidate.TryAddEditProvenance(CreateGameSettingFloatProvenance(
                sources,
                candidate,
                identity,
        ...
        if (request.Role == FormListEditRole.ExistingOutput)
        {
            ...
            {
                return EngineResult<RecordEditIdentity>.Failure(
                    current.Error ?? new EngineError(EngineErrorCode.RecordNotFound, $"Fallout4 output has no GameSettingFloat {formKey}."));
            }

            var identity = new RecordEditIdentity(Guid.NewGuid(), formKey, null, request.Role, request.RecordType);
            candidate.TryAddEditProvenance(CreateGameSettingFloatProvenance(sources, candidate, identity, null));
            return EngineResult<RecordEditIdentity>.Success(identity);
        }

        return EngineResult<RecordEditIdentity>.Failure(new EngineError(
            EngineErrorCode.InvalidRequest,
            $"Unknown Fallout4 GameSettingFloat edit role '{request.Role}'."));
    }
        ...
        foreach (var record in mod.EnumerateMajorRecords())
        {
            ...
            if (record is not GameSettingFloat gameSetting || found is not null)
            {
                return EngineResult<GameSettingFloatSearch>.Failure(new EngineError(
                    EngineErrorCode.ValidationFailed,
                    $"Fallout4 output identity {formKey} is occupied or ambiguous across record families."));
            }
