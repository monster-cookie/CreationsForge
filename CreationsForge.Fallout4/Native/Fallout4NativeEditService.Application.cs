...
public sealed partial class Fallout4NativeEditService
{
    ...
    /// <returns>Whether the command changed immediate native candidate content, or a typed rejection.</returns>
        ...
        PreparedFormListEdit preparedEdit)
    {
        ...
        if (preparedEdit is not Fallout4PreparedFormListEdit prepared)
        {
            return MutationFailure(
                ...
                "The prepared FormList edit was not created by the Fallout 4 native adapter.");
        }
        ...
        if (matches.Count != 1 || matches[0] is not FormList formList)
        {
            return MutationFailure(
                ...
                $"Fallout 4 output identity '{targetFormKey}' is duplicated or belongs to another native record family.");
        }
        ...
        return EngineResult<NativeEditMutationResult>.Success(
            new NativeEditMutationResult(changed),
    ...
    /// <param name="sources">The immutable native sources used for new-reference checks.</param>
    ...
    /// <param name="formKey">The non-null native identity to validate.</param>
    /// <returns>A deterministic validation failure, or <see langword="null"/> for one live target of any native family.</returns>
        ...
        FormKey formKey)
    {
        ...
        return resolution.Value!.Status == ReferenceResolutionStatus.Resolved
            ...
            : new EngineError(
                EngineErrorCode.ValidationFailed,
                $"New Fallout 4 FormList item '{formKey}' does not resolve to a live native record; status was {resolution.Value.Status}.");
    }

    /// <summary>Applies an already validated private payload through concrete native setters.</summary>
    /// <param name="formList">The exact mutable native target.</param>
    /// <param name="prepared">The immutable prepared command.</param>
    /// <returns><see langword="true"/> when native content changed.</returns>
    private static bool ApplyPrepared(FormList formList, Fallout4PreparedFormListEdit prepared)
    {
        switch (prepared.Kind)
        {
            ...
            case PreparedEditKind.SetName:
                ...
                if (NativeSemanticComparer.TranslatedStringEquals(formList.Name, name, CancellationToken.None))
                {
                    return false;
                }
    ...
    /// <summary>Replaces the entire ordered native item list while preserving nulls and duplicates.</summary>
    /// <param name="formList">The mutable target FormList.</param>
    /// <param name="items">The exact desired FormKey sequence.</param>
    /// <returns><see langword="true"/> when the native sequence changed.</returns>
    ...
    /// <summary>Enumerates every native family to enforce unique output identity.</summary>
    /// <param name="mod">The complete staged output.</param>
    /// <param name="formKey">The exact identity to locate.</param>
    /// <returns>All matching records in native enumeration order.</returns>
    ...
    /// <summary>Creates a typed native mutation failure with stable source metadata.</summary>
    ...
    /// <returns>A failed native mutation result.</returns>
    ...
    /// <summary>Propagates a typed native mutation failure with stable source metadata.</summary>
    /// <param name="sources">The immutable source lifetime.</param>
    ...
    private static EngineResult<NativeEditMutationResult> MutationFailure(
        ...
        EngineError error)
    {
        return EngineResult<NativeEditMutationResult>.Failure(
            error,
            workspaceId: sources.WorkspaceId,
            resultRevision: sources.Revision);
    }
