...
public class SimpleMajorRecordRepositoryTests : IDisposable
{
    ...
    public void Upsert_WhenHeadersExist_InsertsSimpleDetailRows()
    {
        ...
        database.ExecuteScalar<string>("SELECT Name FROM Keyword WHERE FormID = @FormId;", new { FormId = "000001" }).ShouldBe(name);
        ...
        database.ExecuteScalar<string>("SELECT Name FROM Faction WHERE FormID = @FormId;", new { FormId = "000002" }).ShouldBe(name);
        database.ExecuteScalar<string>("SELECT KeywordFormKey FROM Faction WHERE FormID = @FormId;", new { FormId = "000002" }).ShouldBe("000100:Example.esm");
        database.ExecuteScalar<string>("SELECT Flags FROM Faction WHERE FormID = @FormId;", new { FormId = "000002" }).ShouldBe("HiddenFromPC, TrackCrime");
