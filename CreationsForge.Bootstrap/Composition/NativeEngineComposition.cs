...
/// <summary>Builds an independently owned native engine service lifetime for a headless host.</summary>
public static class NativeEngineComposition
{
    ...
    /// <exception cref="Autofac.Core.DependencyResolutionException">Thrown when the complete native service graph cannot be resolved.</exception>
    public static NativeEngineServices Create(ILogger? logger = null)
    {
        ...
        try
        {
            ...
            ValidateNativeWireServices(codecs, schemaCatalogs);
            return new NativeEngineServices(
                container,
    ...
    private static void ValidateNativeWireServices(
        ...
        IReadOnlyList<IFormListEditWireSchemaCatalog> schemaCatalogs)
    {
        ...
        {
            throw new InvalidOperationException("Native authoring requires one paired edit codec and schema catalog for each supported game and release.");
        }
    }
}
