...
/// <summary>Writes typed draft nodes to detached command JSON without native construction.</summary>
internal static class NativeWireDraftJsonWriter
{
    /// <summary>Serializes one root node to a detached JSON value.</summary>
    internal static JsonElement WriteDetached(NativeWireDraftNode root, CancellationToken cancellationToken)
    {
    ...
    internal static void WriteNode(Utf8JsonWriter writer, NativeWireDraftNode node, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        switch (node)
        {
            case NativeWireNullableDraftNode nullable:
                if (nullable.IsNull)
                {
                ...
                break;
            case NativeWireUnionDraftNode union when union.SelectedValue is not null:
            ...
            case NativeWireObjectDraftNode objectNode:
                ...
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    if (property.Node.ValueState == NativeWireDraftValueState.RequiredUnset && !property.Node.IsRequired)
                    {
                        continue;
                    }
            ...
            case NativeWireArrayDraftNode array:
                ...
                break;
            case NativeWireBooleanDraftNode boolean:
            ...
            case NativeWireIntegerDraftNode integer:
                ...
                break;
            case NativeWireStringDraftNode text when text.Descriptor.Annotations.ContainsKey("x-presentation-json-number"):
    ...
    private static void WriteFormLink(Utf8JsonWriter writer, NativeWireFormLinkDraftNode node)
    {
        writer.WriteStartObject();
    ...
    private static void WriteFormLinkOrIndex(Utf8JsonWriter writer, NativeWireFormLinkOrIndexDraftNode node, CancellationToken cancellationToken)
    {
        writer.WriteStartObject();
        foreach (var property in node.Descriptor.Properties)
        {
