...
/// <summary>Owns one detached revision-bound native record seed without exposing its JSON to views.</summary>
public sealed class NativeFormListDraftSeed
{
    ...
    /// <param name="recordContext">The exact native record context.</param>
    /// <param name="catalogIdentity">The complete catalog identity used to interpret the record.</param>
    /// <param name="record">The detached expanded native record object.</param>
    /// <exception cref="ArgumentException">Thrown when the workspace is empty or the record is not an object.</exception>
    internal NativeFormListDraftSeed(Guid workspaceId, WorkspaceRevision revision, FormListContext recordContext, NativeWireSchemaCatalogIdentity catalogIdentity, JsonElement record)
    {
        if (workspaceId == Guid.Empty)
        {
            throw new ArgumentException("A native editor seed requires a non-empty workspace identifier.", nameof(workspaceId));
        }
        ...
        {
            throw new ArgumentException("A native editor seed record must be a detached JSON object.", nameof(record));
        }
    ...
    /// <summary>Gets the internally owned detached native record for command-specific extraction.</summary>
