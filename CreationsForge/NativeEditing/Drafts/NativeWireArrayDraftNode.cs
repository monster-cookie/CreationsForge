...
public sealed class NativeWireArrayDraftNode : NativeWireDraftNode
{
    /// <summary>The mutable owned item collection.</summary>
    private readonly ObservableCollection<NativeWireDraftNode> MutableItems;

    /// <summary>The public read-only observable item collection.</summary>
    private readonly ReadOnlyObservableCollection<NativeWireDraftNode> PublicItems;

    /// <summary>Creates a new explicit item for one array position.</summary>
    private readonly Func<int, CancellationToken, EngineResult<NativeWireDraftNode>> ItemFactory;
    ...
    internal NativeWireArrayDraftNode(
        string path,
        string displayName,
        ...
        IEnumerable<NativeWireDraftNode> items,
        Func<int, CancellationToken, EngineResult<NativeWireDraftNode>> itemFactory)
        : base(NativeWireDraftNodeKind.Array, path, displayName, descriptor, isRequired, isReadOnly, valueState)
    {
        ArgumentNullException.ThrowIfNull(items);
        ArgumentNullException.ThrowIfNull(itemFactory);
        MutableItems = new ObservableCollection<NativeWireDraftNode>(items);
        PublicItems = new ReadOnlyObservableCollection<NativeWireDraftNode>(MutableItems);
    ...
    public ReadOnlyObservableCollection<NativeWireDraftNode> Items => PublicItems;

    /// <inheritdoc />
    public override IReadOnlyList<NativeWireDraftNode> Children => PublicItems;
    ...
    public EngineResult<NativeWireDraftNode> Insert(int index, CancellationToken cancellationToken = default)
    {
    ...
    /// <exception cref="OperationCanceledException">Thrown when cancellation is requested.</exception>
    public EngineResult<NativeWireDraftNode> Replace(int index, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (index < 0 || index >= MutableItems.Count)
        {
    ...
    private static EngineResult<NativeWireDraftNode> Failure(string message)
    {
        return EngineResult<NativeWireDraftNode>.Failure(new EngineError(EngineErrorCode.InvalidRequest, message));
    }
}
