...
/// <summary>Captures revision-bound native seeds and creates typed command drafts.</summary>
public interface INativeFormListDraftFactory
{
    ...
    /// <param name="recordContext">The exact selected native record context.</param>
    ...
    EngineResult<NativeFormListDraftSeed> CaptureSeed(NativeFormListWireCatalogContext context, Guid workspaceId, WorkspaceRevision revision, FormListContext recordContext, JsonElement record, NativeWireReadLimits limits, CancellationToken cancellationToken = default);
    ...
    /// <exception cref="OperationCanceledException">Thrown when cancellation is requested.</exception>
    EngineResult<NativeFormListDraft> Create(NativeFormListWireCatalogContext context, NativeWireSchemaNodeKey commandKey, NativeFormListDraftSeed? seed, NativeFormListDraftSeedSelection selection, NativeWireReadLimits limits, CancellationToken cancellationToken = default);
}
