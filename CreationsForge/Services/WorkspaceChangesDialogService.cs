...
/// <summary>Hosts plugin change review and guarded leave decisions in one owner-bound modal window.</summary>
public sealed class WorkspaceChangesDialogService : IWorkspaceChangesDialogService
{
    ...
    /// <summary>Initializes the modal workspace changes service.</summary>
    ...
    public WorkspaceChangesDialogService(
        ...
        ILogger logger)
    {
        ...
        Logger = logger.ForContext<WorkspaceChangesDialogService>();
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        void CloseDialog(WorkspaceChangesDialogResult result, int expectedCloseState)
        {
            Dispatcher.UIThread.VerifyAccess();
        ...
        void ApplyOutcome(WorkspaceLeaveChoiceOutcome outcome, int expectedCloseState)
        {
            Dispatcher.UIThread.VerifyAccess();
            switch (outcome)
            {
                case WorkspaceLeaveChoiceOutcome.ContinueDialog:
                    return;
                case WorkspaceLeaveChoiceOutcome.KeepEditing:
                    CloseDialog(WorkspaceChangesDialogResult.KeepEditing, expectedCloseState);
                    return;
                case WorkspaceLeaveChoiceOutcome.Proceed:
                    CloseDialog(WorkspaceChangesDialogResult.Proceed, expectedCloseState);
                    return;
                default:
                    throw new InvalidOperationException("The plugin change lifecycle returned an unknown dialog outcome.");
            }
        }
        ...
        async Task ApplyChoiceAsync(WorkspaceChangesDialogChoice choice)
        {
            Dispatcher.UIThread.VerifyAccess();
            ...
            {
                Logger.Error(exception, "A workspace changes dialog choice failed.");
            }
            finally
            {
                Dispatcher.UIThread.VerifyAccess();
        ...
        async Task RequestCloseAsync()
        {
            ...
            try
            {
                ...
                if (viewModel.CanDismissDialog)
                {
                    outcome = await viewModel.ApplyLeaveChoiceAsync(WorkspaceChangesDialogChoice.KeepEditing);
                    Dispatcher.UIThread.VerifyAccess();
                }
            }
            catch (Exception exception)
            {
                Logger.Error(exception, "The workspace changes dialog could not close safely.");
            }
            finally
            {
                Dispatcher.UIThread.VerifyAccess();
        ...
        dialog.Content = new WorkspaceChangesView(viewModel, request, ApplyChoiceAsync);
        using var cancellationRegistration = cancellationToken.Register(QueueCloseRequest);
        return await WindowService.ShowDialogAsync<WorkspaceChangesDialogResult>(dialog);
    }
    ...
    private static Window CreateDialog()
    {
        ...
        AutomationProperties.SetAutomationId(dialog, "WorkspaceChangesDialog");
