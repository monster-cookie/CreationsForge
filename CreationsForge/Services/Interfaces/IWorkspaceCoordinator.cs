...
/// <summary>
/// Owns the application-wide workspace and serializes operations that borrow it with replacement and shutdown.
/// </summary>
public interface IWorkspaceCoordinator : INotifyPropertyChanged, IAsyncDisposable
{
    /// <summary>Gets immutable identity for the active workspace, or <see langword="null"/> when none is open.</summary>
    WorkspaceDescriptor? CurrentWorkspace { get; }
    ...
    ValueTask<EngineResult<WorkspaceDescriptor>> OpenAsync(
    ...
    /// <returns>A task that completes after plugin ownership has been released.</returns>
