...
public sealed class NativeWorkspaceTransitionRequest : IAsyncDisposable
{
    ...
    private readonly NativeWorkspaceEditorOperationLease? CapturedEditorOperation;

    /// <summary>Creates the transition lease that receives this request's existing admission ownership.</summary>
    private readonly Func<NativeWorkspaceTransitionLease> TransferOwnership;
    ...
    private NativeWorkspaceTransitionRequestState State = NativeWorkspaceTransitionRequestState.Pending;
    ...
    internal NativeWorkspaceTransitionRequest(
        NativeWorkspaceEditorOperationLease? capturedEditorOperation,
        Func<NativeWorkspaceTransitionLease> transferOwnership,
    ...
    public async ValueTask<NativeWorkspaceTransitionLease> CompleteAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (LifecycleLock)
        {
            if (State != NativeWorkspaceTransitionRequestState.Pending)
            {
                throw new InvalidOperationException("The workspace-transition request has already been completed or abandoned.");
            }

            State = NativeWorkspaceTransitionRequestState.Completing;
        }
        ...
        try
        {
            ...
            lock (LifecycleLock)
            {
                if (State != NativeWorkspaceTransitionRequestState.Completing)
                {
                ...
                State = NativeWorkspaceTransitionRequestState.Transferred;
                return lease;
            }
        }
        catch
        {
            lock (LifecycleLock)
            {
                if (State == NativeWorkspaceTransitionRequestState.Completing)
                {
                    State = NativeWorkspaceTransitionRequestState.Abandoned;
                }
            }
    ...
    public ValueTask DisposeAsync()
    {
        ...
        lock (LifecycleLock)
        {
            switch (State)
            {
                case NativeWorkspaceTransitionRequestState.Pending:
                    State = NativeWorkspaceTransitionRequestState.Abandoned;
                    releaseImmediately = true;
                    break;
                case NativeWorkspaceTransitionRequestState.Completing:
                    State = NativeWorkspaceTransitionRequestState.Abandoned;
                    interruptCompletion = true;
                    completionToAwait = Completion.Task;
                    break;
                case NativeWorkspaceTransitionRequestState.Abandoned:
                    completionToAwait = Completion.Task;
                    break;
                case NativeWorkspaceTransitionRequestState.Transferred:
                    break;
                default:
    ...
    private enum NativeWorkspaceTransitionRequestState
    {
