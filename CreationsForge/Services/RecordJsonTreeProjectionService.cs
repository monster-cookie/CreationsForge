...
/// <summary>
/// Projects detached record JSON into an ordered presentation tree without interpreting game-specific fields.
/// </summary>
public sealed class RecordJsonTreeProjectionService
{
    /// <summary>Projects one optional detached record into a single rooted hierarchy.</summary>
    /// <param name="record">The detached record JSON record, or <see langword="null"/> when the selected context has no inspectable record.</param>
    ...
    public IReadOnlyList<RecordJsonFieldNodeViewModel> Project(
        JsonElement? record,
        CancellationToken cancellationToken = default)
    {
        ...
        return record.HasValue
            ...
            : Array.Empty<RecordJsonFieldNodeViewModel>();
    }
    ...
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken"/> is canceled.</exception>
    private static RecordJsonFieldNodeViewModel ProjectNode(
        string name,
        JsonElement element,
        CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var children = new List<RecordJsonFieldNodeViewModel>();
        ...
        return new RecordJsonFieldNodeViewModel(
            name,
            element.ValueKind,
