...
/// <summary>Defines one bounded reference-selection interaction against an exact workspace revision.</summary>
public sealed class ReferencePickerRequest
{
    /// <summary>The largest accepted presentation-purpose length after trimming.</summary>
    public const int MaximumPurposeLength = 256;

    /// <summary>Initializes an immutable reference-picker request.</summary>
    ...
    /// <param name="recordScope">The record contexts that search and resolution may consider.</param>
        ...
        string purpose)
    {
        ...
        {
            throw new ArgumentException("A reference picker requires a non-empty workspace identity.", nameof(workspaceId));
        }

        if (expectedRevision.BaselineId == Guid.Empty)
        {
            throw new ArgumentException("A reference picker requires a non-empty revision baseline.", nameof(expectedRevision));
        }
        ...
        if (normalizedPurpose.Length > MaximumPurposeLength)
        {
            throw new ArgumentOutOfRangeException(
                nameof(purpose),
                $"A reference-picker purpose cannot exceed {MaximumPurposeLength} characters.");
        }
    ...
    /// <summary>Gets the record contexts considered by search and resolution.</summary>
