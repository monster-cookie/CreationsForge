...
public sealed class FormListEditorView : UserControl
{
    ...
    public FormListEditorView(FormListEditorViewModel viewModel, bool showBeginActions = true)
    {
        ...
        AutomationProperties.SetAutomationId(CommandDescription, "FormListCommandDescription");
        IndexInput = CreateIndexInput("FormListCommandIndex", "Index");
        SourceIndexInput = CreateIndexInput("FormListCommandSourceIndex", "Source index");
        DestinationIndexInput = CreateIndexInput("FormListCommandDestinationIndex", "Destination index");
        SeedSelectionPanel = new StackPanel { Spacing = 6 };
        AutomationProperties.SetAutomationId(SeedSelectionPanel, "FormListCommandSeedSelection");
        CommandError = CreateErrorText("FormListCommandError");
        ...
        DraftScroller.Bind(IsEnabledProperty, new Binding(nameof(FormListEditorViewModel.CanMutateDraft)));
        AutomationProperties.SetAutomationId(DraftScroller, "FormListTypedDraftScroller");
    ...
    {
        var beginNew = CreateCommandButton("New FormList", "FormListBeginNewButton", nameof(FormListEditorViewModel.NewCommand));
        var beginOverride = CreateCommandButton("Override selected", "FormListBeginOverrideButton", nameof(FormListEditorViewModel.OverrideCommand));
        var beginExisting = CreateCommandButton("Edit staged output", "FormListBeginExistingOutputButton", nameof(FormListEditorViewModel.ExistingOutputCommand));
    ...
    {
        var active = CreateIndicator("Active session", nameof(FormListEditorViewModel.IsSessionActive), "FormListEditorActiveIndicator");
        var draftDirty = CreateIndicator("Local draft changes", nameof(FormListEditorViewModel.HasDraftChanges), "FormListEditorDraftDirtyIndicator");
        var stagedDirty = CreateIndicator("Staged changes", nameof(FormListEditorViewModel.HasStagedChanges), "FormListEditorStagedDirtyIndicator");
        var stagedKnown = CreateIndicator("Staged state known", nameof(FormListEditorViewModel.IsStagedChangesKnown), "FormListEditorStagedKnownIndicator");
        var pending = CreateIndicator("Pending outcome", nameof(FormListEditorViewModel.HasPendingOperation), "FormListEditorPendingIndicator");
        var operation = CreateBoundText(nameof(FormListEditorViewModel.OperationState), 11, FontWeight.Normal);
        AutomationProperties.SetAutomationId(operation, "FormListEditorOperationState");
        return new StackPanel
        {
            Orientation = Orientation.Horizontal,
            Spacing = 12,
    ...
    private Control BuildCommandSelector()
    {
        ...
        open.Bind(IsEnabledProperty, new Binding(nameof(FormListEditorViewModel.CanMutateDraft)));
        open.Click += (_, _) => OpenSelectedCommand();
        AutomationProperties.SetAutomationId(open, "FormListOpenCommandButton");
    ...
    private Control BuildDiagnostics()
    {
        ...
        issues.Bind(ItemsControl.ItemsSourceProperty, new Binding(nameof(FormListEditorViewModel.ValidationIssues)));
        AutomationProperties.SetAutomationId(issues, "FormListEditorValidationIssues");
        ...
        warnings.Bind(ItemsControl.ItemsSourceProperty, new Binding(nameof(FormListEditorViewModel.Warnings)));
        AutomationProperties.SetAutomationId(warnings, "FormListEditorWarnings");
        ...
        AutomationProperties.SetAutomationId(detailScroller, "FormListEditorDiagnosticsScroller");
        var error = CreateBoundText(nameof(FormListEditorViewModel.ErrorMessage), 12, FontWeight.Normal);
        error.Foreground = new SolidColorBrush(Color.FromRgb(204, 73, 73));
        error.TextWrapping = TextWrapping.Wrap;
        error.Bind(IsVisibleProperty, new Binding(nameof(FormListEditorViewModel.HasError)));
        AutomationProperties.SetAutomationId(error, "FormListEditorErrorText");
        var status = CreateBoundText(nameof(FormListEditorViewModel.StatusText), 12, FontWeight.Normal);
        status.TextWrapping = TextWrapping.Wrap;
        AutomationProperties.SetAutomationId(status, "FormListEditorStatusText");
    ...
    private ComboBox CreateCommandGroupSelector()
    {
        ...
        AutomationProperties.SetAutomationId(selector, "FormListCommandGroupSelector");
        return selector;
    }
    ...
    private ComboBox CreateCommandSelector()
    {
        ...
        AutomationProperties.SetAutomationId(selector, "FormListCommandSelector");
        return selector;
    }
    ...
    private Button CreateClearComponentsButton()
    {
        ...
        button.Bind(IsEnabledProperty, new Binding(nameof(FormListEditorViewModel.CanMutateDraft)));
        button.Click += (_, _) => OpenAndClearComponents();
        AutomationProperties.SetAutomationId(button, "FormListClearComponentsButton");
        return button;
    }
    ...
    private void RefreshSeedSelection()
    {
        ...
        if (CommandSelector.SelectedItem is not FormListCommandPresentation command)
        {
    ...
    private void OpenAndClearComponents()
    {
        ...
        var result = ViewModel.CreateDraft(command, FormListDraftSeedSelection.CurrentValue());
        ...
        if (result.Value.Root is not RecordWireObjectDraftNode root
            || root.FindProperty("components") is not RecordWireArrayDraftNode components)
        {
    ...
    private bool TryCreateSeedSelection(
        string commandName,
        out FormListDraftSeedSelection? selection,
        out string error)
    {
        switch (GetSeedInputKind(commandName))
        {
            case CommandSeedInputKind.AtIndex:
                ...
                selection = FormListDraftSeedSelection.AtIndex(existingIndex);
                break;
            case CommandSeedInputKind.InsertAt:
                ...
                selection = FormListDraftSeedSelection.InsertAt(insertionIndex);
                break;
            case CommandSeedInputKind.Move:
                ...
                selection = FormListDraftSeedSelection.Move(sourceIndex, destinationIndex);
                break;
            default:
                selection = FormListDraftSeedSelection.CurrentValue();
                break;
        }
    ...
    private static Control CreateEmptyDraftState()
    {
        ...
        AutomationProperties.SetAutomationId(text, "FormListEditorNoDraftText");
        return text;
    }
    ...
    {
        if (string.Equals(eventArgs.PropertyName, nameof(FormListEditorViewModel.AvailableCommands), StringComparison.Ordinal))
        {
            RefreshCommandCatalog();
        }
        else if (string.Equals(eventArgs.PropertyName, nameof(FormListEditorViewModel.SelectedCommand), StringComparison.Ordinal)
