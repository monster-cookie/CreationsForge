...
/// <summary>Coordinates bounded reference search, paging, exact resolution, and modal cancellation.</summary>
public sealed class ReferencePickerViewModel : ViewModelBase, IAsyncDisposable
{
    ...
    /// <summary>Runs operations against the root-owned active workspace.</summary>
    ...
    private readonly ReferencePickerRequest Request;

    /// <summary>Records unexpected picker failures without retaining records.</summary>
    ...
    /// <summary>The latest exact plugin resolution status for the selected row.</summary>
    private ReferenceResolutionStatus? ResolutionStatusValue;

    /// <summary>Whether lifetime disposal has started.</summary>
    private bool IsDisposed;

    /// <summary>Initializes a fresh per-dialog reference picker.</summary>
    ...
    /// <summary>Gets the record scope applied to search and exact resolution.</summary>
    ...
    /// <summary>Gets the most recent plugin resolution status for the selected row.</summary>
    ...
    /// <summary>Gets whether the current plugin row can be resolved for selection.</summary>
    ...
    /// <summary>Resolves and returns the exact selected record context.</summary>
    ...
    public Task<ReferencePickerSelection?> ConfirmSelectionAsync()
    {
        var match = SelectedMatch;
        if (!CanConfirmSelection || match is null)
        {
            return Task.FromResult<ReferencePickerSelection?>(null);
        }
    ...
    public Task<ReferencePickerSelection?> SelectNullAsync()
    {
        if (!CanSelectNull)
        {
            return Task.FromResult<ReferencePickerSelection?>(null);
    ...
    /// <summary>Cancels all current work and waits until plugin borrows and UI publications have drained.</summary>
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            await UiDispatcher.InvokeAsync(() =>
            {
                ...
                {
                    ErrorText = $"The engine returned more than the bounded {PageSize}-record page.";
                    StatusText = "The reference search returned an invalid page.";
                    return;
                }
                Matches.Clear();
                ...
                StatusText = Matches.Count == 0
                    ...
                    : $"Showing {Matches.Count} record{(Matches.Count == 1 ? string.Empty : "s")} on page {PageNumber}.";
                ErrorText = null;
        ...
        {
            Logger.Error(exception, "Unexpected reference search failure for workspace {WorkspaceId}.", Request.WorkspaceId);
            await PublishUnexpectedAsync(searchGeneration, workspaceGeneration, "Unable to search records.").ConfigureAwait(false);
        }
        finally
        {
    ...
    /// <param name="match">The exact visible plugin match.</param>
    /// <returns>A task producing a current resolved selection or <see langword="null"/>.</returns>
    private Task<ReferencePickerSelection?> StartSelectionAsync(ReferenceSearchMatch match)
    {
        CancellationTokenSource cancellationSource;
        ...
        ErrorText = null;
        ResolutionStatus = null;
        StatusText = "Resolving the selected record...";
    ...
    /// <summary>Resolves one selected row and preserves explicit plugin resolution status.</summary>
    ...
    private async Task<ReferencePickerSelection?> ResolveSelectionCoreAsync(
        ReferenceSearchMatch match,
        long selectionGeneration,
        long workspaceGeneration,
        CancellationToken cancellationToken)
    {
        ReferencePickerSelection? selection = null;
        try
        {
            ...
            await UiDispatcher.InvokeAsync(() =>
            {
                ...
                {
                    ErrorText = $"The selected record resolved as {resolution.Status}. Refresh the search or choose another context.";
                ...
                {
                    ErrorText = "The selected record's identity or containing-plugin provenance changed. Refresh the search.";
                    StatusText = "The selected reference became stale.";
                    RequiresRefresh = true;
                    return;
                }

                selection = new ReferencePickerSelection(
                    Request.WorkspaceId,
                    result.ResultRevision!.Value,
                    isNull: false,
                    match);
                ErrorText = null;
                StatusText = "Reference resolved.";
            }).ConfigureAwait(false);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        { }
        ...
        {
            Logger.Error(exception, "Unexpected reference resolution failure for workspace {WorkspaceId}.", Request.WorkspaceId);
            await PublishUnexpectedSelectionAsync(
                selectionGeneration,
                workspaceGeneration,
                "Unable to resolve the selected record.").ConfigureAwait(false);
        }
        finally
        {
    ...
    private Task<ReferencePickerSelection?> StartNullSelectionAsync()
    {
        SelectedMatch = null;
        CancellationTokenSource cancellationSource;
        long selectionGeneration;
    ...
    /// <summary>Validates exact workspace identity and revision without retaining a record.</summary>
    /// <param name="selectionGeneration">The operation's selection generation.</param>
    /// <param name="workspaceGeneration">The operation's workspace generation.</param>
    /// <param name="cancellationToken">The superseding selection token.</param>
    /// <returns>The explicit null selection when current, otherwise <see langword="null"/>.</returns>
    private async Task<ReferencePickerSelection?> ValidateNullSelectionCoreAsync(
        long selectionGeneration,
        long workspaceGeneration,
        CancellationToken cancellationToken)
    {
    ...
    private bool TryValidateCurrentResult<T>(EngineResult<T> result, out string error)
    {
        ...
        {
            error = result.Error?.Message ?? "The workspace operation failed without an error description.";
            return false;
        }

        if (result.WorkspaceId != Request.WorkspaceId || result.ResultRevision != Request.ExpectedRevision)
        {
            error = "The workspace or revision changed. Refresh the editor before selecting a reference.";
            return false;
        }
    ...
    /// <param name="resolution">The fresh plugin resolution.</param>
    ...
    private EngineResult<T> RevisionConflict<T>(IPluginWorkspace workspace)
    {
        return EngineResult<T>.Failure(
            new EngineError(
                ...
                "The workspace or revision changed. Refresh the editor before selecting a reference."),
            workspaceId: workspace.WorkspaceId,
    ...
    private void PublishRefreshOrError(EngineErrorCode? code, string message)
    {
        ...
        StatusText = code == EngineErrorCode.RevisionConflict
            ...
            : "The reference operation did not complete.";
    ...
    private Task PublishUnexpectedAsync(long searchGeneration, long workspaceGeneration, string message)
    {
        return UiDispatcher.InvokeAsync(() =>
        {
            ...
            {
                ErrorText = message;
                StatusText = "The reference search failed.";
            }
        });
    }
    ...
    {
        if (!string.Equals(eventArgs.PropertyName, nameof(IWorkspaceCoordinator.CurrentWorkspace), StringComparison.Ordinal))
        {
            return;
        }
        ...
        UiDispatcher.Post(() =>
        {
            ...
            RequiresRefresh = true;
            ErrorText = "The active workspace changed. Refresh the editor before selecting a reference.";
    ...
    private void ThrowIfDisposed()
    {
        if (IsDisposed)
        {
            throw new ObjectDisposedException(nameof(ReferencePickerViewModel));
        }
    }
