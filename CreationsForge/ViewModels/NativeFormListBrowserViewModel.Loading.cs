...
public sealed partial class NativeFormListBrowserViewModel
{
    ...
    private Task BeginWorkspaceGeneration(
        NativeWorkspaceDescriptor? workspace,
        FormKey? select = null,
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            IReadOnlyList<NativeFormListRecordViewModel>? records = null;
        ...
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            await UiDispatcher.InvokeAsync(() =>
            {
                if (IsCurrentWorkspaceGeneration(workspace.WorkspaceId, generation))
                {
                    SetStatus("Native FormList refresh canceled.");
                }
            }).ConfigureAwait(false);
        }
    ...
    private static async ValueTask<EngineResult<NativeFormListBrowserSnapshot>> ReadBrowserSnapshotAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return CopyFailure<WorkspaceState, NativeFormListBrowserSnapshot>(stateResult);
        }

        if (stateResult.WorkspaceId != workspace.WorkspaceId)
        {
            return EngineResult<NativeFormListBrowserSnapshot>.Failure(
                CreateRevisionError("The native browser state read returned a different workspace identity."),
                workspace.WorkspaceId,
        ...
        {
            return CopyFailure<IReadOnlyList<PluginSummary>, NativeFormListBrowserSnapshot>(pluginsResult);
        }

        if (pluginsResult.WorkspaceId != workspace.WorkspaceId)
        {
            return EngineResult<NativeFormListBrowserSnapshot>.Failure(
                CreateRevisionError("The native browser plugin read returned a different workspace identity."),
                workspace.WorkspaceId,
        ...
        {
            return CopyFailure<IReadOnlyList<FormListSummary>, NativeFormListBrowserSnapshot>(formListsResult);
        }

        if (formListsResult.WorkspaceId != workspace.WorkspaceId)
        {
            return EngineResult<NativeFormListBrowserSnapshot>.Failure(
                CreateRevisionError("The native browser FormList read returned a different workspace identity."),
                workspace.WorkspaceId,
        ...
        {
            return EngineResult<NativeFormListBrowserSnapshot>.Failure(
                CreateRevisionError("The native browser snapshot changed while it was being read."),
                workspace.WorkspaceId,
                resultRevision: revision,
                warnings: CombineWarnings(stateResult.Warnings, pluginsResult.Warnings, formListsResult.Warnings));
        }

        return EngineResult<NativeFormListBrowserSnapshot>.Success(
            new NativeFormListBrowserSnapshot(stateResult.Value, pluginsResult.Value, formListsResult.Value),
            workspace.WorkspaceId,
    ...
    /// <summary>Publishes a successful detached snapshot and its exact native ordering.</summary>
    ...
    private NativeFormListRecordViewModel? PublishSnapshot(
        NativeFormListBrowserSnapshot snapshot,
        ...
        bool preferStagedOutput)
    {
        ...
        SetStatus(RecordsValue.Count == 0
            ? "The native workspace contains no FormLists."
            : $"Loaded {RecordsValue.Count} native FormList record(s) from {PluginsValue.Count} plugin(s).");
        PickReferenceRelayCommand.RaiseCanExecuteChanged();
        RefreshRelayCommand.RaiseCanExecuteChanged();
        if (!select.HasValue)
        {
