...
/// <summary>Presents one collapsible native record-type group above its alphabetically ordered record rows.</summary>
public sealed class NativeRecordTypeGroupViewModel : ViewModelBase, INativeRecordTreeNodeViewModel
{
    /// <summary>Tracks whether the record-type group is expanded.</summary>
    private bool IsExpandedValue;

    /// <summary>Initializes one native record-type group.</summary>
    /// <param name="label">The friendly record type and native signature displayed in the primary column.</param>
    ...
    public NativeRecordTypeGroupViewModel(
        string label,
        IReadOnlyList<INativeRecordTreeNodeViewModel> children)
    {
    ...
    /// <summary>Gets the friendly record type and native signature.</summary>
    ...
    public IReadOnlyList<INativeRecordTreeNodeViewModel> Children { get; }
