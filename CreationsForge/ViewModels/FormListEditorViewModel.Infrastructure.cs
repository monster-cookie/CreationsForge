...
public sealed partial class FormListEditorViewModel
{
    ...
    private Task ExitOperationAsync(long generation)
    {
        return UiDispatcher.InvokeAsync(() =>
        {
            ...
            SetOperationState(PendingOperationValue is null
                ? FormListEditorOperationState.Idle
                : FormListEditorOperationState.PendingOutcome);
        });
    }
    ...
    {
        if (eventArgs.PropertyName != nameof(IWorkspaceCoordinator.CurrentWorkspace) || IsDisposed)
        {
            return;
        }
    ...
    {
        if (eventArgs.PropertyName == nameof(IFormListEditorHost.Selection))
        {
    ...
    {
        if (eventArgs.PropertyName != nameof(IWorkspacePresentationOperationArbiter.IsWorkspaceTransitionPendingOrReserved))
        {
            return;
        }
        ...
        WorkspaceDescriptor descriptor)
    {
        ...
        {
            return new EngineError(EngineErrorCode.InvalidRequest, "The active workspace changed before the editor operation began.");
        }
        ...
        {
            return new EngineError(EngineErrorCode.ExternalChangeDetected, "The selected plugin output changed before the editor operation began.");
        }

        if (state.OutputSynchronization.Status != OutputSynchronizationStatus.Ready)
        {
            return new EngineError(EngineErrorCode.InvalidRequest, $"Record editor operations require ready output synchronization; current state is {state.OutputSynchronization.Status}.");
        }
        ...
        FormListEditorSession session)
    {
        ...
        {
            return new EngineError(EngineErrorCode.ExternalChangeDetected, "The active workspace no longer matches the captured editor session.");
        }
    ...
    private static bool SameCatalogIdentity(
        RecordWireSchemaCatalogIdentity left,
        RecordWireSchemaCatalogIdentity right)
    {
    ...
    private static bool SameSchemaKey(RecordWireSchemaNodeKey left, RecordWireSchemaNodeKey right)
    {
    ...
    private void SetOperationState(FormListEditorOperationState state)
    {
    ...
    {
        var effective = error ?? new EngineError(EngineErrorCode.UnexpectedFailure, "The record editor operation failed without a diagnostic reason.");
    ...
    private static EngineResult<TTarget> CopyFailure<TSource, TTarget>(EngineResult<TSource> source)
    {
        return EngineResult<TTarget>.Failure(
            source.Error ?? new EngineError(EngineErrorCode.UnexpectedFailure, "A record editor dependency failed without a diagnostic reason."),
            source.WorkspaceId,
            source.OperationId,
            source.BaseRevision,
...
internal sealed class FormListEditorBeginOutcome
{
    ...
    public FormListEditorBeginOutcome(
        EditReceipt receipt,
    ...
    public FormListWireCatalogContext CatalogContext { get; }
    /// <summary>Gets every admitted command presentation.</summary>
    public IReadOnlyList<FormListCommandPresentation> Commands { get; }
...
internal sealed class FormListEditorApplyOutcome
{
    ...
    public FormListEditorApplyOutcome(
