...
public sealed partial class NativeWorkspaceChangesViewModel
{
    ...
    public Task SaveChangesAsync(CancellationToken cancellationToken = default)
    {
        ...
        return RunOperationAsync(
            NativeWorkspaceChangesOperationState.Saving,
            cancellationToken,
    ...
    public Task DiscardChangesAsync(CancellationToken cancellationToken = default)
    {
        ...
        return RunOperationAsync(
            NativeWorkspaceChangesOperationState.Discarding,
            cancellationToken,
    ...
    public Task RetryCommittedRefreshAsync(CancellationToken cancellationToken = default)
    {
        ...
        return RunOperationAsync(
            NativeWorkspaceChangesOperationState.Refreshing,
            cancellationToken,
    ...
    {
        EngineResult<NativeWorkspaceSaveBorrowOutcome> borrowResult;
        try
        {
            borrowResult = await WorkspaceCoordinator.ExecuteAsync(
                async (workspace, token) =>
                {
                    ...
                    {
                        return EngineResult<NativeWorkspaceSaveBorrowOutcome>.Failure(
                    ...
                    var envelope = new NativeWorkspaceSaveEnvelope(
                        workspace.WorkspaceId,
                        state.Game,
                        state.Release,
                        state.Output!,
                    ...
                    return EngineResult<NativeWorkspaceSaveBorrowOutcome>.Success(
                        new NativeWorkspaceSaveBorrowOutcome(envelope, saveResult, postSaveState),
                        workspace.WorkspaceId,
                        request.OperationId,
    ...
    {
        EngineResult<NativeWorkspaceDiscardBorrowOutcome> borrowResult;
        try
        {
            borrowResult = await WorkspaceCoordinator.ExecuteAsync(
                async (workspace, token) =>
                {
                    ...
                    if (!captureResult.Succeeded || captureResult.Value is null)
                    {
                        return EngineResult<NativeWorkspaceDiscardBorrowOutcome>.Failure(
                    ...
                    {
                        return EngineResult<NativeWorkspaceDiscardBorrowOutcome>.Success(
                            new NativeWorkspaceDiscardBorrowOutcome(capture, envelope: null, result: null),
                            workspace.WorkspaceId,
                    ...
                    var envelope = new NativeWorkspaceDiscardEnvelope(
                        workspace.WorkspaceId,
                    ...
                    return EngineResult<NativeWorkspaceDiscardBorrowOutcome>.Success(
                        new NativeWorkspaceDiscardBorrowOutcome(capture, envelope, result),
                        workspace.WorkspaceId,
                        request.OperationId,
                        request.ExpectedRevision,
        ...
        var refreshEnvelope = new NativeWorkspaceRefreshEnvelope(
            NativeWorkspaceRefreshKind.Discard,
            outcome.Envelope.WorkspaceId,
            outcome.Result.Value.Revision,
            outcome.Envelope.Request.ExpectedBaseline,
            NativeWorkspaceRefreshStep.RefreshParticipant,
            acceptedState: null,
            saveResult: null);
    ...
    private async Task MapCommittedSaveAsync(
        NativeWorkspaceSaveBorrowOutcome outcome,
        IReadOnlyList<EngineWarning> warnings)
    {
        ...
        {
            PendingRefreshEnvelopeValue = new NativeWorkspaceRefreshEnvelope(
                NativeWorkspaceRefreshKind.Save,
                outcome.Envelope.WorkspaceId,
                result.ResultRevision,
                result.CommittedBaseline!,
                NativeWorkspaceRefreshStep.ReadState,
                acceptedState: null,
                result);
            await PublishFailureAsync(
    ...
    private async Task RetryCommittedRefreshCoreAsync(CancellationToken cancellationToken)
    {
        ...
        if (envelope.Step == NativeWorkspaceRefreshStep.ReadState)
        {
    ...
    private async Task RefreshKnownTransitionAsync(
        NativeWorkspaceRefreshEnvelope envelope,
        IReadOnlyList<EngineWarning> warnings,
    ...
    private static bool SaveResultMatchesEnvelope(NativeWorkspaceSaveEnvelope envelope, SaveResult result)
    {
    ...
    private static bool DiscardResultMatchesEnvelope(
        NativeWorkspaceDiscardEnvelope envelope,
    ...
    private async Task PublishAcceptedReviewCaptureAsync(NativeWorkspaceReviewCapture capture, string status)
    {
        var review = new NativeWorkspaceChangeReview(
            capture.WorkspaceId,
            capture.State.Game,
            ...
            Array.Empty<NativeWorkspaceChangeItemViewModel>(),
    ...
    private static string GetRefreshSuccessStatus(NativeWorkspaceRefreshKind kind)
    {
        return kind switch
        {
            NativeWorkspaceRefreshKind.Save => "Saved and reopened.",
            NativeWorkspaceRefreshKind.Discard => "Discarded staged changes and reopened the selected output.",
            NativeWorkspaceRefreshKind.RecoveryResume => "Unsaved changes resumed.",
            NativeWorkspaceRefreshKind.RecoveryReopen => "Recovered output reopened.",
    ...
    private static string GetRefreshFailureStatus(NativeWorkspaceRefreshKind kind)
    {
        return kind switch
        {
            NativeWorkspaceRefreshKind.Save => "Saved, but the refreshed workspace state could not be displayed.",
            NativeWorkspaceRefreshKind.Discard => "Discarded, but the refreshed workspace state could not be displayed.",
            NativeWorkspaceRefreshKind.RecoveryResume or NativeWorkspaceRefreshKind.RecoveryReopen =>
