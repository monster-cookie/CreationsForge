...
/// <summary>
/// Discovers installed plugins and activates a guarded record editing workspace asynchronously.
/// </summary>
public sealed partial class WorkspaceSelectionViewModel : ViewModelBase
{
    ...
    /// <summary>Acquires local paths without opening records.</summary>
    private readonly IWorkspacePathPicker PathPicker;
    ...
    /// <summary>The currently selected game and plugin release.</summary>
    private WorkspaceGameOption SelectedGameValue;
    ...
    /// <summary>Initializes workspace selection from the persisted active-game preference.</summary>
    ...
    public WorkspaceSelectionViewModel(
        IWorkspaceCoordinator workspaceCoordinator,
        IWorkspacePathPicker pathPicker,
        IGameSelectionService gameSelectionService,
        IUiDispatcher uiDispatcher,
        ILogger logger,
        IPluginDiscoveryService? pluginDiscoveryService = null,
        IApplicationSettingsService? applicationSettingsService = null)
    {
        ...
        Logger = logger.ForContext<WorkspaceSelectionViewModel>();
        PluginDiscoveryService = pluginDiscoveryService;
        Games = WorkspaceGameOption.CreateSupportedGames();
    ...
    /// <summary>Gets the three supported game and plugin-release choices.</summary>
    public IReadOnlyList<WorkspaceGameOption> Games { get; }
    ...
    /// <summary>Gets or sets the selected game and exact plugin release.</summary>
    public WorkspaceGameOption SelectedGame
    {
    ...
    /// <summary>Gets or sets the separate output plugin path.</summary>
    ...
    /// <summary>Gets whether plugin source or output acquisition is currently running.</summary>
    public bool IsBusy
    {
        get => IsBusyValue;
        private set
    ...
    /// <summary>Validates and activates the selected plugin source and output without disturbing a prior workspace on failure.</summary>
    ...
    /// <summary>Validates and activates plugin sources with an optional mutable output without disturbing a prior workspace on failure.</summary>
    ...
    private async Task<bool> OpenWorkspaceAsync(bool includeOutput, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var sources = new Core.Engine.Contracts.WorkspaceOpenRequest(
                Guid.NewGuid(),
                SelectedGame.Game,
            ...
            var request = includeOutput
                ? new WorkspaceLaunchRequest(
                    ...
                    new OutputAssociation(
                        ...
                        LocalizedOutputMode,
                        OutputMasterStyle))
                : new WorkspaceLaunchRequest(sources);
            Logger.Information(
                "Opening plugin {AccessMode} workspace for {Game}; source: {SourcePluginPath}; output: {OutputPluginPath}; load-order plugin count: {LoadOrderPluginCount}",
            ...
            {
                Logger.Warning(
                    "Plugin {AccessMode} workspace opening failed for {Game}; source: {SourcePluginPath}; output: {OutputPluginPath}; error code: {ErrorCode}; message: {ErrorMessage}",
                    includeOutput ? "editing" : "read-only",
                    SelectedGame.Game,
                    SourcePluginPath,
                    includeOutput ? OutputPluginPath : null,
                ...
                ErrorText = result.Error?.Message ?? "The engine returned no workspace or failure reason.";
                StatusText = WorkspaceCoordinator.CurrentWorkspace is null
                    ? "Workspace opening failed."
                    : "Workspace opening failed; the previous workspace remains active.";
                return false;
            }

            activationSucceeded = true;
            Logger.Information(
                "Plugin {AccessMode} workspace {WorkspaceId} opened for {Game}; source: {SourcePluginPath}; output: {OutputPluginPath}",
                includeOutput ? "editing" : "read-only",
            ...
            {
                Logger.Warning(exception, "Workspace {WorkspaceId} opened, but its active-game preference could not be saved.", result.Value.WorkspaceId);
        ...
        {
            Logger.Error(exception, "An unexpected presentation failure prevented workspace activation.");
            if (activationSucceeded)
            {
            ...
            ErrorText = "An unexpected failure prevented the workspace from opening.";
            StatusText = WorkspaceCoordinator.CurrentWorkspace is null
                ? "Workspace opening failed."
                : "Workspace opening failed; the previous workspace remains active.";
            return false;
    ...
    /// <summary>Requests cancellation of the current workspace acquisition.</summary>
    ...
    /// <summary>Closes the active workspace and releases its engine lifetime.</summary>
    ...
    private async Task<T?> RunPickerAsync<T>(Func<Task<T>> picker)
    {
        ...
        {
            Logger.Error(exception, "Unable to acquire a workspace path from the platform picker.");
            ErrorText = "The selected path could not be read from the platform file picker.";
            return default;
        }
    }

    /// <summary>Validates the complete selection before any workspace is acquired.</summary>
    ...
    /// <param name="workspace">The published workspace descriptor.</param>
    /// <returns>A complete ready-state sentence.</returns>
    private static string CreateReadyStatus(WorkspaceDescriptor workspace)
    {
