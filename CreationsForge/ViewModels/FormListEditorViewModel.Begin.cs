...
public sealed partial class FormListEditorViewModel
{
    ...
    private async Task BeginAsync(FormListEditRole role, CancellationToken cancellationToken)
    {
        ...
        FormListEditorPendingBegin? attemptedOperation = null;
        var coreResponded = false;
        try
        {
            if (descriptor is null || descriptor.Output is null)
            {
                await PublishBeginFailureAsync(generation, descriptor?.WorkspaceId ?? Guid.Empty, new EngineError(EngineErrorCode.InvalidRequest, "No editable workspace is open.")).ConfigureAwait(false);
                return;
            }
            ...
            var result = await WorkspaceCoordinator.ExecuteAsync(
                async (workspace, token) =>
                {
                    ...
                    attemptedOperation = new FormListEditorPendingBegin(descriptor.WorkspaceId, request, DescribeBeginRole(role));
    ...
    /// <summary>Validates role-specific browser selection before borrowing the workspace.</summary>
    ...
    private static EngineError? ValidateBeginSelection(
        ...
        FormListEditorSelection? selection)
    {
    ...
    private static FormListEditorTarget CreateBeginTarget(
        FormListEditRole role,
        WorkspaceDescriptor descriptor,
        ...
        FormListEditorSelection? selection)
    {
        return role switch
        {
            FormListEditRole.New => new FormListEditorTarget(descriptor.WorkspaceId, state.Revision, role, null, null, null),
            FormListEditRole.Override => new FormListEditorTarget(descriptor.WorkspaceId, selection!.Revision, role, selection.FormKey, selection.ExactReferenceRequest, null),
            FormListEditRole.ExistingOutput => new FormListEditorTarget(descriptor.WorkspaceId, selection!.Revision, role, null, null, selection.FormKey),
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new FormListEditorBeginOutcome(
            receipt,
            catalogContext,
            commands,
            seed,
    ...
    private async Task PublishBeginSuccessAsync(
        long generation,
        WorkspaceDescriptor descriptor,
        FormListEditorBeginOutcome outcome,
        CancellationToken cancellationToken)
    {
        await UiDispatcher.InvokeAsync(() =>
        {
            ...
            SessionValue = new FormListEditorSession(
                descriptor.WorkspaceId,
                descriptor.Game,
                descriptor.Release,
                descriptor.Output!,
