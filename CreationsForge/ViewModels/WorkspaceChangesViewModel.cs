...
public sealed partial class WorkspaceChangesViewModel : ViewModelBase, IDisposable, IAsyncDisposable
{
    ...
    /// <summary>Owns and lends the application-wide workspace.</summary>
    ...
    private readonly IWorkspacePresentationOperationArbiter PresentationArbiter;

    /// <summary>Exposes request-local editor state and guarded browser refresh.</summary>
    private readonly IWorkspaceEditParticipant EditParticipant;
    ...
    private readonly IWorkspaceChangesDialogService DialogService;
    ...
    private WorkspaceChangeReview? CurrentReviewValue;
    ...
    private WorkspaceChangesOperationState OperationStateValue = WorkspaceChangesOperationState.Idle;
    ...
    /// <summary>Initializes the navigation-scope workspace change lifecycle.</summary>
    /// <param name="workspaceCoordinator">The root-lifetime workspace owner.</param>
    ...
    /// <param name="jsonTreeProjectionService">The detached record JSON projector.</param>
    ...
    public WorkspaceChangesViewModel(
        IWorkspaceCoordinator workspaceCoordinator,
        IWorkspaceSaveCoordinator saveCoordinator,
        IWorkspacePresentationOperationArbiter presentationArbiter,
        IWorkspaceEditParticipant editParticipant,
        ...
        ILogger logger)
    {
        ...
        Logger = logger.ForContext<WorkspaceChangesViewModel>();
    ...
    public WorkspaceChangeReview? CurrentReview => CurrentReviewValue;
    ...
    public WorkspaceChangesOperationState OperationState => OperationStateValue;
    ...
    public bool ShowSaveAndProceed => (ActiveDialogRequest?.Purpose is WorkspaceChangesDialogPurpose.Save or WorkspaceChangesDialogPurpose.Leave)
    ...
    public bool ShowDiscardAndProceed => (ActiveDialogRequest?.Purpose is WorkspaceChangesDialogPurpose.Discard or WorkspaceChangesDialogPurpose.Leave)
        && !ShowActiveEditorOperationDecision
        && !HasPendingEditorOperation;
    ...
    public bool ShowReturnToEditor => ActiveDialogRequest?.Purpose == WorkspaceChangesDialogPurpose.Leave
        && !ShowActiveEditorOperationDecision
        && HasPendingEditorOperation;
    ...
    public bool ShowActiveEditorOperationDecision => ActiveDialogRequest?.Purpose == WorkspaceChangesDialogPurpose.Leave
    ...
    public bool ShowConfirmedAbandonmentForOpen => ActiveDialogRequest?.Purpose == WorkspaceChangesDialogPurpose.Leave
        && !ShowActiveEditorOperationDecision
        && !HasPendingEditorOperation
        && ActiveDialogRequest.LeaveReason == WorkspaceLeaveReason.OpenWorkspace
    ...
    internal WorkspaceRecoveryEnvelope? RecoveryEnvelope => RecoveryEnvelopeValue;

    /// <summary>Gets the known successful Core transition awaiting only presentation refresh.</summary>
    internal WorkspaceRefreshEnvelope? PendingRefreshEnvelope => PendingRefreshEnvelopeValue;
    ...
    private async Task ShutdownCoreAsync()
    {
        ...
        lease?.Dispose();
        DetachSubscriptions();
        await PublishOperationStateAsync(WorkspaceChangesOperationState.Idle, false, StatusTextValue).ConfigureAwait(false);
    }
    ...
    private Task RunOperationAsync(
        WorkspaceChangesOperationState state,
        ...
        finally
        {
            try
            {
                await UiDispatcher.InvokeAsync(() =>
                {
                    ...
                    OperationStateValue = WorkspaceChangesOperationState.Idle;
                    RaiseAllBoundState();
        ...
        string statusText)
    {
        return UiDispatcher.InvokeAsync(() =>
        {
            ...
            IsBusyValue = state != WorkspaceChangesOperationState.Idle;
    ...
    private static string GetActiveStatus(WorkspaceChangesOperationState state)
    {
        return state switch
        {
            WorkspaceChangesOperationState.ReservingTransition => "Preparing change review...",
            WorkspaceChangesOperationState.WaitingForEditorOperation => "Waiting for the current editor operation to finish.",
            WorkspaceChangesOperationState.Reviewing => "Reviewing changes...",
            WorkspaceChangesOperationState.Saving => "Saving output...",
            WorkspaceChangesOperationState.Discarding => "Discarding staged changes...",
            WorkspaceChangesOperationState.InspectingRecovery => "Inspecting save outcome...",
            WorkspaceChangesOperationState.Repairing => "Repairing saved output...",
            WorkspaceChangesOperationState.AdoptingRecovery => "Reopening resolved output...",
            WorkspaceChangesOperationState.Refreshing => "Refreshing workspace...",
            WorkspaceChangesOperationState.CancelRequestedWaitingForResult => "Cancel requested; waiting for a safe result.",
            _ => string.Empty,
        };
    }
