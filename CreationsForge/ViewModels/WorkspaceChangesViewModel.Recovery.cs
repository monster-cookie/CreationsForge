...
public sealed partial class WorkspaceChangesViewModel
{
    ...
    public Task InspectSaveOutcomeAsync(CancellationToken cancellationToken = default)
    {
        ...
        return RunOperationAsync(
            WorkspaceChangesOperationState.InspectingRecovery,
            cancellationToken,
    ...
    public Task ResumeUnsavedChangesAsync(CancellationToken cancellationToken = default)
    {
        ...
        return RunOperationAsync(
            WorkspaceChangesOperationState.AdoptingRecovery,
            cancellationToken,
    ...
    private Task RepairSaveAsync(RepairSaveDirection direction, CancellationToken cancellationToken)
    {
        ...
        return RunOperationAsync(
            WorkspaceChangesOperationState.Repairing,
            cancellationToken,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (repairEnvelope is null)
        {
            ...
            var request = new RepairSaveRequest(
                ...
                direction);
            repairEnvelope = new WorkspaceRepairEnvelope(request, recovery);
        ...
        CancellationToken cancellationToken)
    {
        ...
        EngineResult<WorkspaceAdoptionBorrowOutcome> borrowResult;
        try
        {
            borrowResult = await WorkspaceCoordinator.ExecuteAsync(
                async (workspace, token) =>
                {
                    ...
                    {
                        return EngineResult<WorkspaceAdoptionBorrowOutcome>.Failure(
                    ...
                    var envelope = new WorkspaceAdoptionEnvelope(workspace.WorkspaceId, request);
                    ...
                    var postState = adoptionResult.Succeeded
                        ...
                        : stateResult;
                    return EngineResult<WorkspaceAdoptionBorrowOutcome>.Success(
                        new WorkspaceAdoptionBorrowOutcome(envelope, adoptionResult, postState),
                        workspace.WorkspaceId,
                        request.OperationId,
        ...
        var refreshEnvelope = new WorkspaceRefreshEnvelope(
            mode == OutputRecoveryAdoptionMode.ResumeStagedAfterNotCommitted
                ? WorkspaceRefreshKind.RecoveryResume
                : WorkspaceRefreshKind.RecoveryReopen,
            outcome.Envelope.WorkspaceId,
            receipt.Revision,
            receipt.Baseline,
            stateError is null ? WorkspaceRefreshStep.RefreshParticipant : WorkspaceRefreshStep.ReadState,
        ...
        IReadOnlyList<EngineWarning> warnings)
    {
        switch (repairResult.Status)
        {
            ...
            case RepairSaveStatus.BlockedByExternalChange:
                ...
                {
                    AbandonmentEnvelopeValue = new WorkspaceAbandonmentEnvelope(liveWorkspaceId, state.Output);
                }
