...
public sealed partial class NativeWorkspaceChangesViewModel
{
    ...
    public async ValueTask<NativeWorkspaceLeaveReservation?> ReserveLeaveAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var request = NativeWorkspaceChangesDialogRequest.ForLeave(reason);
        if (!TryBeginDialogSession(request, requireWorkspace: false))
        {
            return null;
        }

        NativeWorkspaceLeaveReservation? transferredReservation = null;
        try
        {
            await RunOperationAsync(
                NativeWorkspaceChangesOperationState.ReservingTransition,
                cancellationToken,
            ...
            if (IsCapturedEditorOperationActive)
            {
                ...
                if (!IsCapturedEditorOperationActive)
                {
                    var completedBeforeDialogOutcome = await CompleteEditorTransitionAndClassifyAsync(
                        ...
                        NativeWorkspaceChangesOperationState.WaitingForEditorOperation).ConfigureAwait(false);
                    if (completedBeforeDialogOutcome == NativeWorkspaceLeaveChoiceOutcome.Proceed)
                    {
            ...
            else
            {
                var initialOutcome = await CompleteEditorTransitionAndClassifyAsync(
                    NativeWorkspaceTransitionDrainMode.WaitForCurrentOperation,
                    NativeWorkspaceChangesOperationState.WaitingForEditorOperation).ConfigureAwait(false);
                if (initialOutcome == NativeWorkspaceLeaveChoiceOutcome.Proceed)
                {
                    transferredReservation = await FinalizeProvedLeaveAsync(reason, cancellationToken).ConfigureAwait(false);
                    return transferredReservation;
                }
            ...
            if (dialogResult != NativeWorkspaceChangesDialogResult.Proceed)
            {
                return null;
            }
    ...
    internal async Task<NativeWorkspaceLeaveChoiceOutcome> ApplyLeaveChoiceAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        switch (choice)
        {
            case NativeWorkspaceChangesDialogChoice.WaitForEditorOperation:
                ...
                return await CompleteEditorTransitionAndClassifyAsync(
                    NativeWorkspaceTransitionDrainMode.WaitForCurrentOperation,
                    NativeWorkspaceChangesOperationState.WaitingForEditorOperation).ConfigureAwait(false);
            case NativeWorkspaceChangesDialogChoice.CancelEditorOperationAndWait:
                ...
                return await CompleteEditorTransitionAndClassifyAsync(
                    ...
                    NativeWorkspaceChangesOperationState.CancelRequestedWaitingForResult).ConfigureAwait(false);
            case NativeWorkspaceChangesDialogChoice.KeepEditing:
                return CanDismissDialog
                    ? NativeWorkspaceLeaveChoiceOutcome.KeepEditing
                    : NativeWorkspaceLeaveChoiceOutcome.ContinueDialog;
            case NativeWorkspaceChangesDialogChoice.ReturnToEditor:
                return CanReturnToEditor
                    ? NativeWorkspaceLeaveChoiceOutcome.KeepEditing
                    : NativeWorkspaceLeaveChoiceOutcome.ContinueDialog;
            case NativeWorkspaceChangesDialogChoice.SaveAndProceed:
                if (request.Purpose is not NativeWorkspaceChangesDialogPurpose.Review
                    and not NativeWorkspaceChangesDialogPurpose.Save
                    and not NativeWorkspaceChangesDialogPurpose.Leave)
                {
                    return NativeWorkspaceLeaveChoiceOutcome.ContinueDialog;
                }
            ...
            case NativeWorkspaceChangesDialogChoice.DiscardAndProceed:
                if (request.Purpose is not NativeWorkspaceChangesDialogPurpose.Review
                    and not NativeWorkspaceChangesDialogPurpose.Discard
                    and not NativeWorkspaceChangesDialogPurpose.Leave)
                {
                    return NativeWorkspaceLeaveChoiceOutcome.ContinueDialog;
                }
                ...
                return await ApplyPersistenceDialogChoiceAsync(
                    ...
                    request,
                    cancellationToken).ConfigureAwait(false);
            case NativeWorkspaceChangesDialogChoice.ConfirmAbandonmentForOpen:
                if (!CanConfirmAbandonmentForOpen
                    || request.LeaveReason != NativeWorkspaceLeaveReason.OpenWorkspace
                ...
                lock (LifecycleGate)
                {
                    PendingLeaveDisposition = NativeWorkspaceLeaveDisposition.ConfirmedAbandonmentForOpen;
                }
    ...
    private async Task<NativeWorkspaceLeaveChoiceOutcome> CompleteEditorTransitionAndClassifyAsync(
        NativeWorkspaceTransitionDrainMode drainMode,
        NativeWorkspaceChangesOperationState operationState)
    {
        ...
        await RunOperationAsync(
            ...
            async _ =>
            {
                if (drainMode == NativeWorkspaceTransitionDrainMode.CancelAndWaitForCurrentOperation)
                {
    ...
    private async Task<NativeWorkspaceLeaveChoiceOutcome> ClassifyPostDrainLeaveStateAsync(CancellationToken cancellationToken)
    {
    ...
    private async Task<NativeWorkspaceLeaveReservation?> FinalizeProvedLeaveAsync(
        NativeWorkspaceLeaveReason reason,
        CancellationToken cancellationToken)
    {
        ...
        NativeWorkspaceLeaveDisposition? preliminaryDisposition;
        lock (LifecycleGate)
        {
        ...
        {
            case NativeWorkspaceLeaveDisposition.ReadyAndClean:
                ...
                return cleanWorkspaceId is null || cancellationToken.IsCancellationRequested
                    ...
                    : TransferOwnedTransition(
                        reason,
                        NativeWorkspaceLeaveDisposition.ReadyAndClean,
                        cleanWorkspaceId,
                        cancellationToken);
            case NativeWorkspaceLeaveDisposition.NoWorkspace:
                return TryTransferNoWorkspace(reason, cancellationToken, out var noWorkspaceReservation)
                    ? noWorkspaceReservation
                    : null;
            case NativeWorkspaceLeaveDisposition.ConfirmedAbandonmentForOpen:
    ...
    private async Task<NativeWorkspaceLeaveChoiceOutcome> ApplyPersistenceDialogChoiceAsync(
        bool save,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.Purpose != NativeWorkspaceChangesDialogPurpose.Leave)
        {
            return NativeWorkspaceLeaveChoiceOutcome.Proceed;
        }
    ...
    /// <summary>Transfers a no-workspace reservation without borrowing or reading a native workspace.</summary>
    /// <param name="reason">The exact requested leave action.</param>
    ...
    private bool TryTransferNoWorkspace(
        NativeWorkspaceLeaveReason reason,
        CancellationToken cancellationToken,
        out NativeWorkspaceLeaveReservation? reservation)
    {
        reservation = null;
        if (!TryRecordNoWorkspaceDisposition())
        {
    ...
    private bool TryRecordNoWorkspaceDisposition()
    {
        ...
        lock (LifecycleGate)
        {
            PendingLeaveDisposition = NativeWorkspaceLeaveDisposition.NoWorkspace;
        }
    ...
    private NativeWorkspaceLeaveReservation? TransferOwnedTransition(
        NativeWorkspaceLeaveReason reason,
        ...
        CancellationToken cancellationToken)
    {
        lock (LifecycleGate)
        {
            ...
            var reservation = new NativeWorkspaceLeaveReservation(lease, reason, disposition, expectedWorkspaceId);
