...
public sealed partial class NativeWorkspaceChangesViewModel
{
    ...
    {
        return ShowDialogSessionAsync(NativeWorkspaceChangesDialogRequest.ForReview(), cancellationToken);
    }
    ...
    {
        return ShowDialogSessionAsync(NativeWorkspaceChangesDialogRequest.ForSave(), cancellationToken);
    }
    ...
    {
        return ShowDialogSessionAsync(NativeWorkspaceChangesDialogRequest.ForDiscard(), cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await RunOperationAsync(
                ...
                async token =>
                {
                    await AcquireTransitionAsync(NativeWorkspaceTransitionDrainMode.WaitForCurrentOperation, token).ConfigureAwait(false);
    ...
    private async Task<bool> CompleteOwnedTransitionRequestAsync(NativeWorkspaceTransitionDrainMode drainMode)
    {
        ...
        NativeWorkspaceTransitionLease? lease = null;
        var accepted = false;
        try
        {
        ...
        bool allowBlockedSynchronization)
    {
        ...
        try
        {
            items = Array.AsReadOnly(capture.Preview.Comparisons
                .Select(comparison => new NativeWorkspaceChangeItemViewModel(
                    comparison,
        ...
        {
            Logger.Error(exception, "Detached native change projection failed for workspace {WorkspaceId}", capture.WorkspaceId);
            await PublishFailureAsync(
                UnexpectedFailure("The detached native change review could not be displayed."),
                "The current workspace changes could not be reviewed.",
    ...
    private static async ValueTask<EngineResult<NativeWorkspaceReviewCapture>> CaptureReviewWithinBorrowAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return EngineResult<NativeWorkspaceReviewCapture>.Failure(
        ...
        if (state.OutputSynchronization.Status != global::CreationsForge.Core.Engine.Contracts.OutputSynchronizationStatus.Ready)
        {
            ...
            return EngineResult<NativeWorkspaceReviewCapture>.Success(
                new NativeWorkspaceReviewCapture(
                    workspace.WorkspaceId,
                    state,
        ...
        return EngineResult<NativeWorkspaceReviewCapture>.Success(
            new NativeWorkspaceReviewCapture(workspace.WorkspaceId, state, previewResult.Value, warnings),
            workspace.WorkspaceId,
            baseRevision: state.Revision,
            resultRevision: state.Revision,
            warnings: warnings);
    ...
    /// <summary>Determines whether two output associations describe the same exact native output.</summary>
