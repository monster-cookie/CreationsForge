...
public sealed partial class NativeFormListEditorViewModel
{
    ...
    public async Task ApplyAsync(CancellationToken cancellationToken)
    {
        ...
        using var operationLease = TryEnterOperation(NativeFormListEditorOperationState.Applying);
        ...
        NativeFormListEditorPendingApply? attemptedOperation = null;
        ...
        try
        {
            ...
            var result = await WorkspaceCoordinator.ExecuteAsync(
                async (workspace, token) =>
                {
                    ...
                    {
                        return Failure<NativeFormListEditorApplyOutcome>(EngineErrorCode.ExternalChangeDetected, "The native wire catalog no longer matches the active editor session.");
                    }
                    ...
                    if (!decode.Succeeded || decode.Value is null)
                    {
                        return EngineResult<NativeFormListEditorApplyOutcome>.Failure(
                            decode.Error ?? new EngineError(EngineErrorCode.ValidationFailed, "The exact native codec rejected the typed command without a diagnostic reason."),
                    ...
                    attemptedOperation = new NativeFormListEditorPendingApply(
                        session.WorkspaceId,
                        expectedRevision,
                        operationId,
                        session.EditId,
    ...
    private async ValueTask<NativeFormListEditorApplyOutcome> ReadApplyFollowUpAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new NativeFormListEditorApplyOutcome(
            receipt,
            seed,
            previewKnown,
            hasStagedChanges,
    ...
    private async Task PublishApplySuccessAsync(
        ...
        NativeWorkspaceDescriptor descriptor,
        NativeFormListEditorSession session,
        NativeFormListDraft draft,
        NativeFormListEditorApplyOutcome outcome,
        CancellationToken cancellationToken)
    {
        await UiDispatcher.InvokeAsync(() =>
        {
        ...
        EngineError? error)
    {
        return UiDispatcher.InvokeAsync(() =>
        {
            ...
            {
                var issue = NativeWireDraftErrorMapper.Map(draft, effective.Message);
    ...
    private Task PublishUncertainOperationAsync(
        long generation,
        Guid workspaceId,
        NativeFormListEditorPendingOperation pendingOperation)
    {
        return UiDispatcher.InvokeAsync(() =>
        {
            if (!IsCurrentGeneration(generation, workspaceId))
