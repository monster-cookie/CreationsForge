...
/// <summary>Parses the current closed record wire schema vocabulary into typed presentation descriptors.</summary>
internal static class RecordWireSchemaParser
{
    /// <summary>Resolves one schema node into a presentation descriptor.</summary>
    internal static EngineResult<RecordWireSchemaDescriptor> Resolve(IFormListEditWireSchemaCatalog catalog, RecordWireSchemaNodeKey key, RecordWireReadLimits limits, CancellationToken cancellationToken)
    {
        ...
        {
            return EngineResult<RecordWireSchemaDescriptor>.Failure(node.Error ?? new EngineError(EngineErrorCode.ValidationFailed, $"Schema node '{key.Name}' could not be read."));
        }
        ...
        try
        {
            ...
            cancellationToken.ThrowIfCancellationRequested();
            return EngineResult<RecordWireSchemaDescriptor>.Success(descriptor);
        }
    ...
    private static EngineResult<RecordWireSchemaDescriptor> Failure(string message)
    {
        return EngineResult<RecordWireSchemaDescriptor>.Failure(new EngineError(EngineErrorCode.ValidationFailed, message));
    }
    ...
    private sealed class ParseState
    {
        ...
        internal ParseState(IFormListEditWireSchemaCatalog catalog, RecordWireReadLimits limits, CancellationToken cancellationToken)
        {
            Catalog = catalog;
            Limits = limits;
        ...
        internal RecordWireSchemaDescriptor Parse(JsonElement schema, string fallbackTitle, string path, RecordWireSchemaNodeKey? sourceKey, JsonElement? defaultTemplate, HashSet<string> referenceStack, int depth = 1)
        {
            Visit(path, depth);
            if (schema.ValueKind != JsonValueKind.Object)
            {
                throw new SchemaParseException($"{path}: record wire schema fragments must be JSON objects.");
            }
            ...
            if (!hasPrimaryShape && (TryReadUnion(schema, "anyOf", out var anyOf) || TryReadUnion(schema, "oneOf", out anyOf)))
            {
                ...
                if (nullOptions.Length == 1 && options.Length == 2)
                {
                    ...
                    return new RecordWireSchemaDescriptor(
                        RecordWireSchemaValueKind.Nullable,
                        title,
                        description,
                        sourceKey,
                        defaultTemplate,
                ...
                return new RecordWireSchemaDescriptor(
                    RecordWireSchemaValueKind.Union,
                    title,
                    description,
                    sourceKey,
                    defaultTemplate,
            ...
            return type switch
            {
                ...
                "boolean" => new RecordWireSchemaDescriptor(RecordWireSchemaValueKind.Boolean, title, description, sourceKey, defaultTemplate, constant: constant, defaultValue: defaultValue, annotations: annotations),
                "null" => throw new SchemaParseException($"{path}: a standalone null schema is only valid inside a nullable union."),
                _ => throw new SchemaParseException($"{path}: unsupported record wire schema type '{type}'."),
            };
        }

        /// <summary>Parses a closed object and its ordered properties.</summary>
        private RecordWireSchemaDescriptor ParseObject(JsonElement schema, string title, string? description, RecordWireSchemaNodeKey? sourceKey, JsonElement? defaultTemplate, JsonElement? constant, JsonElement? defaultValue, IReadOnlyDictionary<string, string?> annotations, HashSet<string> referenceStack, string path, int depth)
        {
            ...
            var properties = new List<RecordWireSchemaProperty>();
            if (schema.TryGetProperty("properties", out var propertiesElement))
            {
                ...
                foreach (var property in propertiesElement.EnumerateObject())
                {
                    ...
                    var optionalDerived = readOnly && !property.Value.TryGetProperty("const", out _) &&
                        (property.Value.TryGetProperty("x-record-derived", out _) || property.Value.TryGetProperty("x-record-derived-value", out _) || property.Name.StartsWith("uses", StringComparison.Ordinal));
                    properties.Add(new RecordWireSchemaProperty(property.Name, order++, required.Contains(property.Name) && !optionalDerived, readOnly, descriptor));
                }
            }

            var kind = ClassifyObject(schema, properties, annotations);
            return new RecordWireSchemaDescriptor(kind, title, description, sourceKey, defaultTemplate, Array.AsReadOnly(properties.ToArray()), constant: constant, defaultValue: defaultValue, annotations: annotations);
        }

        /// <summary>Parses one ordered array descriptor.</summary>
        private RecordWireSchemaDescriptor ParseArray(JsonElement schema, string title, string? description, RecordWireSchemaNodeKey? sourceKey, JsonElement? defaultTemplate, JsonElement? constant, JsonElement? defaultValue, IReadOnlyDictionary<string, string?> annotations, HashSet<string> referenceStack, string path, int depth)
        {
            if (!schema.TryGetProperty("items", out var itemsElement) || itemsElement.ValueKind != JsonValueKind.Object)
            {
                throw new SchemaParseException($"{path}.items: record wire arrays require one closed item schema.");
            }
            ...
            return new RecordWireSchemaDescriptor(RecordWireSchemaValueKind.Array, title, description, sourceKey, defaultTemplate, itemDescriptor: item, constant: constant, defaultValue: defaultValue, annotations: annotations, maximumItems: maximumItems);
        }

        /// <summary>Parses one integer or enum descriptor.</summary>
        private RecordWireSchemaDescriptor ParseInteger(JsonElement schema, string title, string? description, RecordWireSchemaNodeKey? sourceKey, JsonElement? defaultTemplate, JsonElement? constant, JsonElement? defaultValue, IReadOnlyDictionary<string, string?> annotations, string path)
        {
            var enumValues = new List<RecordWireSchemaEnumValue>();
            if (schema.TryGetProperty("x-record-enum-values", out var valuesElement))
            {
                if (valuesElement.ValueKind != JsonValueKind.Array)
                {
                    throw new SchemaParseException($"{path}.x-record-enum-values: expected an array.");
                }
                ...
                foreach (var value in valuesElement.EnumerateArray())
                {
                    ...
                    {
                        throw new SchemaParseException($"{path}.x-record-enum-values: each value requires string name and value properties.");
                    }

                    enumValues.Add(new RecordWireSchemaEnumValue(nameElement.GetString()!, numberElement.GetString()!));
                }
            }

            return new RecordWireSchemaDescriptor(
                enumValues.Count == 0 ? RecordWireSchemaValueKind.Integer : RecordWireSchemaValueKind.Enum,
                title,
                description,
                sourceKey,
                defaultTemplate,
        ...
        private RecordWireSchemaDescriptor ParseString(JsonElement schema, string title, string? description, RecordWireSchemaNodeKey? sourceKey, JsonElement? defaultTemplate, JsonElement? constant, JsonElement? defaultValue, IReadOnlyDictionary<string, string?> annotations, string path)
        {
            var format = ReadOptionalString(schema, "format", path) ?? ReadOptionalString(schema, "contentEncoding", path);
            return new RecordWireSchemaDescriptor(
                RecordWireSchemaValueKind.String,
                title,
                description,
                sourceKey,
                defaultTemplate,
        ...
        private static RecordWireSchemaDescriptor ParseNumber(JsonElement schema, string title, string? description, RecordWireSchemaNodeKey? sourceKey, JsonElement? defaultTemplate, JsonElement? constant, JsonElement? defaultValue, IReadOnlyDictionary<string, string?> annotations)
        {
        ...
        private RecordWireSchemaUnionOption CreateUnionOption(JsonElement option, int index, string parentTitle, string path, HashSet<string> referenceStack, int depth)
        {
            CancellationToken.ThrowIfCancellationRequested();
            ...
            if (option.TryGetProperty("$ref", out var referenceElement) && referenceElement.ValueKind == JsonValueKind.String)
            {
                ...
                return new RecordWireSchemaUnionOption(
                    targetKey.Name,
                    display,
                    targetKey.Name,
                    (limits, token) => RecordWireSchemaParser.Resolve(Catalog, targetKey, limits, token));
            }
            ...
            return new RecordWireSchemaUnionOption(
                ...
                (limits, token) =>
                {
                    try
                    {
                        ...
                        return EngineResult<RecordWireSchemaDescriptor>.Success(state.Parse(optionCopy, displayName, path, null, null, new HashSet<string>(referenceStack, StringComparer.Ordinal), depth + 1));
                    }
                    catch (SchemaParseException exception)
                    {
                        return Failure(exception.Message);
        ...
        private RecordWireSchemaNodeKey ResolveReferenceKey(string reference, string path)
        {
            ...
            var matches = Catalog.Nodes.Where(candidate => candidate.Kind == RecordWireSchemaNodeKind.Type && string.Equals(candidate.Name, name, StringComparison.Ordinal)).ToArray();
            if (matches.Length != 1)
            {
        ...
        /// <summary>Classifies recognized record wrapper objects before falling back to an ordinary object.</summary>
        private static RecordWireSchemaValueKind ClassifyObject(JsonElement schema, IReadOnlyList<RecordWireSchemaProperty> properties, IReadOnlyDictionary<string, string?> annotations)
        {
            var names = properties.Select(property => property.Name).ToHashSet(StringComparer.Ordinal);
            var recordType = string.Join(' ', annotations.Values.Where(value => value is not null));
            var normalizedRecordType = new string(recordType.Where(char.IsLetterOrDigit).ToArray());
            if (names.SetEquals(new[] { "isNull", "formKey" }) || recordType.Contains("FormLink", StringComparison.Ordinal) && !recordType.Contains("FormLinkOrIndex", StringComparison.Ordinal) && names.Contains("formKey"))
            {
                return RecordWireSchemaValueKind.FormLink;
            }

            if (recordType.Contains("FormLinkOrIndex", StringComparison.Ordinal) || names.Contains("index") && names.Contains("link") && names.Contains("usesLink"))
            {
                return RecordWireSchemaValueKind.FormLinkOrIndex;
            }

            if (recordType.Contains("Array2d", StringComparison.OrdinalIgnoreCase) || annotations.ContainsKey("x-record-array2d"))
            {
                return RecordWireSchemaValueKind.Array2D;
            }

            if (normalizedRecordType.Contains("TranslatedString", StringComparison.OrdinalIgnoreCase) || names.Contains("translations") && names.Contains("targetLanguage"))
            {
                return RecordWireSchemaValueKind.TranslatedString;
            }

            if (normalizedRecordType.Contains("AssetLink", StringComparison.OrdinalIgnoreCase) || names.Contains("givenPath") && names.Contains("isNull"))
            {
                return RecordWireSchemaValueKind.Asset;
            }

            if (recordType.Contains("Color", StringComparison.OrdinalIgnoreCase) && (names.Contains("red") || names.Contains("r") || names.Contains("raw")))
            {
                return RecordWireSchemaValueKind.Color;
            }

            if (names.Contains("bits") && (names.Contains("value") || names.Contains("text") || recordType.Contains("Single", StringComparison.Ordinal) || recordType.Contains("Double", StringComparison.Ordinal)))
            {
                return RecordWireSchemaValueKind.FloatBits;
            }

            if (names.Contains("length") && names.Contains("base64"))
            {
                return RecordWireSchemaValueKind.ByteArray;
            }

            return RecordWireSchemaValueKind.Object;
        }
        ...
        /// <summary>Preserves all record annotation values as exact raw JSON or string content.</summary>
