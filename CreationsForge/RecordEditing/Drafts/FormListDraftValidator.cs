...
/// <summary>Validates typed record wire drafts with exact numeric, structural, and resource semantics.</summary>
public sealed class FormListDraftValidator : IFormListDraftValidator
{
    ...
    /// <inheritdoc />
    public FormListDraftValidationResult Validate(FormListDraft draft, RecordWireReadLimits limits, CancellationToken cancellationToken = default)
    {
        ...
        var result = new FormListDraftValidationResult(state.Issues);
    ...
    private sealed class ValidationState
    {
        ...
        private readonly List<RecordWireDraftIssue> MutableIssues = new();

        /// <summary>The number of visited materialized nodes.</summary>
        private int VisitedNodes;

        /// <summary>Initializes one bounded validation state.</summary>
        internal ValidationState(RecordWireReadLimits limits, CancellationToken cancellationToken)
        {
            Limits = limits;
        ...
        internal IReadOnlyList<RecordWireDraftIssue> Issues => MutableIssues;
        ...
        internal void Validate(RecordWireDraftNode node, int depth, bool expandRequiredObject = false)
        {
            CancellationToken.ThrowIfCancellationRequested();
            if (depth > Limits.MaximumDepth)
            {
                Add(RecordWireDraftIssueCode.ResourceLimitExceeded, node, $"{node.Path}: draft exceeds maximum depth {Limits.MaximumDepth}.");
                return;
            }

            if (++VisitedNodes > Limits.MaximumNodes)
            {
                Add(RecordWireDraftIssueCode.ResourceLimitExceeded, node, $"{node.Path}: draft exceeds maximum node count {Limits.MaximumNodes}.");
                return;
            }

            if (node.ValueState == RecordWireDraftValueState.RequiredUnset && !(expandRequiredObject && node is RecordWireObjectDraftNode))
            {
                if (node.IsRequired)
                {
                    var reason = node.Descriptor.Annotations.GetValueOrDefault("x-default-unavailable-reason");
                    Add(RecordWireDraftIssueCode.RequiredValueUnset, node, $"{node.Path}: required value is unset{(string.IsNullOrWhiteSpace(reason) ? "." : $"; {reason}")}");
                }
            ...
            switch (node)
            {
                ...
                case RecordWireUnionDraftNode union:
                    ...
                    {
                        Add(RecordWireDraftIssueCode.RequiredValueUnset, union, $"{union.Path}: select an explicit union value.");
                    }
                    else
                    {
                    ...
                    break;
                case RecordWireNullableDraftNode nullable when !nullable.IsNull && nullable.Value is null:
                    Add(RecordWireDraftIssueCode.RequiredValueUnset, nullable, $"{nullable.Path}: present nullable value has no typed value.");
                    break;
                case RecordWireNullableDraftNode nullable when !nullable.IsNull:
                ...
                case RecordWireFormLinkOrIndexDraftNode formLinkOrIndex:
                    if (!formLinkOrIndex.IsIndexNull)
                    {
                        ...
                        {
                            Add(RecordWireDraftIssueCode.SchemaResolutionFailed, formLinkOrIndex, $"{formLinkOrIndex.Path}.index: FormLinkOrIndex schema is missing the index descriptor.");
                        }
                        else
                        {
                ...
                case RecordWireFormLinkDraftNode formLink:
                    ...
                    break;
                case RecordWireFloatBitsDraftNode floatBits:
                    ValidateChildren(floatBits, depth);
                    ValidateFloatBits(floatBits);
                    break;
                case RecordWireByteArrayDraftNode bytes:
                    ValidateChildren(bytes, depth);
                    ValidateBytes(bytes);
                    break;
                case RecordWireArray2DDraftNode array2D:
        ...
        private void ValidateChildren(RecordWireObjectDraftNode objectNode, int depth)
        {
        ...
        private void ValidateArray(RecordWireArrayDraftNode array, int depth)
        {
            var maximum = Math.Min(Limits.MaximumArrayElements, array.Descriptor.MaximumItems ?? int.MaxValue);
            if (array.Items.Count > maximum)
            {
                Add(RecordWireDraftIssueCode.ResourceLimitExceeded, array, $"{array.Path}: array contains {array.Items.Count} elements, exceeding maximum {maximum}.");
            }
            ...
            if (!string.IsNullOrWhiteSpace(uniqueProperty))
            {
                ...
                foreach (var item in array.Items.OfType<RecordWireObjectDraftNode>())
                {
                    var property = item.FindProperty(uniqueProperty);
                    if (property is null || property.ValueState == RecordWireDraftValueState.RequiredUnset)
                    {
                        continue;
                    }

                    var canonical = RecordWireDraftJsonWriter.WriteDetached(property, CancellationToken).GetRawText();
                    if (!values.Add(canonical))
                    {
                        Add(RecordWireDraftIssueCode.CollectionConstraintViolated, property, $"{property.Path}: duplicate value violates unique '{uniqueProperty}' collection key.");
                    }
                }
            }
        }
        ...
        private void ValidateIntegerText(RecordWireDraftNode node, string text, string? minimum, string? maximum)
        {
            ...
            {
                Add(RecordWireDraftIssueCode.InvalidValue, node, $"{node.Path}: integer must use canonical decimal text without exponent, separators, or redundant leading characters.");
                return;
            }
            ...
            {
                Add(RecordWireDraftIssueCode.NumericValueOutOfRange, node, $"{node.Path}: integer {text} is below minimum {minimum}.");
            }
            ...
            {
                Add(RecordWireDraftIssueCode.NumericValueOutOfRange, node, $"{node.Path}: integer {text} exceeds maximum {maximum}.");
            }
        }

        /// <summary>Validates one bounded string, exact JSON-number projection, and schema pattern.</summary>
        private void ValidateString(RecordWireStringDraftNode text)
        {
            if (text.Value.Length > Limits.MaximumStringLength)
            {
                Add(RecordWireDraftIssueCode.ResourceLimitExceeded, text, $"{text.Path}: string length {text.Value.Length} exceeds operation maximum {Limits.MaximumStringLength}.");
            }

            if (text.Descriptor.MinimumLength.HasValue && text.Value.Length < text.Descriptor.MinimumLength.Value)
            {
                Add(RecordWireDraftIssueCode.StringConstraintViolated, text, $"{text.Path}: string length {text.Value.Length} is below minimum {text.Descriptor.MinimumLength.Value}.");
            }

            if (text.Descriptor.MaximumLength.HasValue && text.Value.Length > text.Descriptor.MaximumLength.Value)
            {
                Add(RecordWireDraftIssueCode.StringConstraintViolated, text, $"{text.Path}: string length {text.Value.Length} exceeds maximum {text.Descriptor.MaximumLength.Value}.");
            }
            ...
            if (text.Descriptor.Annotations.ContainsKey("x-presentation-json-number"))
            {
                try
                {
                    ...
                    {
                        Add(RecordWireDraftIssueCode.InvalidValue, text, $"{text.Path}: value must be one exact JSON number token.");
                    }
                }
                catch (JsonException)
                {
            ...
            if (text.Descriptor.Pattern is not null)
            {
                try
                {
                    ...
                    {
                        Add(RecordWireDraftIssueCode.StringConstraintViolated, text, $"{text.Path}: string does not match required pattern {text.Descriptor.Pattern}.");
                    }
                }
                catch (ArgumentException)
                {
                    Add(RecordWireDraftIssueCode.SchemaResolutionFailed, text, $"{text.Path}: catalog contains an invalid regular-expression pattern.");
                }
                catch (RegexMatchTimeoutException)
                {
                    Add(RecordWireDraftIssueCode.ResourceLimitExceeded, text, $"{text.Path}: pattern validation exceeded its bounded timeout.");
                }
            }
        }

        /// <summary>Validates FormLink null identity without normalizing JSON null and canonical Null.</summary>
        private void ValidateFormLink(RecordWireFormLinkDraftNode formLink)
        {
            if (formLink.IsNull)
            {
                if (formLink.FormKey is not null && !string.Equals(formLink.FormKey, "Null", StringComparison.Ordinal))
                {
                    Add(RecordWireDraftIssueCode.InvalidValue, formLink, $"{formLink.Path}: an explicit null FormLink requires JSON null or canonical FormKey 'Null'.");
                }
            }
            else if (string.IsNullOrWhiteSpace(formLink.FormKey) || string.Equals(formLink.FormKey, "Null", StringComparison.Ordinal))
            {
                Add(RecordWireDraftIssueCode.InvalidValue, formLink, $"{formLink.Path}: a non-null FormLink requires a non-null canonical FormKey.");
            }
        }

        /// <summary>Validates Base64, decoded length, and redundant declared length before allocation.</summary>
        private void ValidateBytes(RecordWireByteArrayDraftNode bytes)
        {
            ...
            {
                Add(RecordWireDraftIssueCode.ResourceLimitExceeded, bytes, $"{bytes.Path}.base64: encoded value can exceed decoded-byte maximum {Limits.MaximumDecodedByteLength}.");
                return;
            }
            ...
            {
                Add(RecordWireDraftIssueCode.InvalidValue, bytes, $"{bytes.Path}.base64: value is not valid Base64.");
                return;
            }
            ...
            {
                Add(RecordWireDraftIssueCode.InvalidValue, bytes.Length, $"{bytes.Length.Path}: declared byte length {declared} does not match decoded length {written}.");
            }
        }

        /// <summary>Validates authoritative float bits against redundant invariant text and optional finite JSON number.</summary>
        private void ValidateFloatBits(RecordWireFloatBitsDraftNode floatBits)
        {
            if (floatBits.FindProperty("bits") is not RecordWireStringDraftNode bits || floatBits.FindProperty("text") is not RecordWireStringDraftNode text)
            {
                Add(RecordWireDraftIssueCode.SchemaResolutionFailed, floatBits, $"{floatBits.Path}: float-bit draft requires text and bits properties.");
                return;
            }

            var number = floatBits.FindProperty("number") as RecordWireStringDraftNode;
            ...
            {
                Add(RecordWireDraftIssueCode.InvalidValue, bits, $"{bits.Path}: bits must be exact uppercase 0x-prefixed 8- or 16-digit hexadecimal text.");
            }
        }

        /// <summary>Validates redundant float text and number values against authoritative bits.</summary>
        private void ValidateFloatProjection<T>(RecordWireFloatBitsDraftNode owner, RecordWireStringDraftNode text, RecordWireStringDraftNode? number, string expectedText, bool finite, bool bitsRoundTrip, T exactValue) where T : struct, IFormattable
        {
            if (!bitsRoundTrip || !string.Equals(text.Value, expectedText, StringComparison.Ordinal))
            {
                Add(RecordWireDraftIssueCode.InvalidValue, text, $"{text.Path}: invariant text must match authoritative bits as '{expectedText}'.");
            }

            var numberPresent = number is not null && number.ValueState != RecordWireDraftValueState.RequiredUnset;
            if (finite && !numberPresent)
            {
                Add(RecordWireDraftIssueCode.RequiredValueUnset, owner, $"{owner.Path}.number: finite authoritative bits require a matching JSON number projection.");
            }
            else if (!finite && numberPresent)
            {
                Add(RecordWireDraftIssueCode.InvalidValue, number!, $"{number!.Path}: non-finite authoritative bits must omit the JSON number projection.");
            }
            else if (finite && numberPresent)
            {
                ...
                {
                    Add(RecordWireDraftIssueCode.InvalidValue, number, $"{number.Path}: JSON number must match authoritative bits value {expected}.");
                }
            }
        }
        ...
        private void ValidateArray2D(RecordWireArray2DDraftNode array2D)
        {
            if (array2D.FindProperty("width") is not RecordWireIntegerDraftNode width || array2D.FindProperty("height") is not RecordWireIntegerDraftNode height || array2D.FindProperty("rows") is not RecordWireArrayDraftNode rows ||
            ...
            {
                Add(RecordWireDraftIssueCode.InvalidValue, array2D, $"{array2D.Path}: two-dimensional array requires canonical width, height, and rows values.");
                return;
            }

            if (rows.Items.Count != expectedHeight)
            {
                Add(RecordWireDraftIssueCode.CollectionConstraintViolated, rows, $"{rows.Path}: row count {rows.Items.Count} does not match height {expectedHeight}.");
            }

            for (var index = 0; index < rows.Items.Count; index++)
            {
                if (rows.Items[index] is not RecordWireArrayDraftNode row || row.Items.Count != expectedWidth)
                {
                    Add(RecordWireDraftIssueCode.CollectionConstraintViolated, rows.Items[index], $"{rows.Items[index].Path}: row width must equal {expectedWidth}.");
                }
            }
        }

        /// <summary>Adds one exact node-bound issue.</summary>
        private void Add(RecordWireDraftIssueCode code, RecordWireDraftNode node, string message)
        {
            MutableIssues.Add(new RecordWireDraftIssue(code, node.Path, message, node));
        }
    }
}
