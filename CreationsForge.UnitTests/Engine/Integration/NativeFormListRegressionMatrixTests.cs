...
public sealed class NativeFormListRegressionMatrixTests
{
    /// <summary>The exact common version-control value expected after native save and reopen.</summary>
    private const uint ExpectedVersionControl = 0x10203040;

    /// <summary>The exact common FormVersion value expected after native save and reopen.</summary>
    private const ushort ExpectedFormVersion = 55;

    /// <summary>The exact common Version2 value expected after native save and reopen.</summary>
    private const ushort ExpectedVersion2 = 7;

    /// <summary>The raw native compressed-record bit set by every supported game adapter.</summary>
    ...
    {
        /// <summary>Uses a concrete native FormKey link.</summary>
        Link,

        /// <summary>Uses the native null FormKey representation while link mode remains active.</summary>
        NullLink,
    ...
    /// <summary>Verifies a committed source override preserves a previously committed empty output record and every selected native value.</summary>
    /// <param name="game">The native game whose complete production composition is exercised.</param>
    ...
    public async Task EmbeddedOutput_SourceOverridePreservesAllocatedEmptyRecordAndFreshlyReopens(SupportedGame game)
    {
        ...
        await using var services = NativeEngineComposition.Create(logger);
    ...
    /// <summary>Verifies every Starfield link-or-index mode preserves its active native value across save and fresh reopen.</summary>
    /// <param name="mode">The owner-derived native condition-parameter mode.</param>
    ...
    /// <param name="workspace">The independently owned native workspace.</param>
    ...
    /// <param name="workspace">The selected native workspace.</param>
    /// <param name="role">The requested native edit role.</param>
    ...
    /// <param name="workspace">The dirty native workspace.</param>
    ...
    /// <summary>Reads one exact output FormList through the detached native inspector view.</summary>
    /// <param name="workspace">The selected native workspace.</param>
    /// <param name="association">The selected output identity.</param>
    /// <param name="formKey">The exact output FormList identity.</param>
    /// <param name="logSink">The test-owned structured log sink used to expose an underlying native reader exception.</param>
    /// <returns>The complete detached native JSON record.</returns>
    ...
    /// <param name="game">The game that owns the mutable native FormList.</param>
        ...
        NativeWorkspaceIntegrationFixture fixture)
    {
        ...
        switch (game)
        {
            ...
            case SupportedGame.Skyrim:
                ...
                break;
            default:
                throw new ArgumentOutOfRangeException(nameof(game), game, "The regression matrix requires a supported native game.");
        }
    ...
    /// <param name="keywordFormKey">The resolvable source Keyword retained by the active native condition parameter.</param>
    ...
    /// <param name="mode">The native owner mode to encode.</param>
    ...
    /// <summary>Asserts the active owner-derived Starfield condition parameter survives native save and reopen.</summary>
    /// <param name="record">The complete detached native FormList JSON record.</param>
    /// <param name="mode">The native owner mode expected in the condition data.</param>
    ...
    /// <param name="fixture">The generated native identities expected in links and nested payloads.</param>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="game"/> is undefined.</exception>
    ...
    /// <summary>Asserts one expected typed major-record bit remains set after native serialization.</summary>
    /// <param name="record">The complete detached native FormList JSON record.</param>
    ...
    /// <summary>Asserts an embedded native Name retains its exact English value.</summary>
    /// <param name="record">The complete detached native FormList JSON record.</param>
    ...
    /// <summary>Writes one detached Starfield FormList through the production complete native inspector.</summary>
    ...
    private static JsonElement WriteStarfieldReadView(Mutagen.Bethesda.Starfield.IFormListGetter record)
    {
        ...
        {
            new StarfieldFormListNativeInspector().WriteReadView(
                record,
                writer,
                TestContext.Current.CancellationToken);
        }
    ...
    /// <summary>Collects structured exceptions emitted by one independently owned native engine composition.</summary>
    private sealed class CollectingLogSink : ILogEventSink
    {
        /// <summary>Serializes access because native reads run on worker threads.</summary>
        ...
        public string DescribeExceptions()
        {
            lock (_gate)
            {
                ...
                return exceptions.Length == 0
                    ? "Captured native exceptions: <none>."
                    : $"Captured native exceptions: {string.Join(" | ", exceptions)}";
            }
        }
    }
}
