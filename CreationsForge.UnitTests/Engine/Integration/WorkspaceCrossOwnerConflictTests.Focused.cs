...
public sealed partial class WorkspaceCrossOwnerConflictTests
{
    ...
    /// <returns>A task that completes after the retry commits and plugin state reopens.</returns>
    [Fact]
    public async Task SaveAsync_ExternalGuardFileContention_PreservesStagedStateAndRetryCommits()
    {
        using var fixture = WorkspaceIntegrationFixture.Create(SupportedGame.Fallout4);
        var sourceArtifacts = fixture.SnapshotArtifacts();
        var output = CreateEmbeddedOutputAssociation(fixture);
    ...
    public async Task SaveAsync_DirectArtifactDrift_IsRejectedByBothOwnersWithoutOverwrite()
    {
        ...
        await using var servicesA = EngineComposition.Create();
    ...
    /// <summary>Verifies disposing one independent owner leaves another dirty owner's plugin reads and preview usable.</summary>
