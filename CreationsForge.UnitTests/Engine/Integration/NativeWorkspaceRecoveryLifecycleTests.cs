...
/// <summary>Verifies physically terminal native save recovery remains usable through the complete workspace lifecycle.</summary>
public sealed class NativeWorkspaceRecoveryLifecycleTests
{
    ...
    /// <summary>Verifies an unpublished partial initialization directory cannot block a valid native output in the same directory.</summary>
    /// <param name="game">The native game exercised through production composition.</param>
    ...
    public async Task PartialInitializationDirectory_DoesNotBlockFreshNativeOutputSelection(SupportedGame game)
    {
    ...
    /// <summary>Creates and commits one valid native output through the production workspace composition.</summary>
    /// <param name="services">The production native service lifetime.</param>
    /// <param name="fixture">The generated native fixture.</param>
    ...
    /// <summary>Captures the deterministic source baseline produced by the same explicit native input loader used by composition.</summary>
    ...
    private static async Task<NativeSourceInputBaseline> CaptureSourceBaselineAsync(WorkspaceOpenRequest request)
    {
        var prepared = await new NativeSourceInputLoader().PrepareAsync(
            request,
            TestContext.Current.CancellationToken);
    ...
    /// <summary>Seeds one canonical nonterminal journal with an exact all-before or all-prepared native destination.</summary>
    /// <param name="fixture">The generated native fixture that owns all disposable paths.</param>
        ...
        switch (physicalState)
        {
            ...
            case RecoveryPhysicalState.PreparedAllPrepared:
                ...
                for (var index = 0; index < beforeBaseline.Artifacts.Count; index++)
                {
                    ...
                    if (beforeArtifact.Role != NativeArtifactRole.Plugin)
                    {
                        ...
                        var absentStaged = await NativeFileInspector.InspectAsync(
                            stagedPath,
                            beforeArtifact.Role,
    ...
    /// <param name="workspace">The selected production native workspace.</param>
    ...
    /// <param name="expectedEditorId">The EditorID expected after native reopen.</param>
    /// <returns>A task that completes after detached native inspection.</returns>
