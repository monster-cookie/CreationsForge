...
/// <summary>Creates real plugin recovery states shared by engine and physical stdio cross-owner tests.</summary>
internal static class WorkspaceRecoveryFixture
{
    /// <summary>Creates one recognized plugin prior-save scenario and optionally pauses before the prior save or transaction mutation.</summary>
    ...
    internal static async Task<WorkspaceRecoverySeed> CreateAsync(
        WorkspaceIntegrationFixture fixture,
        OutputAssociation output,
        WorkspaceRecoveryPhysicalState state,
        Func<WorkspaceRecoverySetup, CancellationToken, Task>? beforeTransaction,
        CancellationToken cancellationToken)
    {
        ...
        if (state == WorkspaceRecoveryPhysicalState.MutationStartedMixed)
        {
        ...
        var preparedEditorId = state == WorkspaceRecoveryPhysicalState.PreparingAllBefore
            ? null
        ...
        var before = await CreateCommittedOutputAsync(
            ...
            cancellationToken).ConfigureAwait(false);
        var setup = new WorkspaceRecoverySetup(
            output,
            before.FormKey,
            before.Baseline,
            beforeEditorId,
        ...
        return state switch
        {
            WorkspaceRecoveryPhysicalState.PreparingAllBefore => await SeedPreparingAllBeforeAsync(
                fixture,
                setup,
                cancellationToken).ConfigureAwait(false),
            WorkspaceRecoveryPhysicalState.PreparedAllPrepared => await SeedPreparedAllPreparedAsync(
                fixture,
                setup,
                cancellationToken).ConfigureAwait(false),
    ...
    /// <summary>Creates one real plugin factory around a caller-supplied save coordinator.</summary>
    /// <param name="game">The supported plugin game.</param>
        ...
        IWorkspaceSaveCoordinator saveCoordinator)
    {
        ...
        var inputLoader = new PluginSourceInputLoader();
        var outputInputLoader = new PluginOutputInputLoader();
    ...
    private static async Task<WorkspaceRecoverySeed> SeedPreparingAllBeforeAsync(
        WorkspaceIntegrationFixture fixture,
        WorkspaceRecoverySetup setup,
        CancellationToken cancellationToken)
    {
        var identity = await InitializeJournalAsync(fixture, setup, cancellationToken).ConfigureAwait(false);
        return CreateSeed(
            WorkspaceRecoveryPhysicalState.PreparingAllBefore,
            setup,
            identity.Paths,
            identity.Journal,
    ...
    private static async Task<WorkspaceRecoverySeed> SeedPreparedAllPreparedAsync(
        WorkspaceIntegrationFixture fixture,
        WorkspaceRecoverySetup setup,
        CancellationToken cancellationToken)
    {
        ...
        var plans = await CreatePreparedPlansAsync(
            ...
            prepared.Baseline,
            cancellationToken).ConfigureAwait(false);
        var journal = identity.Journal.WithPreparedArtifacts(PluginWriteDisposition.StagedChanges, plans);
        ...
        return CreateSeed(
            WorkspaceRecoveryPhysicalState.PreparedAllPrepared,
            setup,
            identity.Paths,
            journal,
            preparedBaseline,
    ...
    /// <summary>Runs a real first-publication plugin save that faults after its first content-changing destination mutation.</summary>
    /// <param name="fixture">The generated plugin source fixture.</param>
    ...
    private static async Task<WorkspaceRecoverySeed> CreateMutationStartedMixedAsync(
        WorkspaceIntegrationFixture fixture,
        OutputAssociation output,
        Func<WorkspaceRecoverySetup, CancellationToken, Task>? beforeTransaction,
        CancellationToken cancellationToken)
    {
        ...
        var beforeBaseline = await PluginSaveArtifactUtilities.CaptureOutputAsync(
        ...
        var localizedEdit = CreateLocalizedNameEdit(
            ...
            var current = await PluginSaveArtifactUtilities.CaptureOutputAsync(
                ...
                output,
                cancellationToken).ConfigureAwait(false);
            PluginSaveArtifactUtilities.MatchBaseline(beforeBaseline, current).ShouldBeFalse();
            PluginSaveArtifactUtilities.MatchBaseline(preparedBaseline, current).ShouldBeFalse();
            return CreateSeed(
    ...
    /// <summary>Creates durable staged, publication, backup, and retirement plans for a prepared plugin set.</summary>
    ...
    {
        return PluginSaveArtifactUtilities.CreateOutputBaseline(plans.Select(plan =>
    ...
    private static WorkspaceRecoverySeed CreateSeed(
        WorkspaceRecoveryPhysicalState state,
        WorkspaceRecoverySetup setup,
        SaveTransactionPaths paths,
    ...
    /// <summary>Selects a plugin output through the public workspace contract.</summary>
    ...
    /// <summary>Begins a plugin output edit through the public workspace contract.</summary>
    ...
    /// <param name="game">The plugin game whose typed edit is required.</param>
    ...
    /// <param name="request">The complete explicit plugin open request.</param>
    ...
    private static async Task<PluginSourceInputBaseline> CaptureSourceBaselineAsync(
        WorkspaceOpenRequest request,
        CancellationToken cancellationToken)
    {
        var prepared = await new PluginSourceInputLoader().PrepareAsync(request, cancellationToken).ConfigureAwait(false);
    ...
    private static async Task<PluginArtifactAssociation> InspectAsync(
        string path,
        PluginArtifactAssociation destination,
        bool mustExist,
        CancellationToken cancellationToken)
    {
        return await PluginFileInspector.InspectAsync(
            path,
            destination.Role,
    ...
    private static bool ArtifactContentMatches(
        PluginArtifactAssociation left,
        PluginArtifactAssociation right)
    {
        return left.Fingerprint.Equals(right.Fingerprint);
    }

    /// <summary>Creates the real Starfield plugin adapter.</summary>
    ...
    private static IFormListGameAdapter CreateStarfieldAdapter(
        PluginSourceInputLoader sourceInputLoader,
        PluginOutputInputLoader outputInputLoader)
    {
        var outputService = new StarfieldPluginOutputService(outputInputLoader);
        return new StarfieldFormListGameAdapter(
            new StarfieldPluginSourceLoader(sourceInputLoader),
            outputService,
            new StarfieldRecordEditService(),
            new StarfieldPluginWriter(outputService));
    }

    /// <summary>Creates the real Fallout 4 plugin adapter.</summary>
    /// <param name="sourceInputLoader">The shared explicit source-input loader.</param>
    /// <param name="outputInputLoader">The shared output-input loader.</param>
    ...
    private static IFormListGameAdapter CreateFallout4Adapter(
        ...
        PluginOutputInputLoader outputInputLoader)
    {
        var outputService = new Fallout4PluginOutputService(outputInputLoader);
        return new Fallout4FormListGameAdapter(
            new Fallout4PluginSourceLoader(sourceInputLoader),
            outputService,
            new Fallout4RecordEditService(outputService.Inspector),
            new Fallout4PluginWriteService(outputService));
    }

    /// <summary>Creates the real Skyrim plugin adapter.</summary>
    /// <param name="sourceInputLoader">The shared explicit source-input loader.</param>
    /// <param name="outputInputLoader">The shared output-input loader.</param>
    ...
    private static IFormListGameAdapter CreateSkyrimAdapter(
        ...
        PluginOutputInputLoader outputInputLoader)
    {
        var outputService = new SkyrimPluginOutputService(outputInputLoader);
        return new SkyrimFormListGameAdapter(
            new SkyrimPluginSourceLoader(sourceInputLoader),
            outputService,
    ...
    private sealed class ThrowAfterFirstMutationFileOperations : SaveTransactionFileOperations
    {
        ...
        internal override void AfterDestinationMutation(Guid saveOperationId, int artifactIndex)
        {
            ...
            {
                throw new InvalidOperationException("Injected interruption after the first plugin destination mutation.");
            }
        }
    }
}
