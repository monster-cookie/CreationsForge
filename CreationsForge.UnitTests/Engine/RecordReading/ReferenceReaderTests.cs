...
namespace CreationsForge.UnitTests.Engine.RecordReading;

/// <summary>
/// Verifies shared direct plugin-group lookup, provenance, paging, copy isolation, and cancellation behavior.
/// </summary>
public sealed class ReferenceReaderTests
{
    ...
    public void Resolve_AllContextsRequiresContainingPluginWhenOverridesExist()
    {
        ...
        selected.Value.SourcePath.ShouldBe(Path.Combine(
            Path.GetTempPath(),
            "CreationsForge.RecordReading",
            "0-Origin.esm"));
    }

    /// <summary>Verifies resolved records retain their concrete family and ordered record fields while remaining detached.</summary>
    [Fact]
    public void Resolve_ReturnsDetachedPluginSubtypeAndOrderedFields()
    {
        var source = new StarfieldMod("Origin.esm", StarfieldRelease.Starfield);
    ...
    /// <summary>Verifies bounded pages preserve every context exactly once in deterministic plugin order.</summary>
    [Fact]
    ...
    /// <summary>Verifies cancellation is observed after a supported plugin deep-copy boundary.</summary>
    [Fact]
    public void Resolve_CancellationDuringCopyDoesNotPublishRecord()
    {
        ...
        var pluginSource = new ReferenceSource(
            source,
            Path.Combine(Path.GetTempPath(), "CreationsForge.RecordReading", "Origin.esm"),
            0,
            PluginRole.Source,
            record =>
            {
        ...
        var reader = new ReferenceReader(new[] { pluginSource });
    ...
    public void Read_MissingDirectReferenceReportsTypedPathWarning()
    {
        ...
        var reader = new ReferenceReader(
            new[] { pluginSource },
            _ => new[] { new FormLinkReference(missingKey, "Items[0]") });

        var result = reader.Read(new ReferenceRequest(listKey, RecordScope.Source));

        result.Succeeded.ShouldBeTrue(result.Error?.Message);
        result.Warnings.Single().Code.ShouldBe("missing-plugin-reference");
        result.Warnings.Single().Message.ShouldContain(missingKey.ToString());
        result.Warnings.Single().Message.ShouldContain("Items[0]");
    }

    /// <summary>Verifies the public fallback read retains plugin null links while warning only for real unresolved targets at their original position.</summary>
    [Fact]
    public void Read_DefaultDirectLinksSkipsPluginNullSentinelAndRetainsOriginalWarningPosition()
    {
        var source = new StarfieldMod("Origin.esm", StarfieldRelease.Starfield);
        var listKey = new FormKey(source.ModKey, 0x800);
        var bookKey = new FormKey(source.ModKey, 0x801);
        ...
        result.Warnings.ShouldHaveSingleItem().Code.ShouldBe("missing-plugin-reference");
    ...
    /// <summary>Creates shared plugin sources from generated in-memory Starfield mods.</summary>
    /// <param name="mods">The plugin mods in explicit load-order order.</param>
    /// <returns>A shared reader that borrows the supplied plugin mods.</returns>
    private static ReferenceReader CreateReader(params StarfieldMod[] mods)
    {
        return new ReferenceReader(mods
            .Select((mod, index) => new ReferenceSource(
                mod,
