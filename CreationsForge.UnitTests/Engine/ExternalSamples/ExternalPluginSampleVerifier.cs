...
/// <summary>Reopens retained output through direct game-plugin readers independently of the workspace lifecycle.</summary>
internal static class ExternalPluginSampleVerifier
{
    ...
    /// <param name="cancellationToken">A token observed before and between direct record checks.</param>
    ...
    /// <exception cref="InvalidDataException">Thrown when direct record values differ from the expected engine preview.</exception>
    internal static ExternalPluginVerificationResult Verify(
    ...
    /// <summary>Verifies direct Starfield plugin state.</summary>
    ...
    private static ExternalPluginVerificationResult VerifyStarfield(
    ...
    /// <summary>Verifies direct Fallout 4 plugin state.</summary>
    /// <param name="mod">The independently parsed output.</param>
    /// <param name="expectedRecords">Expected complete records.</param>
    /// <param name="intendedEdits">Independent exact EditorID and ordered Items expectations.</param>
    /// <param name="expectedMasters">Expected masters in exact order.</param>
    ...
    /// <param name="cancellationToken">A token observed between record checks.</param>
    /// <returns>Compact direct-reader evidence.</returns>
    private static ExternalPluginVerificationResult VerifyFallout4(
    ...
    /// <summary>Verifies direct Skyrim Special Edition plugin state.</summary>
    /// <param name="mod">The independently parsed output.</param>
    /// <param name="expectedRecords">Expected complete records.</param>
    /// <param name="intendedEdits">Independent exact EditorID and ordered Items expectations.</param>
    /// <param name="expectedMasters">Expected masters in exact order.</param>
    ...
    /// <param name="cancellationToken">A token observed between record checks.</param>
    /// <returns>Compact direct-reader evidence.</returns>
    private static ExternalPluginVerificationResult VerifySkyrim(
    ...
    /// <param name="record">The direct plugin major record.</param>
    /// <param name="formVersion">The game-specific FormVersion.</param>
    /// <param name="version2">The game-specific Version2.</param>
    /// <param name="items">Direct plugin item identities in order.</param>
    /// <param name="name">The optional direct record translated name.</param>
        ...
        JsonElement expected)
    {
        ...
        {
            throw new InvalidDataException($"Direct plugin Items differed for '{record.FormKey}'.");
        }
        ...
        if (expectedName.ValueKind == JsonValueKind.Null)
        {
            if (name is not null)
            {
                throw new InvalidDataException($"Direct plugin Name was present for '{record.FormKey}' but expected null.");
            }
        ...
        if (name is null)
        {
            throw new InvalidDataException($"Direct plugin Name was absent for '{record.FormKey}'.");
        }
        ...
        {
            throw new InvalidDataException($"Direct plugin Name translations differed for '{record.FormKey}'.");
        }
    }
        ...
        OutputAssociation association)
    {
        ...
        {
            throw new InvalidDataException($"Direct plugin output identity '{actualModKey}' differed from '{association.ModKey}'.");
        }

        if (!actualMasters.SequenceEqual(expectedMasters))
        {
            throw new InvalidDataException("Direct plugin output master identities or order differed from the expected source-derived set.");
        }
    }
    ...
    private static void VerifyRecordSet(IEnumerable<FormKey> actual, IEnumerable<FormKey> expected)
    {
        ...
        {
            throw new InvalidDataException("Direct plugin output FormList identities differed from the exact intended record set.");
        }
    }

    /// <summary>Requires equality for one directly inspected record field.</summary>
    ...
    /// <param name="formKey">The containing record identity.</param>
    ...
    private static void RequireEqual<T>(T actual, T expected, string field, FormKey formKey)
    {
        ...
        {
            throw new InvalidDataException($"Direct record field '{field}' differed for '{formKey}'.");
        }
    }
    ...
    private static ExternalPluginVerificationResult CreateResult(
        ModKey modKey,
    ...
    {
        var stringsDirectory = Path.Combine(Path.GetDirectoryName(association.PluginPath)!, "Strings");
        return new ExternalPluginVerificationResult
        {
...
/// <summary>Reports evidence obtained by the independent direct game-plugin reader.</summary>
internal sealed class ExternalPluginVerificationResult
{
    /// <summary>Initializes an empty serializable direct-verification result.</summary>
    internal ExternalPluginVerificationResult()
    { }
