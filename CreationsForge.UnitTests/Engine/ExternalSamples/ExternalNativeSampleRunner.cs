...
internal static class ExternalNativeSampleRunner
{
    ...
    /// <summary>Runs every explicitly requested output representation without overlapping native source lifetimes.</summary>
    ...
    internal static async Task RunAsync(ExternalNativeSampleManifest manifest)
    {
        ArgumentNullException.ThrowIfNull(manifest);
        var cancellationToken = TestContext.Current.CancellationToken;
        var preflightBefore = ExternalNativeSamplePreflight.Inspect(manifest);
        var runDirectory = ExternalNativeSamplePreflight.CreateRunDirectory(manifest, "execution");
        var report = new ExternalNativeSampleExecutionReport
        {
            SchemaVersion = 1,
        ...
        try
        {
            ...
            report.PreflightAfter = ExternalNativeSamplePreflight.Inspect(manifest);
    ...
    /// <summary>Runs one embedded or localized output case through initial save, fresh engine reopen, and direct native verification.</summary>
    ...
    private static async Task<ExternalNativeSampleCaseReport> RunCaseAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var caseReport = new ExternalNativeSampleCaseReport
        {
        ...
        await using (var services = NativeEngineComposition.Create())
        {
            var open = await services.WorkspaceFactory.OpenAsync(
                ...
                cancellationToken);
            var workspace = RequireSuccess(open, "opening external native sources");
            await using (workspace)
            {
                ...
                if (mode == LocalizedOutputMode.SeparateStringFiles)
                {
                    ...
                    if (!committed.CommittedBaseline.Artifacts.Any(
                        artifact => artifact.Role != NativeArtifactRole.Plugin && artifact.Fingerprint.Exists))
                    {
                        throw new InvalidDataException("The localized first save did not retain any present native string sidecar.");
                    }
                }
                ...
                foreach (var expected in expectedAfterPreview)
                {
                    ...
                    ExternalNativeSampleAssertions.RequireInspectorValues(
                        saved,
        ...
        caseReport.LocalizedNameEvidence = ExternalNativeSampleLocalizationVerifier.Verify(
            manifest, preflight, intendedEdits.Values.Select(edit => edit.Source).ToArray(), acceptanceGaps, cancellationToken);
        await using (var freshServices = NativeEngineComposition.Create())
        {
        ...
        caseReport.DirectVerification = ExternalNativeSampleVerifier.Verify(
            manifest,
            preflight,
            association,
            expectedRecords,
    ...
    /// <param name="workspace">The opened native workspace.</param>
    /// <param name="formKey">The exact requested FormKey.</param>
    /// <param name="cancellationToken">A token observed by the native inspector.</param>
    ...
    /// <param name="workspace">The opened native workspace.</param>
    /// <param name="items">Ordered native FormList items.</param>
    ...
    /// <summary>Begins one native edit session through the public workspace contract.</summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidDataException($"Guarded native save did not commit: {DescribeSave(result)}");
        }
    ...
    /// <param name="cancellationToken">A token observed during native traversal.</param>
        ...
        IReadOnlyList<ExternalIntendedRecordEdit> intendedRecords)
    {
        foreach (var intended in intendedRecords)
        {
            ...
            {
                throw new InvalidDataException($"External override comparison for '{source.FormKey}' lacked complete before/after native state.");
            }
    ...
    /// <returns>Ordered native FormKeys including duplicates and null sentinels.</returns>
    ...
    /// <summary>Creates a deterministic non-empty edited EditorID within conservative native length bounds.</summary>
    ...
    /// <summary>Recursively collects native FormKey JSON leaves from one inspector subtree.</summary>
    ...
    private static void VerifySourceEvidence(ExternalNativeSampleExecutionReport report)
    {
    ...
    /// <param name="before">Metadata captured before native parsing.</param>
    /// <param name="after">Metadata captured after all native lifetimes were released.</param>
    /// <returns><see langword="true"/> when every observation is identical.</returns>
    private static bool MetadataMatches(
        IReadOnlyList<ExternalNativeFileMetadata> before,
        IReadOnlyList<ExternalNativeFileMetadata> after)
    {
    ...
    /// <summary>Requires exact JSON text emitted by the deterministic native inspector.</summary>
    ...
    private static void RequireJsonEqual(string expected, string actual, string context)
    {
        ...
        {
            throw new InvalidDataException($"Complete native inspector JSON differed for {context}.");
        }
    }
