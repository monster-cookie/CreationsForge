...
/// <summary>Proves selected source names against explicit loose or archive-backed record string tables.</summary>
internal static class ExternalPluginSampleLocalizationVerifier
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (manifest.SupportedGame == SupportedGame.Skyrim)
        {
            ...
            {
                acceptanceGaps.Add($"Selected Skyrim FormList '{selected.FormKey}' has no game-plugin Name field; localized-name table coverage is not applicable.");
            }
    ...
    /// <param name="cancellationToken">A token observed during plugin and table reads.</param>
    ...
    /// <summary>Verifies selected records against their direct plugin names and explicit physical tables.</summary>
    /// <typeparam name="TRecord">The game-specific FormList getter.</typeparam>
    ...
    private static void VerifyNamedRecords<TRecord>(
        ...
        ExternalPluginSampleManifest manifest,
        ExternalPluginSamplePreflightReport preflight,
        PluginStringsFolderLookup lookup,
        IReadOnlyList<ExternalSelectedRecord> selectedRecords,
        ICollection<ExternalLocalizedNameEvidence> evidence,
        ICollection<string> acceptanceGaps,
        CancellationToken cancellationToken)
    {
        foreach (var selected in selectedRecords)
        {
            ...
            foreach (var claimedTranslation in translations)
            {
                ...
                if (!name.TryLookup(language, out var recordText)
                    || !string.Equals(recordText, claimedText, StringComparison.Ordinal))
                {
        ...
        CancellationToken cancellationToken)
    {
        var format = GameConstants.Get(manifest.GameRelease).StringsLanguageFormat
            ?? throw new InvalidDataException($"Release '{manifest.GameRelease}' does not define record strings file names.");
    ...
    private static PluginStringsFolderLookup CreateStringsLookup(ExternalPluginSampleManifest manifest, ModKey modKey)
    {
        ...
        return new PluginStringsFolderLookup(lookups);
    }
