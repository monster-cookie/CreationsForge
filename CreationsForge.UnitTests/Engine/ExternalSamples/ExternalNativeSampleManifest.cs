...
/// <summary>Loads and validates the process-local manifest for one opt-in external native sample.</summary>
internal sealed class ExternalNativeSampleManifest
{
    ...
    public ExternalNativeSampleManifest()
    { }
    ...
    /// <summary>Gets the validated native release.</summary>
    [JsonIgnore]
    ...
    internal static ExternalNativeSampleManifest? LoadFromEnvironment()
    {
    ...
    internal static ExternalNativeSampleManifest LoadFromPath(string manifestPath)
    {
        ...
        {
            throw new InvalidDataException($"The supplied external native sample manifest path is invalid: {exception.Message}", exception);
        }

        if (!File.Exists(canonicalManifestPath))
        {
            throw new InvalidDataException($"The supplied external native sample manifest does not exist: '{canonicalManifestPath}'.");
        }

        try
        {
            using var stream = new FileStream(canonicalManifestPath, FileMode.Open, FileAccess.Read, FileShare.Read);
            var manifest = JsonSerializer.Deserialize<ExternalNativeSampleManifest>(stream, SerializerOptions)
                ?? throw new InvalidDataException("The supplied external native sample manifest contained JSON null.");
            manifest.Validate();
            return manifest;
        }
        catch (JsonException exception)
        {
            throw new InvalidDataException($"The supplied external native sample manifest is not valid closed-schema JSON: {exception.Message}", exception);
        }
    }
    ...
    private void Validate()
    {
        ...
        {
            throw new InvalidDataException($"External native sample schemaVersion must be {CurrentSchemaVersion}.");
        }
        ...
        {
            throw new InvalidDataException("loadOrderPluginPaths must contain the complete explicit native load order.");
        }
        ...
        {
            RejectReparseAncestry(sourcePath, "native source input");
        }
        ...
        {
            throw new InvalidDataException("selectedFormKeys cannot contain duplicate native identities.");
        }
        ...
        {
            throw new InvalidDataException("outputPluginFileName must be one native file name without directory components.");
        }

        if (!ModKey.TryFromNameAndExtension(OutputPluginFileName, out var outputModKey, out var modKeyError))
        {
            throw new InvalidDataException($"outputPluginFileName is not a valid native identity: {modKeyError}");
        }
    ...
    /// <summary>Parses one exact non-null native FormKey and rejects sentinel identities.</summary>
    /// <param name="value">The manifest FormKey text.</param>
    /// <returns>The parsed real native FormKey.</returns>
    ...
    private static FormKey ParseFormKey(string value)
    {
        ...
        {
            throw new InvalidDataException($"selectedFormKeys contains invalid native identity '{value}': {exception.Message}", exception);
        }
    }
    ...
    /// <param name="pluginPaths">Canonical native plugin paths.</param>
        ...
        IReadOnlyList<string> stringDirectoryPaths)
    {
        ...
        foreach (var sourceLocation in sourceLocations)
        {
            ...
            {
                throw new InvalidDataException($"taskOutputRoot overlaps native source directory '{sourceLocation}'.");
            }
        }
        ...
        foreach (var stringDirectoryPath in stringDirectoryPaths)
        {
            ...
            {
                throw new InvalidDataException($"taskOutputRoot overlaps nonempty or non-dedicated native strings directory '{stringDirectoryPath}'.");
            }
        }
