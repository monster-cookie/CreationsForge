...
/// </summary>
public sealed class SkyrimNativeOutputServiceTests
{
    /// <summary>
    /// Verifies new full and small .esm outputs have exact native flags and create no destination artifacts.
    /// </summary>
    /// <param name="masterStyle">The requested Skyrim-supported native master style.</param>
    /// <param name="expectedSmallMaster">Whether the in-memory native header must carry the small-master flag.</param>
    /// <returns>A task that completes after every native lifetime is released.</returns>
    [Theory]
        ...
        bool expectedSmallMaster)
    {
        ...
        try
        {
            ...
            var state = result.Value!.Output.ShouldBeOfType<SkyrimNativeOutputState>();
            try
            {
                ...
                state.Baseline.Artifacts.Single(artifact => artifact.Role == NativeArtifactRole.Plugin)
    ...
    public async Task OpenAndReopenAsync_PreserveCompleteExistingOutputAndExactBaseline()
    {
        ...
        try
        {
            ...
            var state = open.Value!.Output.ShouldBeOfType<SkyrimNativeOutputState>();
            try
            {
                var snapshot = state.CreateSnapshot(TestContext.Current.CancellationToken);
                ...
                var reopenedState = reopen.Value!.Output.ShouldBeOfType<SkyrimNativeOutputState>();
                try
                {
