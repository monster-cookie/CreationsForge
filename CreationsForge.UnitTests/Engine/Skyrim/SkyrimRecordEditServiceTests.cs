...
public sealed class SkyrimRecordEditServiceTests
{
    ...
    public async Task ApplyEdit_AllSupportedFields_PreservesTypedValuesAndOrder()
    {
        ...
        var editService = new SkyrimRecordEditService(outputService.Inspector);
    ...
    [Fact]
    public void PrepareEdit_CanonicalizesPluginIdentityAndHashesRejectedPayloads()
    {
        var service = new SkyrimRecordEditService(new SkyrimFormListInspector());
    ...
    /// <returns>The successful immediate plugin mutation result.</returns>
    private static RecordEditMutationResult Apply(
        SkyrimRecordEditService service,
        SkyrimPluginSourceSet sources,
        SkyrimPluginOutputState state,
        FormKey target,
        FormListEdit edit)
    {
    ...
    /// <param name="sources">The immutable plugin sources.</param>
    /// <param name="state">The unpublished output candidate.</param>
    /// <returns>The allocated record identity.</returns>
    private static FormKey BeginNew(
        SkyrimPluginOutputService service,
        SkyrimPluginSourceSet sources,
        SkyrimPluginOutputState state)
    {
    ...
    /// <summary>Opens generated Skyrim sources through the production plugin loader.</summary>
    ...
    private static async Task<SkyrimPluginSourceSet> OpenSourcesAsync(
        SkyrimPluginOutputTestFixture fixture)
    {
        var open = await new SkyrimPluginSourceLoader(new PluginSourceInputLoader()).OpenAsync(
            fixture.Sources.CreateOpenRequest(),
            TestContext.Current.CancellationToken);
        open.Succeeded.ShouldBeTrue(open.Error?.Message);
        return open.Value!.Sources.ShouldBeOfType<SkyrimPluginSourceSet>();
    }

    /// <summary>Creates and selects one absent plugin Skyrim output without writing it.</summary>
    /// <param name="fixture">The generated complete source and output fixture.</param>
    /// <param name="sources">The immutable plugin sources.</param>
    /// <param name="service">The plugin output service.</param>
    /// <param name="fileName">The absent output file name.</param>
    /// <returns>The independently owned empty output state.</returns>
    private static async Task<SkyrimPluginOutputState> CreateNewStateAsync(
        SkyrimPluginOutputTestFixture fixture,
        SkyrimPluginSourceSet sources,
        SkyrimPluginOutputService service,
        string fileName)
    ...
    private static async Task<SkyrimPluginOutputState> OpenExistingStateAsync(
        SkyrimPluginOutputTestFixture fixture,
        SkyrimPluginSourceSet sources,
        SkyrimPluginOutputService service)
    {
    ...
    /// <summary>Creates the production Skyrim output service with shared plugin input admission.</summary>
    /// <returns>A new output service instance.</returns>
    private static SkyrimPluginOutputService CreateOutputService()
    {
        return new SkyrimPluginOutputService(new PluginOutputInputLoader());
    }
