...
/// <summary>
/// Verifies Skyrim Special Edition's explicit plugin source acquisition, baseline, and owned-lifetime behavior.
/// </summary>
public sealed class SkyrimPluginSourceLoaderTests
{
    /// <summary>
    /// Verifies full and small plugin identities, localized strings, references, and exact source-byte preservation.
    /// </summary>
    ...
    public async Task OpenAsync_LoadsFullAndSmallSourcesAndPreservesArtifacts()
    {
        ...
        var sources = openResult.Sources.ShouldBeOfType<SkyrimPluginSourceSet>();
        try
        {
            ...
            var smallRecord = smallResolution.Record
                ?? throw new InvalidOperationException("A resolved source FormList did not contain a detached record.");
            ...
            var bookRecord = bookResolution.Record
                ?? throw new InvalidOperationException("A resolved source Book did not contain a detached record.");
            ...
            var detachedName = localizedName as TranslatedString
                ?? throw new InvalidOperationException("The detached Book Name did not retain its record translated-string type.");
    ...
    /// <summary>
    /// Verifies a plugin whose required plugin master is absent fails before a source lifetime is published.
    /// </summary>
    ...
    /// </summary>
    [Fact]
    public void PluginModel_RejectsMediumMasterStyle()
    {
    ...
    /// <summary>
    /// Confirms the parsed source retains its external string-table key before the detached plugin copy intentionally normalizes that writer-owned metadata.
    /// </summary>
    /// <param name="fixture">The generated localized Skyrim fixture.</param>
    /// <returns>A task that completes after the independently prepared plugin input lifetime is released.</returns>
    private static async Task VerifyParsedSourceLocalizationAsync(SkyrimPluginTestFixture fixture)
    {
        var inputResult = await new PluginSourceInputLoader().PrepareAsync(
        ...
        try
        {
            ...
            var sourceName = sourceBook.Name as TranslatedString
                ?? throw new InvalidOperationException("The parsed source Book did not retain its record translated Name.");
