...
public sealed class StarfieldRecordEditPreviewTests
{
    /// <summary>Verifies exact chosen source contexts remain the before-view after edit reuse and candidate cloning.</summary>
    /// <returns>A task that completes after all plugin state is released.</returns>
    [Fact]
    public async Task Preview_PreservesFirstExactSourceAndAbsentBaselinesAcrossReuseAndClone()
    {
        ...
        var outputService = new StarfieldPluginOutputService(new PluginOutputInputLoader());
        ...
        preview.Value.Warnings.ShouldContain(warning => warning.Code == "missing-plugin-reference");
    ...
    public async Task Preview_UsesOriginalOutputBaselinesAndFreshReopenClearsProvenance()
    {
        ...
        await using var output = open.Value!.Output.ShouldBeOfType<StarfieldPluginOutputState>();
        await using var candidate = outputService.Clone(output);
        ...
        live.Warnings.ShouldContain(warning => warning.Code == "missing-plugin-reference");
        ...
        await using var reopened = reopen.Value!.Output.ShouldBeOfType<StarfieldPluginOutputState>();
    ...
    /// <summary>Verifies replacement validation subtracts retained unresolved multiplicities and move changed-state follows plugin semantics.</summary>
    /// <returns>A task that completes after the plugin fixture and candidate are released.</returns>
    [Fact]
    ...
    /// <summary>Begins one staged edit through the exact requested plugin role.</summary>
    ...
    private static FormKey Begin(
        ...
        StarfieldPluginOutputService service,
    ...
    private static void Apply(
        StarfieldRecordEditService service,
        StarfieldPluginSourceSet sources,
        StarfieldPluginOutputState candidate,
        FormKey target,
    ...
    /// <summary>Writes one deterministic complete existing plugin output fixture.</summary>
    /// <param name="directory">The existing task-owned parent directory.</param>
    /// <param name="fileName">The output plugin file name.</param>
    /// <param name="configure">The typed record builder.</param>
    ...
    /// <param name="fixture">The generated plugin test fixture.</param>
    ...
    {
        var result = await new StarfieldPluginSourceLoader(new PluginSourceInputLoader()).OpenAsync(
    ...
    private static async Task<StarfieldPluginOutputState> OpenNewOutputAsync(
        ...
        string fileName)
    {
        ...
        var result = await service.OpenAsync(
            ...
            TestContext.Current.CancellationToken);
        result.Succeeded.ShouldBeTrue(result.Error?.Message);
        return result.Value!.Output.ShouldBeOfType<StarfieldPluginOutputState>();
    }
}
