...
/// <summary>Exercises representative nondefault values across every generated recursive plugin leaf family.</summary>
public sealed class GeneratedRecordWireLeafRoundTripTests
{
    ...
    public void PrimitiveCollectionAndMemoryLeaves_RoundTripExactly()
    {
        ...
        root["UnknownEnding"] = GeneratedRecordWireTestSupport.Bytes(new byte[] { 0, 1, 2, 254, 255 });

        AssertExactRoundTrip(recordTypeName, root);
    }
    ...
    public void CompositeLeaves_RoundTripExactly()
    {
        ...
        doubles["Longitude"] = GeneratedRecordWireTestSupport.Double(-123.456789012345);
        doubles["Latitude"] = GeneratedRecordWireTestSupport.Double(BitConverter.UInt64BitsToDouble(1UL << 63));
        ...
        firstItem["TerrainHeight"] = GeneratedRecordWireTestSupport.Single(-0.0f);
        firstItem["WaterHeight"] = GeneratedRecordWireTestSupport.Single(17.5f);
        var secondItem = GetDefaultObject(itemType);
        secondItem["TerrainHeight"] = GeneratedRecordWireTestSupport.Single(float.NegativeInfinity);
        secondItem["WaterHeight"] = GeneratedRecordWireTestSupport.Single(float.NaN);
        ...
        array["DATA"] = GeneratedRecordWireTestSupport.Bytes(new byte[] { 3, 1, 4, 1, 5 });
        AssertExactRoundTrip(arrayType, array);
    }

    /// <summary>Verifies a nondefault typed asset path and nullable byte memory survive plugin construction.</summary>
    [Fact]
    public void AssetLinkAndNullableMemory_RoundTripExactly()
    {
        ...
        var written = GeneratedRecordWireTestSupport.Write(recordTypeName, model);

        GeneratedRecordWireTestSupport.RoundTrip(recordTypeName, written).ShouldBe(written);

        var compact = GeneratedRecordWireTestSupport.ParseObject(written);
        ...
        GeneratedRecordWireTestSupport.RoundTrip(recordTypeName, compact.ToJsonString()).ShouldBe(written);
    }

    /// <summary>Verifies an empty record asset path expands omitted read-only projections without losing its null state.</summary>
    [Fact]
    ...
    private static void AssertExactRoundTrip(string recordTypeName, JsonObject root)
    {
        var json = root.ToJsonString();
        GeneratedRecordWireTestSupport.RoundTrip(recordTypeName, json).ShouldBe(json);
    }

    /// <summary>Creates the complete reconstructible wire representation of one plugin color.</summary>
    /// <param name="value">The plugin color.</param>
    ...
    private static JsonObject P2Float(float x, float y)
    {
        return new JsonObject
        {
            ["X"] = GeneratedRecordWireTestSupport.Single(x),
            ["Y"] = GeneratedRecordWireTestSupport.Single(y),
        };
    }
    ...
    private static JsonObject P3Float(float x, float y, float z)
    {
        return new JsonObject
        {
            ["X"] = GeneratedRecordWireTestSupport.Single(x),
            ["Y"] = GeneratedRecordWireTestSupport.Single(y),
            ["Z"] = GeneratedRecordWireTestSupport.Single(z),
        };
    }
