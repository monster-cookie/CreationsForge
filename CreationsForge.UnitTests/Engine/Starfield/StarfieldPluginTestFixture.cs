...
/// <summary>
/// Owns a disposable generated Starfield source/load-order fixture with mixed plugin master styles.
/// </summary>
internal sealed class StarfieldPluginTestFixture : IDisposable
{
    ...
    /// <summary>
    /// Initializes a generated fixture and records its plugin identities.
    /// </summary>
    ...
    /// <param name="warningListFormKey">The source FormList identity containing an unresolved record link.</param>
    /// <param name="missingFormKey">The unresolved record link target used for read warnings.</param>
    private StarfieldPluginTestFixture(
    ...
    /// <summary>Gets the source FormList identity containing an unresolved record link.</summary>
    public FormKey WarningListFormKey { get; }

    /// <summary>Gets the unresolved record link target used for contextual-read warnings.</summary>
    public FormKey MissingFormKey { get; }

    /// <summary>
    /// Creates a generated fixture without consulting installed game or load-order state.
    /// </summary>
    /// <returns>A fixture containing full, small, medium, localized, referenced, and overriding records.</returns>
    public static StarfieldPluginTestFixture Create()
    {
        var rootDirectory = Directory.CreateTempSubdirectory("CreationsForge-StarfieldPlugin-");
        var dataDirectory = rootDirectory.CreateSubdirectory("Data");
        var stringsDirectory = rootDirectory.CreateSubdirectory("Strings");

        var source = new StarfieldMod("PluginSmall.esm", StarfieldRelease.Starfield)
        {
            IsSmallMaster = true,
            UsingLocalization = true,
        };
        var full = new StarfieldMod("PluginFull.esm", StarfieldRelease.Starfield);
        var medium = new StarfieldMod("PluginMedium.esm", StarfieldRelease.Starfield)
        {
            IsMediumMaster = true,
        };
        var patch = new StarfieldMod("PluginPatch.esp", StarfieldRelease.Starfield);
        ...
        return new StarfieldPluginTestFixture(
            rootDirectory,
            dataDirectory,
            stringsDirectory,
            orderedPluginPaths,
    ...
    /// <summary>
    /// Creates a two-language record translated string for localized source verification.
    /// </summary>
    /// <param name="english">The default English text.</param>
    /// <param name="french">The French translation.</param>
    /// <returns>A mutable record translated string containing both languages.</returns>
