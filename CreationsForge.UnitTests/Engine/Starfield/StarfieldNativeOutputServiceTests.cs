...
/// </summary>
public sealed class StarfieldNativeOutputServiceTests
{
    /// <summary>Verifies every supported Starfield master style for both absent and existing output selection.</summary>
    /// <param name="style">The native master style to create and reopen.</param>
    ...
    public async Task OpenAsync_SupportsAllMasterStylesForNewAndExistingOutputs(OutputMasterStyle style)
    {
        ...
        var service = new StarfieldNativeOutputService(new NativeOutputInputLoader());
        service.Inspector.ShouldBeOfType<StarfieldFormListNativeInspector>();
        ...
        await using (var newState = newResult.Value!.Output.ShouldBeOfType<StarfieldNativeOutputState>())
        {
        ...
        await using var existingState = existingResult.Value!.Output.ShouldBeOfType<StarfieldNativeOutputState>();
    ...
    public async Task ExistingOutput_PreservesHeadersTranslationsOrderConditionsAndDefensiveSnapshots()
    {
        ...
        await using var state = open.Value!.Output.ShouldBeOfType<StarfieldNativeOutputState>();
    ...
    public async Task BeginEdit_AllocatesUniqueIdsAndUsesExactLiveOverrideContexts()
    {
        ...
        await using var collisionState = collisionOpen.Value!.Output.ShouldBeOfType<StarfieldNativeOutputState>();
        ...
        await using var overrideState = overrideOpen.Value!.Output.ShouldBeOfType<StarfieldNativeOutputState>();
    ...
    public async Task ReopenAndLifecycle_RejectDriftCancellationAndDisposedState()
    {
        ...
        var open = await service.OpenAsync(
            ...
            TestContext.Current.CancellationToken);
        open.Succeeded.ShouldBeTrue(open.Error?.Message);
        var state = open.Value!.Output.ShouldBeOfType<StarfieldNativeOutputState>();
        ...
        await using var reopenedState = reopened.Value!.Output.ShouldBeOfType<StarfieldNativeOutputState>();
    ...
    {
        var open = await new StarfieldNativeSourceLoader(new NativeSourceInputLoader()).OpenAsync(
            fixture.CreateOpenRequest(),
            TestContext.Current.CancellationToken);
        open.Succeeded.ShouldBeTrue(open.Error?.Message);
        return open.Value!.Sources.ShouldBeOfType<StarfieldNativeSourceSet>();
    }
    ...
    /// <param name="localizedOutputMode">The requested native localization representation.</param>
    ...
    /// <summary>Writes a deterministic complete native output fixture with caller-selected content.</summary>
    ...
    /// <summary>Asserts exact native style flags without conflating full, small, and medium outputs.</summary>
    /// <param name="mod">The detached complete Starfield output.</param>
    /// <param name="style">The expected native style.</param>
    ...
    /// <summary>Creates a deterministic two-language native translated string.</summary>
