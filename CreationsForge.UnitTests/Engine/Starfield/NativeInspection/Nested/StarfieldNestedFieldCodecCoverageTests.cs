...
{
    /// <summary>The logical name assigned to the checked-in generated native field manifest.</summary>
    private const string ManifestResourceName = "CreationsForge.Starfield.NativeFieldManifest.json";

    /// <summary>The package-scoped prefix required on every generated concrete native type discriminator.</summary>
    private const string NativeTypePrefix = "Mutagen.Bethesda.Starfield/0.55.0-alpha.48:";
    ...
    public void Manifest_CoversInstalledRootsAndIndexedFields()
    {
        ...
        var manifestedRoots = entries
            ...
            .Select(entry => entry.GetProperty("nativeType").GetString()!)
            .OrderBy(name => name, StringComparer.Ordinal)
            .ToArray();

        manifestedRoots.ShouldBe(installedRoots);
        entries.Select(entry => entry.GetProperty("nativeType").GetString())
        ...
        {
            var nativeTypeName = GetRequiredString(entry, "nativeType");
            var nativeType = starfieldAssembly.GetType(nativeTypeName, throwOnError: true)!;
            ...
            getterType.IsAssignableFrom(nativeType).ShouldBeTrue($"{nativeTypeName} must implement {getterTypeName}.");

            var fieldIndex = starfieldAssembly.GetType(nativeTypeName + "_FieldIndex", throwOnError: true)!;
            ...
            manifestFields.Select(field => GetRequiredString(field, "Name")).ShouldBe(
                installedFields,
                $"{nativeTypeName} manifest fields must match installed FieldIndex order.");
            ...
            foreach (var field in manifestFields)
            {
                ...
                field.GetProperty("Ordinal").GetUInt16().ShouldBe(
                    ...
                    $"{nativeTypeName}.{name} must retain its installed FieldIndex ordinal.");
            }
        }
    }
    ...
    public void GeneratedVisitors_WriteEveryManifestedKeyAndCompareEveryConcreteType()
    {
        ...
        foreach (var entry in manifest.RootElement.GetProperty("types").EnumerateArray())
        {
            ...
            var nativeType = starfieldAssembly.GetType(nativeTypeName, throwOnError: true)!;
            var getterType = starfieldAssembly.GetType(getterTypeName, throwOnError: true)!;
            var value = CreateNativeValue(nativeType);
            var writeMethod = GetRequiredMethod(
                codecType,
                "Write" + nativeType.Name,
                typeof(Utf8JsonWriter),
                getterType,
                typeof(CancellationToken),
                typeof(NativeJsonWriteContext));

            using var written = InvokeWriter(writeMethod, value, nativeTypeName, null);
            var canonicalContext = new NativeJsonWriteContext(NativeJsonWriteMode.CanonicalFingerprintV1);
            using var canonicalWritten = InvokeWriter(writeMethod, value, nativeTypeName, canonicalContext);
            ...
            written.RootElement.EnumerateObject().Select(property => property.Name).ShouldBe(
                expectedKeys,
                $"{nativeTypeName} must emit each generated native field once and in contract order.");
            written.RootElement.GetProperty("$type").GetString().ShouldBe(NativeTypePrefix + nativeTypeName);
            canonicalWritten.RootElement.EnumerateObject().Select(property => property.Name).ShouldBe(
                expectedKeys,
                $"{nativeTypeName} must retain its generated contract in canonical mode.");
            canonicalContext.ValidationError.ShouldBeNull(
                $"A default {nativeTypeName} must not produce a canonical validation diagnostic.");

            var compareMethod = GetRequiredMethod(
                codecType,
                "Compare" + nativeType.Name,
                getterType,
                getterType,
                typeof(string),
            ...
            changes.ShouldBeEmpty($"{nativeTypeName} must compare equal to the same native instance.");
        }
    }
    ...
    {
        return typeof(StarfieldFormListNativeInspector).Assembly.GetType(
    ...
    /// <summary>Constructs one mutable native value through its generated parameterless constructor.</summary>
    /// <param name="nativeType">The concrete native mutable type.</param>
    /// <returns>A native value assignable to its generated getter interface.</returns>
    private static object CreateNativeValue(Type nativeType)
    {
        try
        {
            return System.Activator.CreateInstance(nativeType, nonPublic: true)
                ?? throw new InvalidOperationException($"Constructor for {nativeType.FullName} returned null.");
        }
        catch (Exception exception)
        {
            throw new InvalidOperationException($"Could not construct generated native type {nativeType.FullName}.", exception);
        }
    }
    ...
    /// <param name="value">The concrete native getter value.</param>
    /// <param name="nativeTypeName">The native type identity used in failures.</param>
        ...
        NativeJsonWriteContext? context)
    {
        ...
        {
            Invoke(writeMethod, nativeTypeName, writer, value, TestContext.Current.CancellationToken, context);
        }

        return JsonDocument.Parse(stream.ToArray());
    }

    /// <summary>Invokes a generated method while adding native type context to reflection failures.</summary>
    /// <param name="method">The generated method to invoke.</param>
    /// <param name="nativeTypeName">The native type identity used in failures.</param>
    /// <param name="arguments">The generated method arguments.</param>
    private static void Invoke(MethodInfo method, string nativeTypeName, params object?[] arguments)
    {
        try
        {
        ...
        catch (TargetInvocationException exception)
        {
            throw new InvalidOperationException(
                $"Generated method {method.Name} failed for {nativeTypeName}.",
