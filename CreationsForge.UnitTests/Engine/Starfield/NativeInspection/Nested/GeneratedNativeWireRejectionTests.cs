...
public sealed class GeneratedNativeWireRejectionTests
{
    ...
    {
        const string nativeTypeName = "Mutagen.Bethesda.Starfield.BiomeHasKeywordConditionData";
        ...
        var rewritten = GeneratedNativeWireTestSupport.RoundTrip(nativeTypeName, root.ToJsonString());

        var rewrittenRoot = GeneratedNativeWireTestSupport.ParseObject(rewritten);
    ...
    /// <param name="mismatchedValue">The value that contradicts the native owner flags.</param>
    [Theory]
        ...
        bool mismatchedValue)
    {
        ...
        AssertInvalidRequestAtPath(nativeTypeName, root, $"$.FirstParameter.{propertyName}");
    }

    /// <summary>Verifies omitted derived condition and owner-mode fields expand from the constructed native owner.</summary>
    [Fact]
    public void DerivedConditionAndOwnerFields_WhenOmitted_ExpandFromNativeState()
    {
        const string nativeTypeName = "Mutagen.Bethesda.Starfield.BiomeHasKeywordConditionData";
        var root = GetDefaultObject(nativeTypeName);
        ...
        var rewritten = GeneratedNativeWireTestSupport.ParseObject(
            GeneratedNativeWireTestSupport.RoundTrip(nativeTypeName, root.ToJsonString()));
    ...
    /// <summary>Verifies zero-length Array2d axes remain valid native shapes without weakening per-axis bounds.</summary>
    ...
    {
        const string nativeTypeName = "Mutagen.Bethesda.Starfield.BlockHeightAdjustmentComponent";
        var root = GetDefaultObject(nativeTypeName);
        ...
        GeneratedNativeWireTestSupport.RoundTrip(nativeTypeName, root.ToJsonString()).ShouldBe(root.ToJsonString());
    }
    ...
    public void Array2d_WithOversizedWidthAndZeroHeight_ReturnsInvalidRequestBeforeAllocation()
    {
        ...
        AssertInvalidRequest(
            nativeTypeName,
            root,
            $"Array length {int.MaxValue} exceeds the per-array limit of {NativeWireReadLimits.DefaultMaximumArrayElements}",
    ...
    public void ConditionData_WithMismatchedDerivedFunction_ReturnsInvalidRequest()
    {
        ...
        AssertInvalidRequestAtPath(nativeTypeName, root, "$.Function", "Derived Function");
    }
    ...
    public void ConcreteObject_WithWrongTypeOrUnknownProperty_ReturnsInvalidRequest(bool replaceType)
    {
        ...
        AssertInvalidRequest(nativeTypeName, root, replaceType ? "Expected native type discriminator" : "not accepted");
    }

    /// <summary>Verifies the native null FormKey sentinel cannot masquerade as a non-null generated FormLink.</summary>
    [Fact]
    public void FormLink_WithNonNullWrapperAroundFormKeyNull_ReturnsInvalidRequest()
    {
        const string nativeTypeName = "Mutagen.Bethesda.Starfield.ObjectProperty";
        var root = GetDefaultObject(nativeTypeName);
        var actorValue = root["ActorValue"]!.AsObject();
        actorValue["isNull"] = false;
        actorValue["formKey"] = FormKey.Null.ToString();

        AssertInvalidRequest(nativeTypeName, root, "null state does not match");
    }

    /// <summary>Verifies nullable native links preserve both accepted null FormKey representations exactly.</summary>
    /// <param name="useJsonNull">Whether the accepted input uses JSON null instead of the explicit native sentinel.</param>
    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public void FormLink_WithAcceptedNullFormKey_RoundTripsExactly(bool useJsonNull)
    {
        const string nativeTypeName = "Mutagen.Bethesda.Starfield.AddToInventoryOnDestroyComponent";
        var root = GetDefaultObject(nativeTypeName);
        ...
        var rewrittenLink = GeneratedNativeWireTestSupport.ParseObject(rewritten)["Item"]!.AsObject();
    ...
    /// <summary>Verifies JSON null accepted for a nonnullable native link canonicalizes to its required FormKey.Null sentinel.</summary>
    [Fact]
    public void FormLink_WithAcceptedJsonNullOnNonNullableWrapper_CanonicalizesToNullSentinel()
    {
        ...
        var rewrittenLink = GeneratedNativeWireTestSupport.ParseObject(rewritten)["ActorValue"]!.AsObject();
        rewrittenLink["isNull"]!.GetValue<bool>().ShouldBeTrue();
        rewrittenLink["formKey"]!.GetValue<string>().ShouldBe(FormKey.Null.ToString());
    }
    ...
    /// <param name="nativeTypeName">The exact concrete native type containing the required polymorphic field.</param>
    ...
    public void RequiredPolymorphicField_WithNullOrMissingValue_ReturnsInvalidRequest(
        ...
        bool omitProperty)
    {
        var root = CreateRequiredPolymorphicRoot(nativeTypeName);
        if (omitProperty)
        {
            root.Remove(propertyName).ShouldBeTrue();
            AssertInvalidRequest(nativeTypeName, root, $"Required property '{propertyName}' is missing.");
            return;
        }

        root[propertyName] = null;
        AssertInvalidRequestAtPath(nativeTypeName, root, $"$.{propertyName}");
    }
    ...
    /// <param name="expectedPath">The exact reported native wire path.</param>
    [Theory]
    ...
    /// <summary>Verifies the required asset null-state flag must agree with an empty native path.</summary>
    [Fact]
    public void AssetLink_WithInconsistentNullState_ReturnsExactPropertyPath()
    {
        ...
        AssertInvalidRequestAtPath(nativeTypeName, root, "$.File.value.isNull");
    }
    ...
    public void AssetLink_WithNullGivenPath_ReturnsExactPropertyPath()
    {
        ...
        AssertInvalidRequestAtPath(nativeTypeName, root, "$.File.value.givenPath");
    }
    ...
    {
        const string nativeTypeName = "Mutagen.Bethesda.Starfield.Activity";
        var root = GetDefaultObject(nativeTypeName);
        ...
        AssertInvalidRequest(nativeTypeName, root, "target value");
    }
    ...
    {
        const string nativeTypeName = "Mutagen.Bethesda.Starfield.BlockEditorMetaDataComponent";
        var arrayRoot = GetDefaultObject(nativeTypeName);
        arrayRoot["UnknownInts"] = new JsonArray(1, 2);
        var arrayLimits = new NativeWireReadLimits(64, 4096, 1, 1024, 1024);
        AssertInvalidRequest(nativeTypeName, arrayRoot, "per-array limit", arrayLimits);

        var stringRoot = GetDefaultObject(nativeTypeName);
        stringRoot["UnknownString1"] = new string('x', 257);
        var stringLimits = new NativeWireReadLimits(64, 4096, 32, 256, 1024);
        AssertInvalidRequest(nativeTypeName, stringRoot, "string length 257 exceeds the limit of 256", stringLimits);

        var bytesRoot = GetDefaultObject(nativeTypeName);
        bytesRoot["UnknownEnding"] = GeneratedNativeWireTestSupport.Bytes(new byte[] { 0, 1, 2, 3 });
        var byteLimits = new NativeWireReadLimits(64, 4096, 32, 1024, 3);
        AssertInvalidRequest(nativeTypeName, bytesRoot, "between 0 and 3", byteLimits);

        var depthRoot = GetDefaultObject(nativeTypeName);
        var depthLimits = new NativeWireReadLimits(2, 4096, 32, 1024, 1024);
        AssertInvalidRequest(nativeTypeName, depthRoot, "Nesting depth", depthLimits);
    }
    ...
    /// <param name="path">The native asset path to write.</param>
    ...
    private static JsonObject CreateModelAssetRoot(string path)
    {
        ...
        return GeneratedNativeWireTestSupport.ParseObject(
            GeneratedNativeWireTestSupport.Write("Mutagen.Bethesda.Starfield.Model", model));
    }

    /// <summary>Creates one valid wire root whose required polymorphic field can be invalidated by a rejection test.</summary>
    /// <param name="nativeTypeName">The exact supported condition-root or volumes-item type name.</param>
    /// <returns>A complete valid mutable generated wire object.</returns>
    /// <exception cref="InvalidOperationException">Thrown when the requested native type is outside this focused fixture set.</exception>
    private static JsonObject CreateRequiredPolymorphicRoot(string nativeTypeName)
    {
        object nativeValue = nativeTypeName switch
        {
            ...
            _ => throw new InvalidOperationException($"Native type {nativeTypeName} has no required-polymorphic fixture."),
        };
        return GeneratedNativeWireTestSupport.ParseObject(
            GeneratedNativeWireTestSupport.Write(nativeTypeName, nativeValue));
    }
    ...
    {
        var result = GeneratedNativeWireTestSupport.Decode(nativeTypeName, root.ToJsonString(), limits);
    ...
    {
        var result = GeneratedNativeWireTestSupport.Decode(nativeTypeName, root.ToJsonString());
