...
public sealed class StarfieldRecordEditPreparationTests
{
    /// <summary>Verifies common and Starfield root commands preserve order, duplicates, null links, and typed plugin header behavior.</summary>
    /// <returns>A task that completes after all plugin test state is released.</returns>
    [Fact]
    ...
    /// <summary>Verifies canonical identities normalize record identity spelling while retaining ordinary text and malformed code units exactly.</summary>
    [Fact]
    public void PrepareEdit_CanonicalizesIdentityAndRetainsExactPayloadDifferences()
    {
        var service = new StarfieldRecordEditService();
    ...
    /// <summary>Applies one prepared edit and requires a successful plugin mutation result.</summary>
    ...
    /// <summary>Creates one representative nested component with a typed record link and exact floating value.</summary>
    /// <param name="actorValue">The record link identity retained by the component.</param>
