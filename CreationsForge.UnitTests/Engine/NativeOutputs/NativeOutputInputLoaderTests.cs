...
/// <summary>Verifies bounded native output admission independently of game-specific mutable output state.</summary>
public sealed class NativeOutputInputLoaderTests
{
    ...
    public async Task PrepareAsync_CreateNew_CapturesAbsenceWithoutCreatingOutputArtifacts()
    {
        ...
        var result = await new NativeOutputInputLoader().PrepareAsync(
            sources,
            association,
    ...
    public async Task PrepareAsync_EnforcesSelectionIntentAndExistingParentDirectory()
    {
        ...
        var createExisting = await new NativeOutputInputLoader().PrepareAsync(
            sources,
            existing,
            OutputSelectionMode.CreateNew,
            TestContext.Current.CancellationToken);
        ...
        var openAbsent = await new NativeOutputInputLoader().PrepareAsync(
            sources,
            absent,
        ...
        var missingDirectory = await new NativeOutputInputLoader().PrepareAsync(
            sources,
            missingParent,
            OutputSelectionMode.CreateNew,
            TestContext.Current.CancellationToken);
    ...
    /// <summary>An existing output whose native header disagrees with its requested association is rejected.</summary>
    /// <returns>A task that completes after native header validation.</returns>
    [Fact]
    ...
    /// <summary>Disposing the output parsing scope invalidates it without disposing the borrowed native sources.</summary>
    ...
    public async Task OutputScope_WithCancellationAndDisposal_DoesNotOwnBorrowedSources()
    {
        ...
        await Should.ThrowAsync<OperationCanceledException>(() => new NativeOutputInputLoader().PrepareAsync(
            sources,
            association,
            OutputSelectionMode.CreateNew,
    ...
    private static async Task<NativeSourceInputs> PrepareSourcesAsync(StarfieldNativeTestFixture fixture)
    {
