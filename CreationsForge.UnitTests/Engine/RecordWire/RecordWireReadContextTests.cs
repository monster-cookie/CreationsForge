...
namespace CreationsForge.UnitTests.Engine.RecordWire;

/// <summary>Verifies bounded strict plugin wire traversal and exact scalar reconstruction.</summary>
public sealed class RecordWireReadContextTests
{
    /// <summary>Defines the closed object shape used by structural tests.</summary>
    private static readonly RecordWireObjectShape ValueShape = new(
        "test value",
        new[] { "value" });
    ...
    public void Decode_WithNonClosedObject_ReturnsInvalidRequest(string json)
    {
        ...
        var result = RecordWireReadContext.Decode(
    ...
    public void Decode_WithExceededStructuralLimit_ReturnsInvalidRequest()
    {
        ...
        var depthLimits = new RecordWireReadLimits(1, 10, 10, 10, 10);
        var arrayLimits = new RecordWireReadLimits(10, 10, 2, 10, 10);

        var depthResult = RecordWireReadContext.Decode(
        ...
        var arrayResult = RecordWireReadContext.Decode(
    ...
    [InlineData(0)]
    [InlineData(17)]
    [InlineData(RecordWireReadLimits.DefaultMaximumArrayElements)]
    ...
    /// <summary>Verifies negative and over-limit rectangular dimensions fail before plugin array allocation.</summary>
    [Theory]
    [InlineData(-1)]
    [InlineData(RecordWireReadLimits.DefaultMaximumArrayElements + 1)]
    ...
    /// <summary>Verifies dimension validation observes cancellation after numeric decoding and before plugin allocation.</summary>
    [Fact]
    public void ValidateArrayLength_WhenCanceled_ThrowsCancellation()
    {
        ...
        Should.Throw<OperationCanceledException>(() => RecordWireReadContext.Decode(
            document.RootElement,
            RecordWireReadLimits.Default,
            cancellation.Token,
            (context, root) =>
    ...
    {
        const int count = RecordWireReadLimits.DefaultMaximumArrayElements;
    ...
    public void ReadBytes_WithSpecializedBound_PreservesCanonicalPayload()
    {
        ...
        var limits = new RecordWireReadLimits(10, 10, 10, 6, 4);
    ...
    {
        using var document = JsonDocument.Parse("{\"length\":4,\"base64\":\"AAECAw==\"}");
        var limits = new RecordWireReadLimits(10, 10, 10, 100, 3);
    ...
    [Fact]
    public void RecordWireSchemaNode_WithDisposedDocuments_RetainsDetachedSchemaAndTemplate()
    {
        var identity = new RecordWireSchemaCatalogIdentity(
            SupportedGame.Starfield,
            GameRelease.Starfield,
            "1",
            new string('a', 64));
        var key = new RecordWireSchemaNodeKey(
            identity.CatalogId,
            RecordWireSchemaNodeKind.Type,
            "Example.Type");
        RecordWireSchemaNode node;
        using (var schemaDocument = JsonDocument.Parse("{\"constructible\":true}"))
        using (var templateDocument = JsonDocument.Parse("{\"value\":0}"))
        {
            node = new RecordWireSchemaNode(
                key,
