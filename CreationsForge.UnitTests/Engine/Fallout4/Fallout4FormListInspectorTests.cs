...
public sealed class Fallout4FormListInspectorTests
{
    /// <summary>Verifies the response view writes every installed record field in exact generated index order.</summary>
    [Fact]
    ...
    public void Inspector_UsesTypedEqualityAndObservesCancellation()
    {
        ...
        var mod = new Fallout4Mod("PluginInspector.esm", Fallout4Release.Fallout4);
    ...
    /// <param name="formKey">The record identity to assign during construction, or <see langword="null"/> for the standard test identity.</param>
    /// <returns>A detached FormList containing localized text and ordered duplicate-capable links.</returns>
    private static FormList CreateCompleteFormList(FormKey? formKey = null)
    {
        var modKey = (ModKey)"PluginInspector.esm";
    ...
    /// <summary>Creates a two-language record translated string for response and comparison coverage.</summary>
    /// <param name="english">The selected English value.</param>
    /// <param name="french">The retained French translation.</param>
    /// <returns>A record translated string retaining both language entries.</returns>
