...
internal sealed class Fallout4NativeOutputTestFixture : IDisposable
{
    ...
    private Fallout4NativeOutputTestFixture(
        Fallout4NativeTestFixture sources,
    ...
    public Fallout4NativeTestFixture Sources { get; }
    ...
    public static Fallout4NativeOutputTestFixture Create()
    {
        var sources = Fallout4NativeTestFixture.Create();
        try
        {
            var outputDirectory = sources.RootDirectory.CreateSubdirectory("Output");
            return new Fallout4NativeOutputTestFixture(sources, outputDirectory);
        }
        catch
        {
            sources.Dispose();
    ...
    /// <param name="fileName">The native output plugin file name.</param>
    /// <param name="masterStyle">The full or small native master style.</param>
    ...
    /// <returns>The association and native identities needed for preservation and selection assertions.</returns>
    public (
        ...
        bool includeMissingReference = false)
    {
        ...
        masterFlags.Set(new KeyedMasterStyle(Sources.SourceModKey, NativeMasterStyle.Small));
        masterFlags.Set(new KeyedMasterStyle(Sources.FullModKey, NativeMasterStyle.Full));
        masterFlags.Set(new KeyedMasterStyle(Sources.PatchModKey, NativeMasterStyle.Full));
        masterFlags.Set(new KeyedMasterStyle(
            mod.ModKey,
            masterStyle == OutputMasterStyle.Small ? NativeMasterStyle.Small : NativeMasterStyle.Full));
