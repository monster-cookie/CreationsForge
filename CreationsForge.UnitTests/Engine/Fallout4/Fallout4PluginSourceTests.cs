...
/// <summary>
/// Verifies Fallout 4 plugin source loading and bounded reference operations through the production helper.
/// </summary>
public sealed class Fallout4PluginSourceTests
{
    /// <summary>Verifies full/small plugin loading, immutable baselining, and byte-preserving source ownership.</summary>
    ...
    public async Task OpenAsync_LoadsFullAndSmallSourcesWithoutChangingArtifacts()
    {
        ...
        await using var sources = result.Value.Sources.ShouldBeOfType<Fallout4PluginSourceSet>();
    ...
    /// <summary>Verifies stable bounded paging across FormList and non-FormList record families.</summary>
    ...
    public async Task Search_PagesCrossFamilyContextsAndRejectsAnotherWorkspaceToken()
    {
        ...
        await using var firstSources = firstOpen.Value!.Sources.ShouldBeOfType<Fallout4PluginSourceSet>();
        ...
        await using var secondSources = secondOpen.Value!.Sources.ShouldBeOfType<Fallout4PluginSourceSet>();
    ...
    /// <summary>Verifies that missing declared masters fail before a plugin source lifetime is returned.</summary>
    /// <returns>A task that completes after plugin input validation rejects the request.</returns>
    [Fact]
    ...
    /// <summary>Verifies the pinned Fallout 4 plugin model rejects medium-master state instead of emitting an ambiguous header.</summary>
    [Fact]
    public void PluginModel_WhenMediumMasterIsRequested_RejectsUnsupportedStyle()
    {
    ...
    /// <summary>Verifies two paths claiming the same plugin ModKey are rejected as ambiguous explicit input.</summary>
    /// <returns>A task that completes after plugin input validation rejects the duplicate identity.</returns>
    [Fact]
    ...
    /// <summary>Verifies cancellation is observed before both plugin acquisition and bounded reference scans.</summary>
    /// <returns>A task that completes after all cancellation paths are observed.</returns>
    [Fact]
    public async Task Operations_WhenCancelled_ThrowOperationCanceledException()
    {
        using var fixture = Fallout4PluginTestFixture.Create();
