...
public sealed class Fallout4RecordEditServiceTests
{
    /// <summary>Every supported typed field edit mutates plugin candidate state with exact ordered item semantics.</summary>
    ...
    /// <returns>A task that completes after new and existing plugin output states are released.</returns>
    [Fact]
    public async Task Preview_NewExistingAndDeletedBaselines_ReportExactContextsAndWarnings()
    {
        ...
        own.Warnings.ShouldHaveSingleItem().Code.ShouldBe("missing-plugin-reference");
        ...
        preview.Value.Warnings.ShouldHaveSingleItem().Code.ShouldBe("missing-plugin-reference");
    }

    /// <summary>Applies one caller-owned command through immutable preparation and requires plugin success.</summary>
    /// <param name="service">The Fallout 4 edit service.</param>
    /// <param name="sources">The immutable plugin sources.</param>
    ...
    private static RecordEditMutationResult Apply(
        Fallout4RecordEditService service,
        Fallout4PluginSourceSet sources,
        Fallout4PluginOutputState state,
        FormKey target,
        FormListEdit edit)
    {
    ...
    /// <param name="service">The plugin output service.</param>
    /// <param name="sources">The immutable plugin sources.</param>
    /// <param name="state">The unpublished output candidate.</param>
    /// <returns>The allocated FormKey.</returns>
    private static FormKey BeginNew(
        Fallout4PluginOutputService service,
        Fallout4PluginSourceSet sources,
        Fallout4PluginOutputState state)
    {
    ...
    /// <summary>Opens generated Fallout 4 sources through the production plugin loader.</summary>
    /// <param name="fixture">The generated fixture.</param>
    /// <returns>The independently owned source lifetime.</returns>
    private static async Task<Fallout4PluginSourceSet> OpenSourcesAsync(Fallout4PluginOutputTestFixture fixture)
    {
        var open = await new Fallout4PluginSourceLoader(new PluginSourceInputLoader()).OpenAsync(
            fixture.Sources.CreateOpenRequest(),
            TestContext.Current.CancellationToken);
        open.Succeeded.ShouldBeTrue(open.Error?.Message);
        return open.Value!.Sources.ShouldBeOfType<Fallout4PluginSourceSet>();
    }

    /// <summary>Creates and selects one absent plugin output without writing it.</summary>
    /// <param name="fixture">The generated fixture.</param>
    /// <param name="sources">The immutable plugin sources.</param>
    /// <param name="service">The plugin output service.</param>
    /// <param name="fileName">The absent output file name.</param>
    /// <returns>The independently owned complete output state.</returns>
    private static async Task<Fallout4PluginOutputState> CreateNewStateAsync(
        ...
        string fileName)
    {
        var result = await service.OpenAsync(
            ...
            TestContext.Current.CancellationToken);
        result.Succeeded.ShouldBeTrue(result.Error?.Message);
        return result.Value!.Output.ShouldBeOfType<Fallout4PluginOutputState>();
    }

    /// <summary>Opens one generated existing plugin output.</summary>
    /// <param name="sources">The immutable plugin sources.</param>
    /// <param name="service">The plugin output service.</param>
    /// <param name="association">The exact existing-output association.</param>
    /// <returns>The independently owned complete output state.</returns>
    private static async Task<Fallout4PluginOutputState> OpenExistingStateAsync(
        Fallout4PluginSourceSet sources,
        Fallout4PluginOutputService service,
    ...
    /// <returns>A Fallout 4 output service using shared plugin artifact admission.</returns>
    private static Fallout4PluginOutputService CreateOutputService()
    {
        return new Fallout4PluginOutputService(new PluginOutputInputLoader());
    }

    /// <summary>Creates a two-language record translated string.</summary>
