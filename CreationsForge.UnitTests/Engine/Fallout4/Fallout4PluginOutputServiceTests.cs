...
/// <summary>Verifies complete Fallout 4 output selection, plugin preservation, cloning, reopening, and lifecycle.</summary>
public sealed class Fallout4PluginOutputServiceTests
{
    /// <summary>CreateNew produces complete full and small in-memory outputs without writing destination artifacts.</summary>
    /// <returns>A task that completes after both plugin output states are disposed.</returns>
    [Fact]
    public async Task OpenAsync_CreateNew_ConfiguresPluginHeadersWithoutWritingArtifacts()
    {
        ...
        await using var fullState = fullResult.Value!.Output.ShouldBeOfType<Fallout4PluginOutputState>();
        ...
        await using var reopenedNewState = reopenedNew.Value!.Output
            .ShouldBeOfType<Fallout4PluginOutputState>();
        ...
        await using var smallState = smallResult.Value!.Output.ShouldBeOfType<Fallout4PluginOutputState>();
    ...
    [Fact]
    public async Task OpenExistingAndClone_PreserveCompletePluginStateAndIsolation()
    {
        using var fixture = Fallout4PluginOutputTestFixture.Create();
        ...
        var state = open.Value!.Output.ShouldBeOfType<Fallout4PluginOutputState>();
    ...
    /// <summary>Reopen accepts the exact artifact baseline and rejects a valid plugin rewrite with a different baseline.</summary>
    ...
    public async Task ReopenAsync_RequiresExactOutputArtifactBaseline()
    {
        ...
        var open = await service.OpenAsync(
            ...
            TestContext.Current.CancellationToken);
        open.Succeeded.ShouldBeTrue(open.Error?.Message);
        await using var initial = open.Value!.Output.ShouldBeOfType<Fallout4PluginOutputState>();
        ...
        await using var unchangedState = unchanged.Value!.Output.ShouldBeOfType<Fallout4PluginOutputState>();
        AssertCompleteExistingState(unchangedState.CreateSnapshot(), existing);

        var forgedArtifacts = open.Value.Baseline.Artifacts
            .Select(artifact => artifact.Role == PluginArtifactRole.Plugin
                ? new PluginArtifactAssociation(
                    artifact.Path,
                    artifact.Role,
                    artifact.Language,
                    new PluginArtifactFingerprint(
                        exists: true,
    ...
    /// <summary>Game-plugin output operations accept the workspace's composite revision after output selection.</summary>
    ...
    public async Task OutputOperations_AcceptWorkspaceOwnedCompositeRevisions()
    {
        ...
        await using var state = open.Value!.Output.ShouldBeOfType<Fallout4PluginOutputState>();
    ...
    /// <returns>The independently owned complete plugin source set.</returns>
    ...
    private static Fallout4PluginOutputService CreateService()
    {
    ...
    /// <param name="mod">The defensive complete plugin output snapshot.</param>
