...
/// <summary>Verifies physical stdio recovery, repair, adoption, and stale-evidence behavior against real plugin artifacts.</summary>
public sealed class McpPluginCrossOwnerStdioTests
{
    ...
    /// <returns>A task that completes after terminal recovery, adoption, plugin inspection, and normal child exit.</returns>
    [Fact(Timeout = 90_000)]
    ...
    /// <returns>A task that completes after stale evidence is rejected and the independent owner's plugin output is reopened.</returns>
    [Fact(Timeout = 90_000)]
    ...
    /// <summary>Verifies a genuinely mixed plugin transaction remains unchanged through physical recovery, then supports explicit repair, reinspection, and adoption.</summary>
    /// <returns>A task that completes after repair publishes the prepared set and the fresh host inspects it through the game adapter.</returns>
    [Fact(Timeout = 90_000)]
    ...
    /// <param name="cancellationToken">The bounded test token.</param>
    /// <returns>A task that completes after state, baseline metadata, and staged plugin inspection are checked.</returns>
    ...
    /// <param name="recovery">The structured recovery receipt.</param>
    private static void AssertUnknownRecovery(WorkspaceRecoverySeed seed, JsonElement recovery)
    {
        recovery.GetProperty("originalWorkspaceId").GetString().ShouldBe(seed.OriginalWorkspaceId.ToString("D"));
        recovery.GetProperty("saveOperationId").GetString().ShouldBe(seed.SaveOperationId.ToString("D"));
    ...
    private static void AssertPreparedArtifactContent(WorkspaceRecoverySeed seed)
    {
    ...
    /// <param name="expectedEditorId">The expected plugin EditorID.</param>
    /// <param name="cancellationToken">The bounded test token.</param>
    ...
    /// <param name="workspaceId">The live workspace identity.</param>
    /// <param name="cancellationToken">The bounded test token.</param>
    /// <returns>A task that completes after plugin disposal.</returns>
    ...
    /// <param name="request">The explicit generated plugin request.</param>
    ...
    /// <param name="seed">The seeded recognized transaction.</param>
    /// <returns>A closed protocol argument dictionary.</returns>
    private static Dictionary<string, object?> RecoverArguments(WorkspaceRecoverySeed seed)
    {
        return new Dictionary<string, object?>
        {
    ...
    /// <param name="output">The exact plugin output association.</param>
