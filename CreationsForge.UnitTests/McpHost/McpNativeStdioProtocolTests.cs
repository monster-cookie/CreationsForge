...
/// <summary>
/// Verifies that the production stdio route exposes the real three-game native read engine.
/// </summary>
public sealed class McpNativeStdioProtocolTests
{
    /// <summary>The exact tools exposed when the production host owns a native workspace factory.</summary>
    ...
    /// <summary>Verifies production discovery, native reads, recursive paging, errors, closure, and source preservation.</summary>
    /// <param name="game">The generated native fixture family exercised through the child process.</param>
    [Theory]
    [InlineData(SupportedGame.Starfield)]
    [InlineData(SupportedGame.Fallout4)]
    [InlineData(SupportedGame.Skyrim)]
    public async Task ProductionStdioHost_ExposesRealNativeReadsForEverySupportedGame(SupportedGame game)
    {
        ...
        using var fixture = NativeMcpFixture.Create(game);
        var sourceArtifacts = fixture.SnapshotArtifacts();

        await using var directServices = NativeEngineComposition.Create();
        ...
        var missingSourcePath = Path.Combine(missingSourceRequest.DataDirectoryPath, "MissingNativeSource.esm");
        ...
        var directNativeFailure = await directServices.WorkspaceFactory.OpenAsync(missingRequest, cancellationToken);
        directNativeFailure.Succeeded.ShouldBeFalse();
        directNativeFailure.Error.ShouldNotBeNull().Code.ShouldBe(EngineErrorCode.InvalidRequest);
        var nativeFailure = await client.CallToolAsync(
            "creationsforge_workspace_open",
            CreateOpenArguments(missingRequest),
            cancellationToken: cancellationToken);
        GetErrorCode(nativeFailure).ShouldBe("invalid_request");
        AssertActiveWorkspaceCount(await GetServerInfoAsync(client, cancellationToken), 0);
    ...
    /// <param name="request">The explicit native request.</param>
    ...
    /// <summary>Reads direct native reference pages using the same query and page size as the protocol request.</summary>
    /// <param name="workspace">The independently opened real workspace.</param>
    /// <param name="cancellationToken">The bounded test token.</param>
    /// <returns>All direct native matches in deterministic page order.</returns>
    ...
    /// <param name="expected">The direct native search matches.</param>
    ...
    /// <param name="workspaceId">The MCP workspace identity.</param>
    /// <param name="selection">The exact native selection.</param>
    /// <param name="path">The canonical JSON Pointer.</param>
    ...
    /// <param name="workspaceId">The MCP workspace identity.</param>
    /// <param name="before">The prior native selection.</param>
    /// <param name="after">The resulting native selection.</param>
    ...
    /// <summary>Maps a native release to its protocol name.</summary>
    ...
    /// <summary>Adapts each existing generated native fixture to one common protocol-test shape.</summary>
    private sealed class NativeMcpFixture : IDisposable
    {
        ...
        /// <summary>Captures every task-owned native artifact.</summary>
        ...
        public static NativeMcpFixture Create(SupportedGame game)
        {
            return game switch
            {
        ...
        /// <summary>Creates an exact native open request with the supplied workspace identity.</summary>
        ...
        private static NativeMcpFixture CreateStarfield()
        {
            var fixture = StarfieldNativeTestFixture.Create();
            return new NativeMcpFixture(
                fixture,
        ...
        /// <returns>The owned fixture adapter.</returns>
        private static NativeMcpFixture CreateFallout4()
        {
        ...
        /// <returns>The owned fixture adapter.</returns>
        private static NativeMcpFixture CreateSkyrim()
        {
