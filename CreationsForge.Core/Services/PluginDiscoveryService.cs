...
{
    /// <summary>The plugin TES4 header flag indicating separate localized string files.</summary>
    private const uint LocalizedHeaderFlag = 0x00000080;

    /// <inheritdoc />
    public async ValueTask<EngineResult<PluginCatalog>> DiscoverAsync(
        SupportedGame game,
    ...
    private static EngineResult<PluginCatalog> Discover(
        SupportedGame game,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return EngineResult<PluginCatalog>.Failure(new EngineError(
            ...
            foreach (var listing in listings)
            {
                ...
                try
                {
                    ...
                    styles[listing.ModKey] = header.MasterStyle switch
                    {
                        ...
                        _ => throw new InvalidDataException($"Plugin '{listing.ModKey.FileName}' uses an unsupported plugin master style.")
                    };
            ...
            var entries = new List<PluginCatalogEntry>(pluginPaths.Count);
            ...
            return EngineResult<PluginCatalog>.Success(
                new PluginCatalog(game, release, dataDirectoryPath, entries));
        }
    ...
    /// <param name="requiredMasters">Receives every reachable master in valid plugin order, or an empty list for a cycle.</param>
    ...
    /// <summary>Reads bounded author and description text directly from a plugin header.</summary>
    /// <param name="header">The already loaded plugin header.</param>
    ...
    /// <summary>Normalizes optional plugin header text for product display.</summary>
    /// <param name="value">The decoded plugin text.</param>
    ...
    /// <summary>Maps a supported UI game to its exact plugin release.</summary>
