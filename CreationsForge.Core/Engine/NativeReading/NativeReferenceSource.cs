...
/// <summary>
/// Associates one borrowed native plugin getter with its explicit load-order and workspace provenance.
/// </summary>
public sealed class NativeReferenceSource
{
    /// <summary>The native semantic deep-copy operation used for resolved records from this source.</summary>
    private readonly Func<IMajorRecordGetter, IMajorRecordGetter> DeepCopyRecord;

    /// <summary>
    /// Initializes one borrowed native reference source.
    /// </summary>
    /// <param name="mod">The native plugin getter whose built-in record groups remain authoritative.</param>
    ...
    /// <param name="deepCopyRecord">An optional game-specific native deep-copy operation.</param>
    ...
    /// <summary>Gets the borrowed native plugin getter for shared reader operations without exposing it through the public context surface.</summary>
    internal IModGetter Mod { get; }

    /// <summary>Gets the native identity of the plugin containing its enumerated record contexts.</summary>
    public ModKey ModKey => Mod.ModKey;

    /// <summary>Gets the canonical path from which the native plugin was opened.</summary>
    ...
    /// <summary>Creates a detached semantic native copy through the configured game-aware operation.</summary>
    /// <param name="record">The borrowed native record to copy.</param>
    /// <returns>A detached native record getter.</returns>
    ...
    internal IMajorRecordGetter CreateDetachedCopy(IMajorRecordGetter record)
    {
        ...
        {
            throw new InvalidOperationException("A native reference deep-copy operation must return independent record state.");
        }

        return copy;
    }

    /// <summary>Creates the Mutagen native deep copy used when a game wrapper does not supply a more specific operation.</summary>
    /// <param name="record">The native record to copy.</param>
    /// <returns>A detached native common-record copy.</returns>
