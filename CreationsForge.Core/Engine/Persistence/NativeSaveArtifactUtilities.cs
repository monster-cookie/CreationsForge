...
internal static class NativeSaveArtifactUtilities
{
    ...
    {
        var plugin = new NativeOutputPluginInput(
            output.PluginPath,
            output.ModKey,
        ...
        var collector = new NativeOutputArtifactCollector(release, plugin);
        var artifacts = await collector.CaptureAsync(cancellationToken).ConfigureAwait(false);
        return new OutputArtifactSetBaseline(
            NativeArtifactSetUtilities.CreateBaselineId("CreationsForge.OutputArtifactSetBaseline/v1", artifacts),
            artifacts);
    }
    ...
    internal static async Task<IReadOnlyList<NativeArtifactAssociation>> RecaptureAsync(
        IReadOnlyList<NativeArtifactAssociation> expected,
        CancellationToken cancellationToken)
    {
        var observations = new NativeArtifactAssociation[expected.Count];
        for (var index = 0; index < expected.Count; index++)
        {
            var artifact = expected[index];
            observations[index] = await NativeFileInspector.InspectAsync(
                artifact.Path,
                artifact.Role,
                artifact.Language,
                mustExist: false,
    ...
    internal static OutputArtifactSetBaseline CreateOutputBaseline(IReadOnlyList<NativeArtifactAssociation> artifacts)
    {
        var ordered = artifacts
            ...
            .ToArray();
        return new OutputArtifactSetBaseline(
            NativeArtifactSetUtilities.CreateBaselineId("CreationsForge.OutputArtifactSetBaseline/v1", ordered),
            ordered);
    }
    ...
    {
        return NativeArtifactSetUtilities.Match(expected, actual);
    }

    /// <summary>Determines whether output associations identify the same canonical output and native options.</summary>
    ...
    internal static bool MatchSourceBaseline(
        NativeSourceInputBaseline left,
        NativeSourceInputBaseline right)
    {
        return left.BaselineId == right.BaselineId && MatchExact(left.Artifacts, right.Artifacts);
    }
    ...
    internal static RecoveryEvidenceToken CreateEvidenceToken(
        ...
        IEnumerable<NativeArtifactAssociation> observations)
    {
    ...
    /// <summary>Maps the shared output master style to Mutagen's native value.</summary>
    /// <param name="style">The shared output style.</param>
    /// <returns>The native master style.</returns>
