...
/// <summary>
/// Prepares bounded plugin source inputs exclusively from caller-supplied plugin, data, and localized-string paths.
/// </summary>
public sealed class PluginSourceInputLoader
{
    ...
    /// <summary>Initializes a stateless plugin source-input loader.</summary>
    public PluginSourceInputLoader()
    { }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var plugins = new List<PluginSourcePluginInput>(pluginPaths.Count);
            ...
            for (var index = 0; index < pluginPaths.Count; index++)
            {
                ...
                if (!modKeys.Add(modKey))
                {
                    throw new PluginSourceInputException(
                        EngineErrorCode.InvalidRequest,
                        $"The explicit load order contains plugin identity '{modKey.FileName}' more than once.");
                }
                ...
                plugins.Add(new PluginSourcePluginInput(
                    path,
                    modKey,
                    index,
                    role,
            ...
            var lookups = CreateStringsLookups(
                ...
                fileSystem);
            var inputs = new PluginSourceInputs(
                request.Release,
                plugins,
                dataDirectoryPath,
                stringDirectoryPaths,
        ...
        {
            return EngineResult<PluginSourceInputs>.Failure(
    ...
    /// <summary>Reads the supported games' plugin localized-file selection bit from a TES4 header.</summary>
    /// <param name="header">The plugin header frame.</param>
    ...
    /// <exception cref="PluginSourceInputException">Thrown when two plugin descriptors alias the same physical file.</exception>
    private static void ValidateDistinctPluginIdentities(IReadOnlyList<PluginArtifactAssociation> artifacts)
    {
        var identities = new HashSet<ArtifactFileIdentity>();
        foreach (var artifact in artifacts.Where(artifact => artifact.Role == PluginArtifactRole.Plugin))
        {
            if (artifact.FileIdentity is not null && !identities.Add(artifact.FileIdentity))
            {
                throw new PluginSourceInputException(
                    EngineErrorCode.InvalidRequest,
                    "Two explicit plugin paths identify the same physical file.");
            }
        }
    }

    /// <summary>Validates the exact CreationsForge game and plugin release pair supported by this bounded backend.</summary>
    /// <param name="game">The requested CreationsForge game.</param>
    /// <param name="release">The requested Mutagen game release.</param>
    /// <exception cref="PluginSourceInputException">Thrown when the pair is unsupported or mismatched.</exception>
    private static void ValidateGameRelease(SupportedGame game, GameRelease release)
    {
        ...
        if (!supported)
        {
            throw new PluginSourceInputException(
                ...
                $"The plugin source-input loader does not support {game} with release {release}.");
        }
    }
    ...
    /// <exception cref="PluginSourceInputException">Thrown when the load order is empty, invalid, missing, or duplicated.</exception>
    private static IReadOnlyList<string> CanonicalizePluginPaths(IReadOnlyList<string> paths)
    {
        ...
        if (paths.Count == 0)
        {
            throw new PluginSourceInputException(
                EngineErrorCode.InvalidRequest,
                "At least one explicit plugin path is required.");
        }
    ...
    /// <exception cref="PluginSourceInputException">Thrown when a directory is invalid, missing, or duplicated.</exception>
    ...
    /// <exception cref="PluginSourceInputException">Thrown when the path is invalid or the file is absent.</exception>
    ...
    /// <exception cref="PluginSourceInputException">Thrown when the path is invalid or the directory is absent.</exception>
    ...
    /// <summary>Parses a plugin identity from one canonical plugin file name.</summary>
    /// <param name="path">The canonical plugin path.</param>
    /// <returns>The plugin identity.</returns>
    /// <exception cref="PluginSourceInputException">Thrown when the file name is not a valid plugin identity.</exception>
    private static ModKey ParseModKey(string path)
    {
        ...
        throw new PluginSourceInputException(
            EngineErrorCode.InvalidRequest,
            $"The plugin file name '{fileName}' is invalid: {error}");
    }

    /// <summary>Rejects a header master style unsupported by the selected plugin release.</summary>
    /// <param name="release">The selected plugin release.</param>
    /// <param name="modKey">The plugin being validated.</param>
    /// <param name="masterStyle">The style read from its plugin header.</param>
    /// <exception cref="PluginSourceInputException">Thrown when the release cannot decode the style.</exception>
    private static void ValidateMasterStyle(GameRelease release, ModKey modKey, MasterStyle masterStyle)
    {
        ...
        if (!supported)
        {
            throw new PluginSourceInputException(
                ...
                $"Plugin '{modKey.FileName}' uses {masterStyle} master style, which release {release} does not support.");
        }
    }

    /// <summary>Validates that every declared plugin master exists earlier in the explicit load order and preserves plugin declaration order.</summary>
    /// <param name="plugins">The explicit plugin descriptors.</param>
    /// <param name="declaredMasters">The plugin header masters corresponding to each descriptor.</param>
    /// <exception cref="PluginSourceInputException">Thrown when a master is missing, duplicated, or ordered after its dependent plugin.</exception>
            ...
            foreach (var master in declaredMasters[pluginIndex])
            {
                if (!indexByModKey.TryGetValue(master, out var masterIndex))
                {
                    throw new PluginSourceInputException(
                        ...
                        $"Plugin '{plugins[pluginIndex].ModKey.FileName}' requires missing explicit master '{master.FileName}'.");
                }
                ...
                if (masterIndex >= pluginIndex || masterIndex <= previousMasterIndex)
                {
                    throw new PluginSourceInputException(
                        EngineErrorCode.SourceOpenFailed,
                        $"Plugin '{plugins[pluginIndex].ModKey.FileName}' declares master '{master.FileName}' outside valid explicit load-order order.");
                }
    ...
    /// <param name="declaredMasters">The initially observed plugin master lists.</param>
    /// <param name="fileSystem">The file-system adapter used for plugin header reads.</param>
    /// <param name="cancellationToken">The token checked between header reads.</param>
    /// <exception cref="PluginSourceInputException">Thrown when plugin header metadata changed.</exception>
        ...
        for (var index = 0; index < plugins.Count; index++)
        {
                ...
                || !currentMasters.SequenceEqual(declaredMasters[index]))
            {
                throw new PluginSourceInputException(
                    ...
                    $"Plugin '{plugins[index].ModKey.FileName}' changed its header metadata during source preparation.");
            }
        }
    }

    /// <summary>Creates one ordered record strings lookup for each explicit plugin without consulting an installed game.</summary>
    /// <param name="release">The selected plugin release.</param>
    /// <param name="plugins">The explicit plugin descriptors.</param>
    ...
    /// <returns>An immutable plugin-keyed dictionary of record strings lookups.</returns>
    private static IReadOnlyDictionary<ModKey, PluginStringsFolderLookup> CreateStringsLookups(
        GameRelease release,
        IReadOnlyList<PluginSourcePluginInput> plugins,
        ...
        IFileSystem fileSystem)
    {
        var result = new Dictionary<ModKey, PluginStringsFolderLookup>();
        foreach (var plugin in plugins)
        {
            ...
            var lookups = stringDirectoryPaths
                ...
                .ToArray();
            result.Add(plugin.ModKey, new PluginStringsFolderLookup(lookups));
        }

        return new ReadOnlyDictionary<ModKey, PluginStringsFolderLookup>(result);
    }
}
