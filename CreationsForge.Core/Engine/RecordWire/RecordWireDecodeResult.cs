...
public sealed class RecordWireDecodeResult<T>
    where T : class
{
    /// <summary>Initializes an immutable record wire decode result.</summary>
    ...
    private RecordWireDecodeResult(bool succeeded, T? value, EngineError? error)
    {
        Succeeded = succeeded;
        Value = value;
        Error = error;
    ...
    public static RecordWireDecodeResult<T> Success(T value)
    {
        ArgumentNullException.ThrowIfNull(value);
        return new RecordWireDecodeResult<T>(true, value, null);
    }
    ...
    public static RecordWireDecodeResult<T> Failure(EngineError error)
    {
        ArgumentNullException.ThrowIfNull(error);
        return new RecordWireDecodeResult<T>(false, null, error);
    }
}
