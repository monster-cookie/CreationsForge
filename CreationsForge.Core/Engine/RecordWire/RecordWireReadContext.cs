...
/// <summary>Tracks one bounded record wire traversal and converts expected input failures into typed results.</summary>
public sealed partial class RecordWireReadContext
{
    ...
    public static RecordWireDecodeResult<T> Decode<T>(
        JsonElement root,
        RecordWireReadLimits limits,
        CancellationToken cancellationToken,
        Func<RecordWireReadContext, RecordWireValue, T> decoder)
        where T : class
    {
        ...
        var context = new RecordWireReadContext(limits, cancellationToken);
        try
        {
            ...
            if (value is null)
            {
                throw new InvalidOperationException("A record wire decoder returned null without reporting an input failure.");
            }

            return RecordWireDecodeResult<T>.Success(value);
        }
        catch (RecordWireInputException exception)
        {
            return RecordWireDecodeResult<T>.Failure(exception.Error);
        }
    }
    ...
    /// <exception cref="OperationCanceledException">Thrown when cancellation is requested.</exception>
    public RecordWireObject ReadObject(RecordWireValue value, RecordWireObjectShape shape)
    {
    ...
    /// <summary>Validates one array and its element count before generated code allocates a record collection.</summary>
    ...
    /// <exception cref="OperationCanceledException">Thrown when cancellation is requested.</exception>
    public RecordWireArray ReadArray(RecordWireValue value)
    {
        EnsureOwned(value);
        CancellationToken.ThrowIfCancellationRequested();
        ...
        return new RecordWireArray(this, value.Element, value.Path!, value.Depth);
    }

    /// <summary>Validates one decoded array or rectangular-array dimension before generated code allocates record storage.</summary>
    ...
    /// <exception cref="OperationCanceledException">Thrown when cancellation is requested.</exception>
    public void ValidateArrayLength(RecordWireValue value, int length)
    {
        EnsureOwned(value);
        CancellationToken.ThrowIfCancellationRequested();
        if (length < 0)
    ...
    /// <exception cref="OperationCanceledException">Thrown when cancellation is requested.</exception>
    public bool IsNull(RecordWireValue value)
    {
        EnsureOwned(value);
        CancellationToken.ThrowIfCancellationRequested();
    ...
    /// <summary>Rejects a statically recognized invalid record state at the owning value's path.</summary>
    /// <param name="value">The context-owned value whose state is invalid.</param>
    /// <param name="message">A non-empty description of the violated record invariant.</param>
    /// <exception cref="ArgumentException">Thrown when <paramref name="message"/> is empty or whitespace.</exception>
    [DoesNotReturn]
    public void Reject(RecordWireValue value, string message)
    {
    ...
    /// <exception cref="OperationCanceledException">Thrown when cancellation is requested.</exception>
    internal RecordWireValue CreateChild(JsonElement element, string path, int depth)
    {
    ...
    private void EnsureOwned(RecordWireValue value)
    {
        if (!ReferenceEquals(value.Context, this) || value.Path is null)
        {
            throw new InvalidOperationException("The record wire value does not belong to this decode context.");
        }
    }
    ...
    /// <exception cref="OperationCanceledException">Thrown when cancellation is requested.</exception>
    private RecordWireValue CreateValue(JsonElement element, string path, int depth)
    {
        CancellationToken.ThrowIfCancellationRequested();
        ...
        VisitedNodes++;
        return new RecordWireValue(this, element, path, depth);
    }
    ...
    [DoesNotReturn]
    private void Fail(RecordWireValue value, string message)
    {
        EnsureOwned(value);
    ...
    {
        throw new RecordWireInputException(new EngineError(
    ...
    private sealed class RecordWireInputException : Exception
    {
        ...
        internal RecordWireInputException(EngineError error)
            : base(error.Message)
        {
            Error = error;
        }
