...
/// <summary>Defines the complete ordinal property set accepted by one generated record wire object.</summary>
public sealed class RecordWireObjectShape
{
        ...
        string parameterName)
    {
        ...
        {
            throw new ArgumentException("Plugin wire property names cannot be empty or whitespace.", parameterName);
        }

        if (!ordinals.TryAdd(name, ordinal))
        {
            throw new ArgumentException($"Plugin wire property '{name}' is duplicated.", parameterName);
        }
    }
}
