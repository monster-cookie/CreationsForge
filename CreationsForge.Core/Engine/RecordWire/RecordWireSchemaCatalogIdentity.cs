...
/// <summary>Identifies one immutable compiled record wire schema catalog.</summary>
public sealed class RecordWireSchemaCatalogIdentity
{
    ...
    /// <param name="release">The exact record release used by the compiled constructors.</param>
    ...
    public RecordWireSchemaCatalogIdentity(
        SupportedGame game,
        GameRelease release,
    ...
    /// <summary>Gets the exact record release used by the compiled constructors.</summary>
    ...
    internal static string NormalizeCatalogId(string catalogId, string parameterName)
    {
        ...
        if (catalogId.Length != 64 || catalogId.Any(character => !Uri.IsHexDigit(character)))
        {
            throw new ArgumentException(
                "A record wire schema catalog identity must contain exactly 64 hexadecimal characters.",
                parameterName);
        }
