...
/// <summary>Provides strict access to one validated closed native wire object.</summary>
public sealed class NativeWireObject
{
    ...
    private readonly NativeWireObjectShape Shape;
    ...
    public NativeWireValue GetRequiredProperty(string name)
    {
        var ordinal = Shape.GetOrdinal(name);
        if (!Shape.IsRequired(ordinal))
        {
            throw new InvalidOperationException(
                $"Property '{name}' is not required by native wire object shape '{Shape.Name}'.");
        }

        if (!Element.TryGetProperty(name, out var child))
        {
            throw new InvalidOperationException(
                $"Validated native wire object shape '{Shape.Name}' no longer contains required property '{name}'.");
        }
    ...
    /// <exception cref="OperationCanceledException">Thrown when cancellation is requested.</exception>
    public bool TryGetOptionalProperty(string name, out NativeWireValue value)
    {
        var ordinal = Shape.GetOrdinal(name);
        if (ordinal < Shape.RequiredProperties.Count || ordinal >= Shape.PropertyCount)
        {
            throw new InvalidOperationException(
                $"Property '{name}' is not optional in native wire object shape '{Shape.Name}'.");
        }
