...
namespace CreationsForge.Specification.Validation.Specs.NPC;
...
public static class NPCValidationSpecs
{
    ...
    {
        return BaseNPC(SpecificationGame.Starfield, sampleName, formKey);
    }

    private static ValidationSpecBuilder Fallout4NPC(string sampleName, string formKey)
    {
        return BaseNPC(SpecificationGame.Fallout4, sampleName, formKey);
    }

    private static ValidationSpecBuilder SkyrimNPC(string sampleName, string formKey)
    {
        return BaseNPC(SpecificationGame.Skyrim, sampleName, formKey);
    }

    private static ValidationSpecBuilder BaseNPC(SpecificationGame game, string sampleName, string formKey)
    {
        return ValidationSpecBuilder
            .ForRecord(game, SupportedRecordSpecifications.NPC)
            .Sample(sampleName)
            .FormKey(formKey)
    ...
    private static IEnumerable<ValidationFieldRule> GetNPCTypedRules(SpecificationGame game)
    {
        ...
        yield return game == SpecificationGame.Starfield
        ...
        if (game == SpecificationGame.Skyrim)
        {
        ...
        if (game == SpecificationGame.Fallout4)
        {
            yield return ValidationFieldRule.DtoDefaultWhenSpriggitAbsent(
                "UseTemplateActors",
                "UseTemplateActors",
