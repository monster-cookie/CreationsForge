...
/// <summary>Forwards bounded major-record listings without retaining an MCP-side index or cursor.</summary>
public sealed class MajorRecordsListTool : McpToolBase
{
    ...
    /// <summary>The closed engine-list input schema with engine-owned bounds.</summary>
    private static readonly JsonElement InputSchema = ParseSchema(
        """{"type":"object","properties":{"workspaceId":{"type":"string","format":"uuid"},"scope":{"type":"string","enum":["winning_overrides","all_contexts","source","staged_output"]},"containingModKey":{"type":"string","minLength":1,"maxLength":1024},"maxResults":{"type":"integer","minimum":1,"maximum":250},"cursor":{"type":"string","minLength":1,"maxLength":16384}},"required":["workspaceId","scope"],"additionalProperties":false}""");

    /// <summary>The closed major-record list output schema.</summary>
    private static readonly JsonElement OutputSchema = McpToolSchema.Output(
        "{\"type\":\"object\",\"properties\":{\"workspaceId\":{\"type\":\"string\",\"format\":\"uuid\"},\"revision\":" + McpToolSchema.Revision + ",\"records\":{\"type\":\"array\",\"items\":{\"type\":\"object\",\"properties\":{\"formKey\":{\"type\":\"string\",\"minLength\":1},\"recordType\":{\"type\":\"string\",\"minLength\":1},\"editorId\":" + McpToolSchema.NullableString + ",\"containingModKey\":" + McpToolSchema.NullableString + ",\"sourcePath\":" + McpToolSchema.NullableString + ",\"loadOrderIndex\":" + McpToolSchema.NullableInteger + ",\"role\":" + McpToolSchema.NullableString + ",\"isDeleted\":{\"type\":\"boolean\"}},\"required\":[\"formKey\",\"recordType\",\"editorId\",\"containingModKey\",\"sourcePath\",\"loadOrderIndex\",\"role\",\"isDeleted\"],\"additionalProperties\":false}},\"cursor\":" + McpToolSchema.NullableString + ",\"warnings\":{\"type\":\"array\",\"items\":" + McpToolSchema.Warning + "}},\"required\":[\"workspaceId\",\"revision\",\"records\",\"cursor\",\"warnings\"],\"additionalProperties\":false}");

    /// <summary>The host-owned workspace registry.</summary>
    private readonly McpWorkspaceRegistry WorkspaceRegistry;

    /// <summary>Initializes the major-record listing tool.</summary>
    /// <param name="workspaceRegistry">The registry that owns active engine workspaces.</param>
    public MajorRecordsListTool(McpWorkspaceRegistry workspaceRegistry)
    {
        ArgumentNullException.ThrowIfNull(workspaceRegistry);
        WorkspaceRegistry = workspaceRegistry;
    }

    /// <summary>Gets the closed read-only major-record listing descriptor.</summary>
    public override Tool ProtocolTool { get; } = new Tool
    {
        Name = "creationsforge_records_list",
        Title = "List major records",
        Description = "Lists lightweight major-record contexts through the engine's bounded deterministic pager.",
    ...
    /// <returns>One exact engine major-record page.</returns>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        MajorRecordListRequest engineRequest;
        try
        {
            engineRequest = new MajorRecordListRequest(maximumResults, cursor, scope, containingModKey);
        }
        ...
        var engineResult = await WorkspaceRegistry.ExecuteAsync(
            workspaceId,
            (workspace, token) => workspace.ListMajorRecordsAsync(engineRequest, token),
            ...
            !McpProjection.TryGetResultRevision(engineResult, out var revision, out error))
        {
            return Error("unexpected_failure", engineResult.Value is null
                ? "The engine reported success without a major-record page."
                : error);
        }
        ...
        var projected = McpProjection.Json(new
        {
            ...
            records = page.Records.Select(record => new
            {
                ...
                containingModKey = record.ContainingModKey?.ToString(),
                sourcePath = record.SourcePath,
                loadOrderIndex = record.LoadOrderIndex,
                role = record.Role.HasValue ? McpProjection.PluginRole(record.Role.Value) : null,
        ...
        {
            return Error("result_too_large", "The exact engine major-record page exceeds the 64 KiB structured result budget; retry with a smaller maxResults value.");
        }

        return Success(projected, $"Returned {page.Records.Count} major-record context(s).");
    }
}
