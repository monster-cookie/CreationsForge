...
/// <summary>
/// Reads one exact major-record context and exposes its detached JSON through stateless pointer pages.
/// </summary>
public sealed class MajorRecordInspectTool : McpToolBase
{
    ...
    private static readonly JsonElement OutputSchema = McpToolSchema.Output(
        "{\"type\":\"object\",\"properties\":{\"workspaceId\":{\"type\":\"string\",\"format\":\"uuid\"},\"revision\":" + McpToolSchema.Revision + ",\"context\":" + McpToolSchema.Context + ",\"recordType\":" + McpToolSchema.NullableString + ",\"page\":" + McpToolSchema.JsonPage + ",\"warnings\":{\"type\":\"array\",\"items\":" + McpToolSchema.Warning + "}},\"required\":[\"workspaceId\",\"revision\",\"context\",\"recordType\",\"page\",\"warnings\"],\"additionalProperties\":false}");
    ...
    public override Tool ProtocolTool { get; } = new Tool
    {
        ...
        Description = "Re-reads one exact major-record context and pages its detached typed JSON by immediate JSON Pointer children or Unicode-safe scalar chunks.",
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var count = maximumResults; count >= 1; count--)
        {
            ...
            if (FitsStructuredResultBudget(projected))
            {
                return Success(
                    projected,
                    $"Returned one bounded major-record inspection page with status {McpProjection.ResolutionStatus(engineResult.Value.Context.Status)}.");
            }
        }
