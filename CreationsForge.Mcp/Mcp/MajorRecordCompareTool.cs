...
/// <summary>
/// Compares two exact major-record contexts and pages typed views, semantic changes, or warnings independently.
/// </summary>
public sealed class MajorRecordCompareTool : McpToolBase
{
    ...
    private static readonly JsonElement OutputSchema = McpToolSchema.Output(
        "{\"type\":\"object\",\"properties\":{\"workspaceId\":{\"type\":\"string\",\"format\":\"uuid\"},\"revision\":" + McpToolSchema.Revision + ",\"formKey\":{\"type\":\"string\",\"minLength\":1},\"recordType\":" + McpToolSchema.NullableString + ",\"beforeContext\":" + McpToolSchema.Context + ",\"afterContext\":" + McpToolSchema.Context + ",\"section\":{\"type\":\"string\",\"enum\":[\"changes\",\"before\",\"after\",\"warnings\"]},\"page\":{\"oneOf\":[" + McpToolSchema.JsonPage + "," + ChangesPageSchema + "," + WarningsPageSchema + "]}},\"required\":[\"workspaceId\",\"revision\",\"formKey\",\"recordType\",\"beforeContext\",\"afterContext\",\"section\",\"page\"],\"additionalProperties\":false}");

    /// <summary>The host-owned workspace registry.</summary>
    private readonly McpWorkspaceRegistry WorkspaceRegistry;

    /// <summary>Initializes the bounded major-record comparison tool.</summary>
    /// <param name="workspaceRegistry">The registry that owns active engine workspaces.</param>
    public MajorRecordCompareTool(McpWorkspaceRegistry workspaceRegistry)
    {
        ArgumentNullException.ThrowIfNull(workspaceRegistry);
        WorkspaceRegistry = workspaceRegistry;
    }

    /// <summary>Gets the closed read-only major-record comparison descriptor.</summary>
    public override Tool ProtocolTool { get; } = new Tool
    {
        Name = "creationsforge_record_compare",
        Title = "Compare major-record contexts",
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var comparisonRequest = new CompareMajorRecordRequest(before, after);
        var engineResult = await WorkspaceRegistry.ExecuteAsync(
            workspaceId,
            (workspace, token) => workspace.CompareMajorRecordAsync(comparisonRequest, token),
            ...
            !McpProjection.TryGetResultRevision(engineResult, out var revision, out error))
        {
            return Error("unexpected_failure", engineResult.Value is null
                ? "The engine reported success without a major-record comparison."
                : error);
        }

        if (expectedRevision.HasValue && expectedRevision.Value != revision)
        {
            return Error("revision_conflict", "The expected revision does not match the fresh major-record comparison result revision.");
        }
