...
/// <summary>
/// Writes and compares complete Starfield FormList state through direct typed record getters.
/// </summary>
public sealed class StarfieldFormListInspector : IFormListInspector
{
    /// <summary>Initializes the stateless Starfield FormList inspector.</summary>
    public StarfieldFormListInspector()
    { }

    /// <summary>Writes every Starfield FormList field in generated field-index order.</summary>
    /// <param name="record">The detached Mutagen Starfield FormList getter.</param>
    ...
    /// <param name="formList">The typed Mutagen Starfield FormList getter.</param>
    /// <param name="writer">The caller-owned writer that receives exactly one JSON object.</param>
        ...
        RecordJsonWriteContext? context)
    {
        ...
        RecordJsonLeafWriter.WriteFormLink(writer, formList.AddToList, context);
        writer.WriteEndObject();
    }

    /// <summary>Compares every Starfield FormList field through direct typed record values.</summary>
    ...
    /// <param name="cancellationToken">A token observed while comparing record fields and collections.</param>
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!RecordSemanticComparer.CompareRecordPresence(
            beforeFormList,
            afterFormList,
            changes,
            cancellationToken))
        ...
        RecordSemanticComparer.CompareValue(
            ...
            RecordSemanticComparer.FormLinkEquals(presentBefore.AddToList, presentAfter.AddToList),
            changes,
            cancellationToken);
    ...
    /// <summary>Writes the ordered polymorphic component collection as complete nested Mutagen objects.</summary>
    /// <param name="writer">The caller-owned JSON writer.</param>
    /// <param name="components">The Mutagen component getters in source order.</param>
    ...
    /// <param name="writer">The caller-owned JSON writer.</param>
    /// <param name="entries">The Mutagen conditional entries in source order.</param>
    ...
    /// <param name="path">The canonical Mutagen condition-collection path.</param>
    /// <param name="changes">The ordered semantic change collector.</param>
    ...
    /// <param name="path">The canonical Mutagen collection path.</param>
    ...
    private static IFormListGetter RequireFormList(IMajorRecordGetter record, string parameterName)
    {
        return record as IFormListGetter
            ?? throw new ArgumentException("The record must be a Starfield FormList getter.", parameterName);
    }
