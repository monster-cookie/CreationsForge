...
/// <summary>
/// Owns one complete mutable Starfield output and its independent Mutagen baseline without retaining parsing resources.
/// </summary>
public sealed class StarfieldPluginOutputState : IPluginOutputState
{
    ...
    private List<RecordEditProvenance>? Provenance;

    /// <summary>Tracks whether this independently owned state has been disposed.</summary>
    private int IsDisposed;

    /// <summary>
    /// Initializes a complete Starfield output state after Mutagen admission and artifact verification succeed.
    /// </summary>
        ...
        IReadOnlyList<RecordEditProvenance>? provenance = null)
    {
        ...
        {
            throw new ArgumentException("The plugin output state must retain the selected output ModKey.", nameof(mod));
        }

        MutableMod = mod;
        OriginalMod = originalMod;
        Provenance = provenance?.ToList() ?? new List<RecordEditProvenance>();
        Association = association;
        Baseline = baseline;
    }

    /// <summary>Gets the exact output identity and Mutagen formatting choices.</summary>
    ...
    /// <summary>
    /// Creates a complete detached Mutagen snapshot of the current staged output.
    /// </summary>
    ...
    /// <summary>Creates a complete detached Mutagen snapshot of the output as it appeared when selected.</summary>
    /// <param name="cancellationToken">A token checked immediately before and after the uninterruptible Mutagen copy.</param>
    ...
    /// <summary>Gets the complete mutable plugin output for same-assembly adapter operations.</summary>
    ...
    /// <summary>Gets the independently owned complete plugin output baseline for same-assembly comparisons.</summary>
    /// <returns>The immutable-by-convention original plugin output.</returns>
    /// <exception cref="ObjectDisposedException">Thrown after this state is disposed.</exception>
    ...
    /// <exception cref="ObjectDisposedException">Thrown after this state is disposed.</exception>
    internal IReadOnlyList<RecordEditProvenance> BorrowProvenance()
    {
    ...
    /// <exception cref="ObjectDisposedException">Thrown after this state is disposed.</exception>
    internal bool AddProvenanceIfAbsent(RecordEditProvenance provenance)
    {
    ...
    /// <summary>Releases the complete records retained by this independently owned output state.</summary>
