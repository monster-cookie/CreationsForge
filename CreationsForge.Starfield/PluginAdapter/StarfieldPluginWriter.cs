...
/// <summary>Writes complete Starfield output state only into private staging and proves its Mutagen semantic preservation before commit.</summary>
public sealed class StarfieldPluginWriter
{
    /// <summary>The typed output service used for destination freshness checks and strict staged reopen.</summary>
    private readonly StarfieldPluginOutputService _outputService;
    ...
    public StarfieldPluginWriter(StarfieldPluginOutputService outputService)
    {
        ...
        CancellationToken cancellationToken = default)
    {
            ...
            && expected.EnumerateMajorRecords().Any(record => record is not IFormListGetter))
        {
            return Failure(sources, new EngineError(
                EngineErrorCode.UnsupportedInput,
                $"Localized Starfield output '{output.Association.ModKey.FileName}' contains a record family whose complete multilingual preservation is not supported."));
        }
        ...
        catch (Exception exception)
        {
            return Failure(sources, new EngineError(
                ...
                $"Starfield Mutagen serialization into private staging failed: {exception.Message}"));
        }
        ...
        if (!stagedOpen.Succeeded || stagedOpen.Value is null)
        {
            return Failure(
                sources,
                stagedOpen.Error ?? new EngineError(EngineErrorCode.OutputOpenFailed, "The staged Starfield output could not be reopened through Mutagen."),
                stagedOpen.Warnings);
        }

        var stagedState = (StarfieldPluginOutputState)stagedOpen.Value.Output;
        var valid = false;
        try
        {
            ...
            var stagedLifetime = new StarfieldStagedPluginOutputSet(stagedState);
            valid = true;
            return Success(
                sources,
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var record in candidate.EnumerateMajorRecords())
        {
            ...
            if (!allKeys.Add(record.FormKey))
            {
                return new EngineError(
                    EngineErrorCode.ValidationFailed,
                    $"Starfield candidate identity '{record.FormKey}' is duplicated across record families.");
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        masterFlags.Set(new KeyedMasterStyle(
            ...
            {
                OutputMasterStyle.Full => MutagenMasterStyle.Full,
                OutputMasterStyle.Small => MutagenMasterStyle.Small,
                OutputMasterStyle.Medium => MutagenMasterStyle.Medium,
        ...
        {
            throw new InvalidOperationException("The normalized Starfield master list changed before Mutagen serialization.");
        }
...
internal sealed class StarfieldStagedPluginOutputSet : IStagedPluginOutputSet
{
    /// <summary>The independently owned strictly reopened staged output.</summary>
    private StarfieldPluginOutputState? _output;
    ...
    internal StarfieldStagedPluginOutputSet(StarfieldPluginOutputState output)
    {
        ArgumentNullException.ThrowIfNull(output);
        _output = output;
    }
