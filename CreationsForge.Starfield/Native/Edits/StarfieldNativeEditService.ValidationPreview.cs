...
public sealed partial class StarfieldNativeEditService
{
    ...
    /// <param name="target">The exact requested native identity.</param>
    ...
    private static EngineResult<FormList> FindMutableTarget(StarfieldMod mod, FormKey target)
    {
        ...
        if (selected is null)
        {
            return EngineResult<FormList>.Failure(new EngineError(
                ...
                wrongFamily
                    ? $"Starfield record {target} belongs to another native record family."
        ...
        foreach (var reference in references)
        {
            ...
            if (!reference.ExpectedType.IsInstanceOfType(resolution.Value))
            {
                return new EngineError(
                    EngineErrorCode.ValidationFailed,
                    $"Starfield reference {reference.FormKey} at {reference.FieldPath} must resolve to native target family {reference.ExpectedType.Name}.");
            }
        }
    ...
    /// <param name="formKey">The non-null native identity to resolve.</param>
    ...
    /// <returns>The exact detached prior native side or a typed baseline failure.</returns>
    ...
    /// <returns>The detached current native side or a typed missing-state failure.</returns>
    ...
    /// <summary>Finds one FormList getter in an exact complete native plugin.</summary>
    ...
    /// <returns>Warnings in native link traversal order.</returns>
        ...
        out int unresolvedCount)
    {
        ...
        foreach (var link in record.EnumerateFormLinks(false))
        {
            ...
            {
                warnings.Add(new EngineWarning(
                    "invalid-native-reference-family",
                    $"Starfield reference {formKey} at FormLinks[{index}] does not resolve to declared native target family {link.Type.Name}."));
            }
    ...
    /// <summary>Compares two complete captured native contexts without substituting current winners.</summary>
    ...
    /// <summary>Pairs one exact native context with its detached FormList, including a confirmed absent side.</summary>
    private sealed class PreviewRecord
    {
        ...
        /// <summary>Gets the exact native selection context.</summary>
        internal FormListContext Context { get; }

        /// <summary>Gets the detached native FormList, or <see langword="null"/> for confirmed absence.</summary>
