...
/// <summary>Composes the complete Starfield native FormList engine behind the shared game-adapter contract.</summary>
public sealed class StarfieldFormListGameAdapter : IFormListGameAdapter
{
    ...
    private readonly StarfieldNativeSourceLoader SourceLoader;

    /// <summary>The Starfield output acquisition and edit-target boundary.</summary>
    private readonly StarfieldNativeOutputService OutputService;

    /// <summary>The Starfield typed edit and preview boundary.</summary>
    private readonly StarfieldNativeEditService EditService;

    /// <summary>The Starfield private staged-write and native-reopen boundary.</summary>
    private readonly StarfieldNativeWriter Writer;

    /// <summary>Initializes the adapter from the complete Starfield native services.</summary>
    ...
    /// <param name="writer">The private staged-write and native-reopen service.</param>
    ...
    public StarfieldFormListGameAdapter(
        StarfieldNativeSourceLoader sourceLoader,
        StarfieldNativeOutputService outputService,
        StarfieldNativeEditService editService,
        StarfieldNativeWriter writer)
    {
    ...
    /// <inheritdoc />
    public IFormListNativeInspector Inspector => OutputService.Inspector;
    ...
    public async ValueTask<EngineResult<NativeOutputOpenResult>> OpenOutputAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var sourceResult = RequireSources<NativeOutputOpenResult>(sources);
        if (!sourceResult.Succeeded)
        {
            return EngineResult<NativeOutputOpenResult>.Failure(sourceResult.Error!);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (output is not StarfieldNativeOutputState starfieldOutput)
        {
            throw new ArgumentException("The Starfield adapter requires Starfield native output state.", nameof(output));
        }
    ...
    public EngineResult<NativeEditIdentity> BeginEdit(
        ...
        CancellationToken cancellationToken)
    {
        var sourceResult = RequireSources<NativeEditIdentity>(sources);
        if (!sourceResult.Succeeded)
        {
            return EngineResult<NativeEditIdentity>.Failure(sourceResult.Error!);
        }

        var outputResult = RequireOutput<NativeEditIdentity>(candidate);
        if (!outputResult.Succeeded)
        {
            return EngineResult<NativeEditIdentity>.Failure(outputResult.Error!);
        }
    ...
    {
        var sourceResult = RequireSources<NativeEditMutationResult>(sources);
        if (!sourceResult.Succeeded)
        {
            return EngineResult<NativeEditMutationResult>.Failure(sourceResult.Error!);
        }

        var outputResult = RequireOutput<NativeEditMutationResult>(candidate);
        if (!outputResult.Succeeded)
        {
            return EngineResult<NativeEditMutationResult>.Failure(outputResult.Error!);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var nativeSources = CreateReferenceSources(starfieldSources, starfieldOutput, cancellationToken);
            var reader = new NativeReferenceReader(nativeSources);
            var summaries = new List<FormListSummary>();
            for (var sourceIndex = 0; sourceIndex < nativeSources.Count; sourceIndex++)
            {
                cancellationToken.ThrowIfCancellationRequested();
                var source = nativeSources[sourceIndex];
                if (!IncludesSource(scope, source.Role))
                {
                    continue;
                }

                var mod = sourceIndex < starfieldSources.GetNativeMods().Count
                    ? starfieldSources.GetNativeMods()[sourceIndex]
                    : starfieldOutput.BorrowMod();
                foreach (var formList in mod.FormLists)
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    if (scope == RecordScope.WinningOverrides
                        && HasLaterFormListContext(nativeSources, starfieldSources, starfieldOutput, sourceIndex, formList.FormKey, cancellationToken))
                    {
                        continue;
                    }
    ...
    public EngineResult<NativeRecordRead> ReadFormListContext(
        ...
        CancellationToken cancellationToken)
    {
        var sourceResult = RequireSources<NativeRecordRead>(sources);
        if (!sourceResult.Succeeded)
        {
            return EngineResult<NativeRecordRead>.Failure(sourceResult.Error!);
        }
        ...
        var readerResult = CreateReader<NativeRecordRead>(sources, output, cancellationToken);
        if (!readerResult.Succeeded)
        {
            return Failure<NativeRecordRead>(sourceResult.Value!, readerResult.Error!);
        }
        ...
        return Success(
            sourceResult.Value!,
            new NativeRecordRead(unsupported, read.RecordType, null),
            readResult.Warnings);
    }
    ...
    public async ValueTask<EngineResult<NativeStagedOutputSet>> WriteAndValidateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var sourceResult = RequireSources<NativeStagedOutputSet>(sources);
        if (!sourceResult.Succeeded)
        {
            return EngineResult<NativeStagedOutputSet>.Failure(sourceResult.Error!);
        }

        var outputResult = RequireOutput<NativeStagedOutputSet>(output);
        if (!outputResult.Succeeded)
        {
            return EngineResult<NativeStagedOutputSet>.Failure(outputResult.Error!);
        }
    ...
    /// <summary>Creates a complete output-aware native reference reader over borrowed Starfield state.</summary>
    /// <typeparam name="T">The eventual adapter result type used for typed failure construction.</typeparam>
    /// <param name="sources">The opaque native source handle.</param>
    /// <param name="output">The optional opaque output handle.</param>
    /// <param name="cancellationToken">A token observed while borrowing native state.</param>
    /// <returns>The reader or a typed game-state mismatch.</returns>
    private static EngineResult<NativeReferenceReader> CreateReader<T>(
        INativeSourceSet sources,
        INativeOutputState? output,
        CancellationToken cancellationToken)
    {
        ...
        if (!states.Succeeded)
        {
            return EngineResult<NativeReferenceReader>.Failure(states.Error!);
        }
    ...
    private static IReadOnlyList<NativeReferenceSource> CreateReferenceSources(
    ...
    /// <param name="nativeSources">The ordered native source descriptors.</param>
    /// <param name="sources">The borrowed Starfield source set.</param>
    ...
    private static bool HasLaterFormListContext(
        IReadOnlyList<NativeReferenceSource> nativeSources,
        StarfieldNativeSourceSet sources,
        StarfieldNativeOutputState output,
        int sourceIndex,
        FormKey formKey,
        CancellationToken cancellationToken)
    {
        var sourceMods = sources.GetNativeMods();
        for (var laterIndex = sourceIndex + 1; laterIndex < nativeSources.Count; laterIndex++)
        {
            cancellationToken.ThrowIfCancellationRequested();
    ...
    /// <summary>Creates a detached complete Starfield native record.</summary>
    /// <param name="record">The borrowed record selected by the shared reader.</param>
    /// <returns>An independent Starfield native record copy.</returns>
    /// <exception cref="InvalidOperationException">Thrown when the record is not a Starfield native family.</exception>
    ...
    private static EngineResult<(StarfieldNativeSourceSet Sources, StarfieldNativeOutputState? Output)> RequireStates<T>(
        INativeSourceSet sources,
        INativeOutputState? output)
    {
        ...
        if (!sourceResult.Succeeded)
        {
            return EngineResult<(StarfieldNativeSourceSet Sources, StarfieldNativeOutputState? Output)>.Failure(sourceResult.Error!);
        }

        if (output is not null && output is not StarfieldNativeOutputState)
        {
            return EngineResult<(StarfieldNativeSourceSet Sources, StarfieldNativeOutputState? Output)>.Failure(
                new EngineError(EngineErrorCode.InvalidRequest, "The Starfield adapter requires Starfield native output state."));
        }

        return EngineResult<(StarfieldNativeSourceSet Sources, StarfieldNativeOutputState? Output)>.Success(
            (sourceResult.Value!, output as StarfieldNativeOutputState));
    }
    ...
    private static EngineResult<StarfieldNativeSourceSet> RequireSources<T>(INativeSourceSet sources)
    {
        ArgumentNullException.ThrowIfNull(sources);
        return sources is StarfieldNativeSourceSet starfieldSources
            ? EngineResult<StarfieldNativeSourceSet>.Success(starfieldSources)
            : EngineResult<StarfieldNativeSourceSet>.Failure(new EngineError(
                EngineErrorCode.InvalidRequest,
                "The Starfield adapter requires Starfield native source state."));
    }
    ...
    private static EngineResult<StarfieldNativeOutputState> RequireOutput<T>(INativeOutputState output)
    {
        ArgumentNullException.ThrowIfNull(output);
        return output is StarfieldNativeOutputState starfieldOutput
            ? EngineResult<StarfieldNativeOutputState>.Success(starfieldOutput)
            : EngineResult<StarfieldNativeOutputState>.Failure(new EngineError(
                EngineErrorCode.InvalidRequest,
                "The Starfield adapter requires Starfield native output state."));
    }
    ...
    /// <typeparam name="T">The native reader result type.</typeparam>
    /// <param name="sources">The typed Starfield source lifetime.</param>
