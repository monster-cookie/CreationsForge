...
/// <summary>
/// Admits, clones, and begins transactional edits against complete Starfield native output state.
/// </summary>
public sealed class StarfieldNativeOutputService
{
    /// <summary>The shared guarded output admission boundary.</summary>
    private readonly NativeOutputInputLoader InputLoader;

    /// <summary>
    /// Initializes the Starfield native output service.
    /// </summary>
    ...
    public StarfieldNativeOutputService(NativeOutputInputLoader inputLoader)
    {
        ArgumentNullException.ThrowIfNull(inputLoader);
        InputLoader = inputLoader;
        Inspector = new StarfieldFormListNativeInspector();
    }

    /// <summary>Gets the stateless complete Starfield FormList inspector used for detached snapshots.</summary>
    public StarfieldFormListNativeInspector Inspector { get; }
    ...
    /// <param name="cancellationToken">A token that cancels admission or native parsing before publication.</param>
    ...
    public Task<EngineResult<NativeOutputOpenResult>> OpenAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return Task.FromResult(EngineResult<NativeOutputOpenResult>.Failure(new EngineError(
    ...
    /// <summary>
    /// Creates an independently disposable deep native copy of complete staged and original Starfield output state.
    /// </summary>
    /// <param name="output">The current complete Starfield output.</param>
    /// <param name="cancellationToken">A token checked immediately before and after each uninterruptible native copy.</param>
    /// <returns>A complete transactional candidate preserving every edited and unrelated native record.</returns>
    ...
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken"/> is canceled.</exception>
    public StarfieldNativeOutputState Clone(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cancellationToken.ThrowIfCancellationRequested();
        return new StarfieldNativeOutputState(mod, originalMod, output.Association, output.Baseline, provenance);
    }

    /// <summary>
    /// Allocates, overrides, or selects a FormList only inside an unpublished complete Starfield output candidate.
    /// </summary>
    /// <param name="sources">The borrowed native source lifetime used to resolve override origins.</param>
    /// <param name="candidate">The unpublished complete output candidate to mutate.</param>
    /// <param name="request">The guarded native edit-target request.</param>
    /// <param name="cancellationToken">A token checked throughout selection and around native mutation.</param>
    /// <returns>The newly assigned stable edit identity, or a typed selection or validation failure.</returns>
    /// <exception cref="ArgumentNullException">Thrown when a required argument is <see langword="null"/>.</exception>
    /// <exception cref="ObjectDisposedException">Thrown after a borrowed native lifetime is disposed.</exception>
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken"/> is canceled.</exception>
    ...
    /// <param name="sources">The borrowed Starfield source lifetime.</param>
    /// <param name="association">The requested output identity and native formatting choices.</param>
    ...
    /// <param name="cancellationToken">A token observed until a complete native state is ready to publish.</param>
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (inputs.Release != GameRelease.Starfield)
            {
                return EngineResult<NativeOutputOpenResult>.Failure(new EngineError(
                    ...
                    $"Starfield native output loading does not support release {inputs.Release}."));
            }
            ...
            {
                return EngineResult<NativeOutputOpenResult>.Failure(validationError);
            }
            ...
            var state = new StarfieldNativeOutputState(mod, originalMod, canonicalAssociation, baseline);
            var result = new NativeOutputOpenResult(state, canonicalAssociation, baseline);
            cancellationToken.ThrowIfCancellationRequested();
        ...
        catch (ObjectDisposedException exception)
        {
            return EngineResult<NativeOutputOpenResult>.Failure(new EngineError(
                ...
                $"The Starfield native lifetime is unavailable: {exception.Message}"));
        }
        catch (Exception exception)
        {
            return EngineResult<NativeOutputOpenResult>.Failure(new EngineError(
                EngineErrorCode.OutputOpenFailed,
                $"Starfield native output parsing failed: {exception.Message}"));
        }
    }
    ...
    private static StarfieldMod CreateNewMod(NativeOutputPluginInput output)
    {
    ...
    /// <summary>Validates fully materialized native identity, master style, and localization state.</summary>
        ...
        NativeOutputPluginInput output)
    {
        if (mod.ModKey != output.ModKey)
        {
            return new EngineError(
                EngineErrorCode.OutputOpenFailed,
                $"Native Starfield output identity {mod.ModKey} did not match admitted output {output.ModKey}.");
        }
        ...
        if (actualStyle is null || actualStyle.Value != output.MasterStyle)
        {
            return new EngineError(
                EngineErrorCode.OutputOpenFailed,
                $"Native Starfield output {output.ModKey} did not retain the admitted {output.MasterStyle} master style.");
        }
        ...
        if (mod.UsingLocalization != output.UsesLocalization)
        {
            return new EngineError(
                EngineErrorCode.OutputOpenFailed,
                $"Native Starfield output {output.ModKey} did not retain the admitted localization mode.");
        }

        return null;
    }

    /// <summary>Allocates one native FormList through Mutagen and rejects a cross-family FormKey collision before publication.</summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var identity = new NativeEditIdentity(
            Guid.NewGuid(),
            formList.FormKey,
            null,
            FormListEditRole.New);
        candidate.AddProvenanceIfAbsent(new NativeEditProvenance(
            identity.EditId,
            identity.FormKey,
            EditBaselineKind.Absent));
        cancellationToken.ThrowIfCancellationRequested();
        return EngineResult<NativeEditIdentity>.Success(identity);
    }
    ...
    /// <param name="cancellationToken">A token observed during source resolution and around native copying.</param>
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (selected is null)
        {
            ...
            var detail = wrongFamily ? "belongs to another native record family" : "is not contained in the selected output";
            return EngineResult<NativeEditIdentity>.Failure(new EngineError(
                code,
    ...
    /// <returns>Complete metadata that reconstructs the exact native before-view.</returns>
    private static NativeEditProvenance CreateProvenance(
        StarfieldNativeSourceSet sources,
        StarfieldNativeOutputState candidate,
        NativeEditIdentity identity,
        FormListContext? sourceContext)
    {
        ...
        if (original is not null)
        {
            ...
            return new NativeEditProvenance(
                identity.EditId,
                identity.FormKey,
    ...
    /// <param name="record">The original native FormList baseline.</param>
    ...
    /// <summary>Finds one FormList by exact native identity inside a complete Starfield plugin.</summary>
    /// <param name="mod">The complete native plugin to scan.</param>
    ...
    /// <returns>A missing-record error for absence or a validation error for unusable native contexts.</returns>
    private static EngineError CreateOriginSelectionError(FormKey originFormKey, NativeRecordRead read)
    {
    ...
    /// <summary>Gets a singular native master style and rejects contradictory header flags.</summary>
    /// <param name="mod">The complete Starfield plugin.</param>
    /// <returns>The native style, or <see langword="null"/> when multiple style flags are set.</returns>
    ...
    /// <summary>Maps native Mutagen style metadata back to the shared output contract.</summary>
    /// <param name="style">The verified native master style.</param>
    /// <returns>The corresponding shared master style.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown for a native style unsupported by the shared contract.</exception>
