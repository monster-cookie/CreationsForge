...
{
    /// <summary>Builds detached native before-and-after comparisons for every staged Skyrim FormList target.</summary>
    /// <param name="sources">The borrowed immutable source set used for exact source baselines and link resolution.</param>
    /// <param name="output">The borrowed complete staged output and its original native baseline.</param>
        ...
        SkyrimNativeOutputState output)
    {
            ...
            foreach (var provenance in output.GetEditProvenance())
            {
                ...
                for (var index = 0; index < afterDetached.Items.Count; index++)
                {
                    ...
                    var warning = new EngineWarning(
                        ...
                        $"Native reference {item} at {nameof(IFormListGetter.Items)}[{index}] does not resolve to a live record in the current workspace view.");
        ...
        catch (Exception exception)
        {
            return PreviewFailure(
                new EngineError(
                    EngineErrorCode.ValidationFailed,
                    $"Skyrim rejected the native FormList preview: {exception.Message}"),
                Array.Empty<EngineWarning>());
        }
    }

    /// <summary>Creates the exact staged-output context for one current native FormList.</summary>
    /// <param name="sources">The source set establishing output load-order position.</param>
    /// <param name="output">The selected complete native output state.</param>
    ...
    /// <summary>Compares every field of two exact immutable native contexts.</summary>
    ...
    /// <param name="record">The detached native FormList getter.</param>
