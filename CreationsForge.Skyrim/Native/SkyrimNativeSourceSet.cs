...
/// <summary>
/// Owns fully materialized Skyrim Special Edition source plugins and performs bounded native reference operations over them.
/// </summary>
public sealed class SkyrimNativeSourceSet : INativeSourceSet
{
    /// <summary>The shared validated input lifetime, including native resource fingerprints.</summary>
    private readonly NativeSourceInputs _inputs;

    /// <summary>The independently materialized native Skyrim plugins in explicit load-order order.</summary>
    private readonly List<ISkyrimModGetter> _nativeMods;

    /// <summary>The bounded reader that borrows the materialized native plugin handles.</summary>
    private NativeReferenceReader? _referenceReader;

    /// <summary>The immutable native plugin and provenance pairs used by source-only enumeration.</summary>
    private IReadOnlyList<SkyrimNativeFormListSource>? _formListSources;
    ...
    /// <param name="nativeMods">The fully materialized Skyrim plugins in explicit load-order order.</param>
    ...
    internal SkyrimNativeSourceSet(
        Guid workspaceId,
        NativeSourceInputs inputs,
        IReadOnlyList<ISkyrimModGetter> nativeMods,
        NativeSourceInputBaseline baseline)
    {
        if (workspaceId == Guid.Empty)
        {
            throw new ArgumentException("A Skyrim native source lifetime requires a non-empty workspace identifier.", nameof(workspaceId));
        }
        ...
        {
            throw new ArgumentException("Every admitted Skyrim plugin must have exactly one materialized native source.", nameof(nativeMods));
        }

        WorkspaceId = workspaceId;
        _inputs = inputs;
        _nativeMods = nativeMods.ToList();
        Baseline = baseline;
        Revision = new WorkspaceRevision(baseline.BaselineId, 0);
        var formListSources = Array.AsReadOnly(inputs.Plugins
            .Select((plugin, index) => new SkyrimNativeFormListSource(
                _nativeMods[index],
                plugin.Path,
        ...
        _referenceReader = new NativeReferenceReader(
            formListSources
                .Select(source => new NativeReferenceSource(
                    source.Mod,
                    source.Path,
    ...
    /// <summary>
    /// Lists Skyrim FormList contexts directly from native groups in deterministic containing-plugin and record order.
    /// </summary>
    /// <param name="scope">The requested source-only native view.</param>
    /// <param name="cancellationToken">A token observed during native enumeration and override counting.</param>
    /// <returns>Immutable native FormList summaries bound to this workspace and source revision.</returns>
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken"/> is canceled.</exception>
    ...
    /// <param name="scope">The requested native source, output, all-context, or winning view.</param>
    /// <param name="cancellationToken">A token observed during native enumeration and override counting.</param>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var contexts = new List<(SkyrimNativeFormListSource Source, IFormListGetter Record)>();
        var sources = sourcesResult.Value!;
        foreach (var source in sources)
        {
            cancellationToken.ThrowIfCancellationRequested();
    ...
    /// <returns>The contextual FormList read and missing native reference warnings, bound to this workspace and source revision.</returns>
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="request"/> is <see langword="null"/>.</exception>
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken"/> is canceled.</exception>
    internal EngineResult<NativeRecordRead> ReadFormListContext(
        ReferenceRequest request,
        SkyrimNativeOutputState? output,
        CancellationToken cancellationToken)
    {
        ...
        var unsupported = new NativeRecordRead(
            new FormListContext(
                request,
            ...
            read.RecordType,
            null);
        return BindResult(EngineResult<NativeRecordRead>.Success(unsupported, warnings: result.Warnings));
    }
    ...
    /// <summary>Resolves one Skyrim native reference across sources and an optional staged output.</summary>
    /// <param name="request">The exact native identity, scope, and optional containing-plugin selection.</param>
    /// <param name="output">The borrowed staged output appended after sources, or <see langword="null"/>.</param>
    /// <param name="cancellationToken">A token observed during the bounded native scan.</param>
    /// <returns>The detached native resolution or a typed failure.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="request"/> is <see langword="null"/>.</exception>
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken"/> is canceled.</exception>
    ...
    /// <summary>Searches Skyrim native references across sources and an optional staged output.</summary>
        ...
        CancellationToken cancellationToken = default)
    {
        if (Volatile.Read(ref _disposed) != 0)
        {
            return Task.FromResult(EngineResult<NativeSourceInputBaseline>.Failure(
                new EngineError(EngineErrorCode.WorkspaceDisposed, "The Skyrim native source lifetime has been disposed."),
                workspaceId: WorkspaceId,
                resultRevision: Revision));
        }
    ...
    /// <summary>
    /// Gets the same-game native source handles for later Skyrim adapter composition within this assembly.
    /// </summary>
    /// <returns>The immutable ordered collection of owned native Skyrim source getters.</returns>
    /// <exception cref="ObjectDisposedException">Thrown after this source set has been disposed.</exception>
    internal IReadOnlyList<ISkyrimModGetter> GetNativeMods()
    {
        ObjectDisposedException.ThrowIf(Volatile.Read(ref _disposed) != 0, this);
        return _nativeMods.AsReadOnly();
    }

    /// <summary>
    /// Borrows the validated native source inputs for same-game output admission within this assembly.
    /// </summary>
    ...
    /// <summary>
    /// Releases shared input resources and references to the fully materialized native plugins.
    /// </summary>
    ...
    /// <summary>
    /// Captures the borrowed shared reader while its native source references remain valid for the current operation.
    /// </summary>
    ...
    private EngineResult<NativeReferenceReader> GetReferenceReader(SkyrimNativeOutputState? output)
    {
        if (output is null)
        {
        ...
        var referenceSources = sourcesResult.Value!
            .Select(source => new NativeReferenceSource(
                ...
                CreateDetachedSkyrimRecord))
            .ToArray();
        return EngineResult<NativeReferenceReader>.Success(
            new NativeReferenceReader(referenceSources, EnumerateSkyrimDirectLinks));
    }

    /// <summary>
    /// Captures the immutable plugin and provenance snapshot while native source references remain valid.
    /// </summary>
    /// <returns>The source snapshot or a typed disposed failure.</returns>
    private EngineResult<IReadOnlyList<SkyrimNativeFormListSource>> GetFormListSources()
    {
        var sources = Volatile.Read(ref _formListSources);
        if (Volatile.Read(ref _disposed) != 0 || sources is null)
        {
            return EngineResult<IReadOnlyList<SkyrimNativeFormListSource>>.Failure(
                new EngineError(EngineErrorCode.WorkspaceDisposed, "The Skyrim native source lifetime has been disposed."),
                workspaceId: WorkspaceId,
                resultRevision: Revision);
        }

        return EngineResult<IReadOnlyList<SkyrimNativeFormListSource>>.Success(sources);
    }
        ...
        SkyrimNativeOutputState? output)
    {
        ...
        try
        {
            ...
            sources.Add(new SkyrimNativeFormListSource(
                output.GetMutableMod(),
                output.Association.PluginPath,
                sources.Count,
                PluginRole.Output));
            return EngineResult<IReadOnlyList<SkyrimNativeFormListSource>>.Success(
    ...
    /// <summary>
    /// Finds winning FormList positions while preserving the original containing-plugin and native group order.
    /// </summary>
    /// <param name="contexts">Every native FormList context in explicit load-order and group order.</param>
    ...
    private static IReadOnlySet<int> GetWinningContextIndices(
        IReadOnlyList<(SkyrimNativeFormListSource Source, IFormListGetter Record)> contexts,
        CancellationToken cancellationToken)
    {
    ...
    /// <summary>
    /// Determines whether one native FormList context belongs in the requested view.
    /// </summary>
    ...
    /// <summary>
    /// Creates and validates a complete detached Skyrim record rather than accepting another game's native subtype.
    /// </summary>
    /// <param name="record">The borrowed native Skyrim record selected by the shared reader.</param>
    /// <returns>A complete detached getter retaining its concrete Skyrim record family and fields.</returns>
    /// <exception cref="InvalidOperationException">Thrown when the source or copied record is not a Skyrim native record.</exception>
    private static IMajorRecordGetter CreateDetachedSkyrimRecord(IMajorRecordGetter record)
    {
        ...
        {
            throw new InvalidOperationException($"Record {record.FormKey} is not a Skyrim native record.");
        }

        var copy = skyrimRecord.DeepCopy();
        if (copy is not ISkyrimMajorRecordGetter)
        {
            throw new InvalidOperationException($"Detached record {record.FormKey} did not retain its Skyrim native record family.");
        }
    ...
    /// </summary>
    /// <param name="record">The selected borrowed native record.</param>
    /// <returns>Immutable non-null direct FormList links in native item order, or an empty collection for another record family.</returns>
    private static IReadOnlyList<NativeFormLinkReference> EnumerateSkyrimDirectLinks(IMajorRecordGetter record)
    {
        ...
        for (var index = 0; index < formList.Items.Count; index++)
        {
            ...
            references.Add(new NativeFormLinkReference(formKey, $"{nameof(IFormListGetter.Items)}[{index}]"));
        }
    ...
    /// </summary>
    private sealed class SkyrimNativeFormListSource
    {
        /// <summary>
        /// Initializes one borrowed native Skyrim plugin context.
        /// </summary>
        /// <param name="mod">The fully materialized native plugin.</param>
        ...
        internal SkyrimNativeFormListSource(
        ...
        /// <summary>Gets the borrowed fully materialized native Skyrim plugin.</summary>
        internal ISkyrimModGetter Mod { get; }

        /// <summary>Gets the canonical native plugin path.</summary>
