...
/// <summary>
/// Implements guarded private Skyrim serialization and strict native reopen validation for the output service.
/// </summary>
public sealed partial class SkyrimNativeOutputService
{
    /// <summary>
    /// Writes a complete private Skyrim output set, reopens it strictly, and proves native and FormList semantic equality before destination commit.
    /// </summary>
    ...
    public async Task<EngineResult<NativeStagedOutputSet>> WriteAndValidateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!freshness.Succeeded || freshness.Value?.Output is not SkyrimNativeOutputState freshOutput)
        {
        ...
        if (existingOutput && AreNativeOutputsEquivalent(candidate, original, cancellationToken))
        {
            return WriteSuccess(
                sources,
            ...
            && output.Association.LocalizedOutputMode == LocalizedOutputMode.SeparateStringFiles)
        {
            return WriteFailure(
                ...
                "An existing localized Skyrim output cannot be materially rewritten without losing native localized-string identity.",
                freshness.Warnings);
        }
        ...
        SkyrimNativeOutputState? stagedState = null;
        try
        {
            cancellationToken.ThrowIfCancellationRequested();
            ...
            var stagedOpen = await OpenCoreAsync(
                ...
                cancellationToken).ConfigureAwait(false);
            if (!stagedOpen.Succeeded || stagedOpen.Value?.Output is not SkyrimNativeOutputState openedState)
            {
            ...
            var stagedLifetime = new SkyrimNativeStagedOutputSet(stagedState);
            stagedState = null;
            return WriteSuccess(
                sources,
                new NativeStagedOutputSet(
        ...
        catch (Exception exception)
        {
            return WriteFailure(
                sources,
                EngineErrorCode.ValidationFailed,
                $"Skyrim private output writing or native validation failed: {exception.Message}",
                freshness.Warnings);
        }
        finally
        {
    ...
    /// <param name="mod">The complete staged native output to serialize.</param>
    ...
    /// <param name="cancellationToken">A token checked around the synchronous native writer.</param>
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken"/> is canceled.</exception>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var masterFlags = new Cache<IModMasterStyledGetter, ModKey>(nativeMod => nativeMod.ModKey);
        ...
        if (!actualMasters.SequenceEqual(retainedMasters))
        {
            throw new InvalidOperationException(
                "The normalized Skyrim master list changed before native serialization.");
        }
        ...
        if (association.LocalizedOutputMode == LocalizedOutputMode.SeparateStringFiles)
        {
            ...
            using (var stringsWriter = new StringsWriter(
                ...
                association.ModKey,
                stringsDirectory,
                new SkyrimNativeEncodingProvider(),
                new FileSystem()))
            {
        ...
        CancellationToken cancellationToken)
    {
            ...
            || candidate.UsingLocalization != expectedLocalization)
        {
            return (
                EngineErrorCode.ValidationFailed,
                "The complete Skyrim candidate identity or native header flags differ from the selected output.");
        }
        ...
        foreach (var record in candidate.EnumerateMajorRecords())
        {
            ...
            if (!allKeys.Add(record.FormKey))
            {
                return (
                    EngineErrorCode.ValidationFailed,
                    $"Skyrim candidate identity '{record.FormKey}' is duplicated across native record families.");
            }
        }
        ...
        foreach (var entry in provenance)
        {
            ...
            if (matches.Length != 1 || matches[0] is not IFormListGetter)
            {
                return (
                    EngineErrorCode.ValidationFailed,
                    $"Skyrim edit provenance target '{entry.TargetFormKey}' is missing, duplicated, or belongs to another native family.");
            }
    ...
    /// <summary>Validates complete native equality, exact master retention, and all-field FormList equality after strict reopen.</summary>
    ...
    /// <param name="cancellationToken">A token observed throughout native comparison.</param>
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!SkyrimModMixIn.Equals(expected, reopened, equalsMask: null))
        {
            return (
                EngineErrorCode.ValidationFailed,
                "The staged Skyrim output changed one or more complete native fields during serialization or strict reopen.");
        }
    ...
    /// <summary>Compares complete native state and every FormList field through the shared Skyrim inspector.</summary>
    /// <param name="expected">The complete expected native output.</param>
    /// <param name="actual">The complete actual native output.</param>
    ...
    private bool AreNativeOutputsEquivalent(
    ...
    /// <summary>Compares every FormList in native order through the explicit all-field inspector.</summary>
    ...
    /// <summary>Rejects unrelated native records from the bounded new localized-output preservation surface.</summary>
    /// <param name="mod">The complete new localized output.</param>
    /// <param name="cancellationToken">A token observed for every native record.</param>
    /// <returns><see langword="true"/> when every native major record is a supported FormList.</returns>
    ...
    /// <param name="baseline">The complete artifact baseline captured by strict native staged-output admission.</param>
    ...
    private static bool ContainsOnlyEmptyLocalizedTable(OutputArtifactSetBaseline baseline)
    {
        var sidecars = baseline.Artifacts
            .Where(artifact => artifact.Role != NativeArtifactRole.Plugin && artifact.Fingerprint.Exists)
            .ToArray();
        return sidecars.Length == 1
            && sidecars[0].Role == NativeArtifactRole.Strings
    ...
    private static EngineResult<IReadOnlyList<NativeStagedArtifactMapping>> CreateArtifactMappings(
        ...
        string destinationPluginPath)
    {
        ...
        if (Directory.EnumerateFileSystemEntries(stagingDirectoryPath).Any())
        {
            return (
                EngineErrorCode.InvalidRequest,
                "The private Skyrim staging directory must be empty before native writing begins.");
        }
    ...
    private static EngineResult<NativeStagedOutputSet> WriteSuccess(
        SkyrimNativeSourceSet sources,
    ...
    private static EngineResult<NativeStagedOutputSet> WriteFailure(
        SkyrimNativeSourceSet sources,
    ...
    private sealed class SkyrimNativeEncodingProvider : IMutagenEncodingProvider
    {
        /// <summary>Gets the native encoding registered for one Skyrim release and language.</summary>
        /// <param name="release">The native game release.</param>
        /// <param name="language">The localized strings language.</param>
        /// <returns>The registered Mutagen native encoding.</returns>
