# CreationsForge.Skyrim rules

This folder contains Skyrim-specific Mutagen mapping, DTOs, services, repositories, importers, factories, and record support.

## Boundaries

- Keep Skyrim-specific Mutagen APIs and record quirks in this project.
- Do not leak Skyrim-only assumptions into CreationsForge.Core.
...
- Inspect the installed Mutagen package, current repository usage, official Mutagen documentation/source, and Spriggit
  output before choosing APIs.
...
- If Skyrim differs from another game's record shape, preserve the difference in the game-specific DTO/schema/service rather than forcing a false shared abstraction.
