...
/// <summary>
/// Owns one complete mutable Skyrim output and an independent Mutagen baseline captured when the output was selected.
/// </summary>
public sealed class SkyrimPluginOutputState : IPluginOutputState
{
    /// <summary>The complete mutable Skyrim output, or <see langword="null"/> after disposal.</summary>
    private SkyrimMod? _mod;

    /// <summary>The independent complete Mutagen baseline, or <see langword="null"/> after disposal.</summary>
    ...
    /// <summary>Tracks whether this independently owned Mutagen state has been released.</summary>
    ...
    /// <param name="original">An independent complete Mutagen baseline for preview and preservation checks.</param>
    ...
    /// <exception cref="ArgumentException">Thrown when a Mutagen mod identity disagrees with the selected output or the Mutagen instances are aliased.</exception>
    /// <exception cref="ArgumentNullException">Thrown when any argument is <see langword="null"/>.</exception>
    internal SkyrimPluginOutputState(
        SkyrimMod mod,
        SkyrimMod original,
        ...
        IReadOnlyList<RecordEditProvenance>? editProvenance = null)
    {
        ...
        {
            throw new ArgumentException("A Skyrim output requires an independent original Mutagen baseline.", nameof(original));
        }
        ...
        Baseline = baseline;
        _editProvenance = editProvenance?.ToList() ?? new List<RecordEditProvenance>();
    }
    ...
    /// <summary>
    /// Creates a defensive complete Mutagen copy of the current staged output for inspection.
    /// </summary>
    /// <param name="cancellationToken">A token checked immediately before and after the uninterruptible Mutagen copy.</param>
    ...
    /// <summary>
    /// Creates a defensive complete Mutagen copy of the output state captured when selection completed.
    /// </summary>
    /// <param name="cancellationToken">A token checked immediately before and after the uninterruptible Mutagen copy.</param>
    ...
    /// <returns>A candidate with independent current and original Mutagen state.</returns>
    /// <exception cref="ObjectDisposedException">Thrown after this output state has been disposed.</exception>
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken"/> is canceled.</exception>
    internal SkyrimPluginOutputState Clone(CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var mod = CopyMod(GetMod(), cancellationToken);
        var original = CopyMod(GetOriginal(), cancellationToken);
        return new SkyrimPluginOutputState(
            mod,
            original,
            Association,
            Baseline,
    ...
    /// <exception cref="ObjectDisposedException">Thrown after this output state has been disposed.</exception>
    internal bool RegisterEditProvenance(RecordEditProvenance provenance)
    {
    ...
    /// <summary>
    /// Borrows the complete original Mutagen baseline for synchronous same-assembly adapter operations.
    /// </summary>
    /// <returns>The live original Mutagen baseline owned by this state.</returns>
    /// <exception cref="ObjectDisposedException">Thrown after this output state has been disposed.</exception>
    ...
    /// <summary>
    /// Releases references to this independently owned materialized Mutagen state.
    /// </summary>
    ...
    /// <summary>Gets the current Mutagen mod or throws after disposal.</summary>
    ...
    /// <summary>Gets the original Mutagen mod or throws after disposal.</summary>
    ...
    /// <summary>Copies every reachable record field into an independent mutable Skyrim mod.</summary>
    /// <param name="source">The complete record getter to copy.</param>
