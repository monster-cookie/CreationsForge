...
/// <summary>Verifies native editor controls resynchronize after inactive tab detachment.</summary>
[Collection(AvaloniaControlTestCollection.Name)]
public sealed class NativeFormListEditorViewHeadlessTests
{
    ...
    [AvaloniaFact]
    public async Task NativeFormListEditorView_WhenDetachedStateChanges_ReattachesToCurrentSnapshot()
    {
        ...
        using var editor = new NativeFormListEditorViewModel(
            coordinator,
            host,
            new NativeWorkspacePresentationOperationArbiter(),
        ...
        var view = new NativeFormListEditorView(editor);
        ...
        try
        {
            ...
            editor.CreateDraft(setEditorId, NativeFormListDraftSeedSelection.CurrentValue()).Succeeded.ShouldBeTrue();
            Dispatcher.UIThread.RunJobs();
            ControlFinder.FindByAutomationId<ComboBox>(view, "NativeFormListCommandGroupSelector")!
            ...
            editor.CreateDraft(clearEditorId, NativeFormListDraftSeedSelection.CurrentValue()).Succeeded.ShouldBeTrue();

            window.Content = view;
            Dispatcher.UIThread.RunJobs();
            ControlFinder.FindByAutomationId<ComboBox>(view, "NativeFormListCommandGroupSelector")!
                .SelectedItem.ShouldBe("Identity");
            ControlFinder.FindByAutomationId<ComboBox>(view, "NativeFormListCommandSelector")!
            ...
            var draftResult = editor.CreateDraft(setFormVersion, NativeFormListDraftSeedSelection.CurrentValue());
            draftResult.Succeeded.ShouldBeTrue();
            var root = draftResult.Value!.Root.ShouldBeOfType<NativeWireObjectDraftNode>();
            var formVersion = root.FindProperty("formVersion").ShouldBeOfType<NativeWireIntegerDraftNode>();
            formVersion.Text = "-1";
            ...
            var groupSelector = ControlFinder.FindByAutomationId<ComboBox>(
                view,
                "NativeFormListCommandGroupSelector").ShouldNotBeNull();
            groupSelector.SelectedItem.ShouldBe("Record");
            var commandSelector = ControlFinder.FindByAutomationId<ComboBox>(
                view,
                "NativeFormListCommandSelector").ShouldNotBeNull();
            commandSelector.SelectedItem.ShouldBeSameAs(setFormVersion);
            ControlFinder.FindByAutomationId<TextBox>(view, "NativeWireInteger:$.formVersion")!
                .Text.ShouldBe("-1");
            ControlFinder.FindByAutomationId<ItemsControl>(
                view,
                "NativeFormListEditorValidationIssues").ShouldNotBeNull().ItemCount.ShouldBeGreaterThan(0);
            ControlFinder.FindByAutomationId<Button>(view, "NativeFormListEditorApplyButton")!
                .IsEnabled.ShouldBeFalse();
            ControlFinder.FindByAutomationId<Button>(view, "NativeFormListDiscardFormChangesButton")!
    ...
    private static string GetValidationScreenshotPath()
    {
        return Path.GetFullPath(Path.Combine(
            ...
            "NativeReplacement",
            "screenshots",
            "native-form-list-editor-validation.png"));
    }

    /// <summary>Provides the editor's selection and deterministic refresh boundary.</summary>
    private sealed class ReattachEditorHost : INativeFormListEditorHost
    {
        /// <inheritdoc />
        public event PropertyChangedEventHandler? PropertyChanged;

        /// <inheritdoc />
        public NativeFormListEditorSelection? Selection => null;
