...
/// <summary>Verifies the production native changes dialog owns asynchronous modal work and queued close requests on Avalonia's UI thread.</summary>
[Collection(AvaloniaControlTestCollection.Name)]
public sealed class NativeWorkspaceChangesDialogServiceHeadlessTests
{
    ...
    public Task ReviewChangesAsync_DelayedPreview_ShowsRealModalAndReleasesTransition()
    {
        return VerifyToolbarModalAsync(
            ...
            NativeWorkspaceChangesDialogPurpose.Review,
            "Done");
    }
    ...
    public Task ShowSaveChangesDialogAsync_DelayedPreview_ShowsRealModalAndReleasesTransition()
    {
        return VerifyToolbarModalAsync(
            ...
            NativeWorkspaceChangesDialogPurpose.Save,
            "Keep Editing");
    }
    ...
    public Task ShowDiscardChangesDialogAsync_DelayedPreview_ShowsRealModalAndReleasesTransition()
    {
        return VerifyToolbarModalAsync(
            ...
            NativeWorkspaceChangesDialogPurpose.Discard,
            "Keep Editing");
    }
    ...
    public async Task ReserveLeaveAsync_DelayedDirtyPreview_CloseGestureReleasesTransition()
    {
        ...
        var dialogService = new NativeWorkspaceChangesDialogService(
            windowService,
            new LoggerConfiguration().CreateLogger());
        await using var context = NativeWorkspaceChangesTestContext.CreateReadyWithPresentationServices(
            dialogService,
            new AvaloniaUiDispatcher(),
            hasStagedChanges: true);
        var preview = DelayPreview(context);
        Task<NativeWorkspaceLeaveReservation?>? leaveTask = null;
        Window? dialog = null;
    ...
    public async Task ReserveLeaveAsync_ActiveEditor_WindowCloseDuringWaitOverridesProceed()
    {
        ...
        fixture.Context.ViewModel.OperationState.ShouldBe(NativeWorkspaceChangesOperationState.WaitingForEditorOperation);
        fixture.EditorLease.CancellationToken.IsCancellationRequested.ShouldBeFalse();
        fixture.Dialog.Close();
    ...
    public async Task ReserveLeaveAsync_ActiveEditor_CallerCancellationDuringCancelAndWaitDrainsThenKeepsEditing()
    {
        ...
        fixture.Context.ViewModel.OperationState.ShouldBe(NativeWorkspaceChangesOperationState.CancelRequestedWaitingForResult);
        callerCancellation.Cancel();
    ...
    private static async Task VerifyToolbarModalAsync(
        Func<NativeWorkspaceChangesViewModel, CancellationToken, Task> showDialogAsync,
        NativeWorkspaceChangesDialogPurpose expectedPurpose,
    ...
    private static DelayedPreview DelayPreview(NativeWorkspaceChangesTestContext context)
    {
    ...
    /// <returns>The real native workspace changes view.</returns>
    private static NativeWorkspaceChangesView AssertProductionDialog(
        Window dialog,
        ...
        HeadlessApplicationWindowService windowService)
    {
        ...
        var view = dialog.Content.ShouldBeOfType<NativeWorkspaceChangesView>();
        view.DataContext.ShouldBeSameAs(viewModel);
        ControlFinder.FindByAutomationId<NativeWorkspaceChangesView>(
            view,
            "NativeWorkspaceChangesView").ShouldBeSameAs(view);
        return view;
    }
    ...
    private static void AssertTransitionReleased(NativeWorkspaceChangesTestContext context)
    {
    ...
    private static string ExpectedPurposeText(NativeWorkspaceChangesDialogPurpose purpose)
    {
    ...
    {
        if (dialog?.IsVisible != true || dialog.Content is not NativeWorkspaceChangesView view)
        {
            return;
        }

        var keepEditing = ControlFinder.FindByAutomationId<Button>(view, "NativeWorkspaceKeepEditingButton");
        keepEditing?.RaiseEvent(new RoutedEventArgs(Button.ClickEvent));
    }
    ...
    private sealed class ActiveEditorDialogFixture : IAsyncDisposable
    {
        ...
        /// <param name="context">The complete native changes presentation graph.</param>
        ...
        private ActiveEditorDialogFixture(
            ...
            NativeWorkspaceEditorOperationLease editorLease,
            Window dialog,
            NativeWorkspaceChangesView view,
            Task<NativeWorkspaceLeaveReservation?> leaveTask)
        {
            Owner = owner;
            Context = context;
        ...
        /// <summary>Gets the complete native changes presentation graph.</summary>
        internal NativeWorkspaceChangesTestContext Context { get; }

        /// <summary>Gets the exact editor operation captured by the pending transition.</summary>
        internal NativeWorkspaceEditorOperationLease EditorLease { get; }
        ...
        internal NativeWorkspaceChangesView View { get; }

        /// <summary>Gets the active public leave request.</summary>
        internal Task<NativeWorkspaceLeaveReservation?> LeaveTask { get; }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var dialogService = new NativeWorkspaceChangesDialogService(
                windowService,
                new LoggerConfiguration().CreateLogger());
            var context = NativeWorkspaceChangesTestContext.CreateReadyWithPresentationServices(
                dialogService,
                new AvaloniaUiDispatcher());
