...
/// <summary>
/// Verifies native browser ordering, exact selections, typed outcomes, retry, picker, and generation safety.
/// </summary>
public sealed partial class NativeFormListBrowserViewModelTests
{
    ...
    public async Task StartAndSelectRecord_AllContexts_PreserveOrderProvenanceAndDetachedComparison()
    {
        ...
        afterRoot.Children.Select(node => node.Name)
            .ShouldBe(["$type", "Items", "Name"]);
        beforeRoot.ComparisonState.ShouldBe(NativeComparisonFieldState.Conflict);
        afterRoot.ComparisonState.ShouldBe(NativeComparisonFieldState.WinningOverride);
        beforeRoot.Children.Single(node => node.Name == "$type").ComparisonState
            .ShouldBe(NativeComparisonFieldState.Identical);
        ...
        beforeRoot.Children.Single(node => node.Name == "Name").Children.ShouldHaveSingleItem().ComparisonState
            .ShouldBe(NativeComparisonFieldState.Conflict);
        afterRoot.Children.Single(node => node.Name == "Name").Children.ShouldHaveSingleItem().ComparisonState
            .ShouldBe(NativeComparisonFieldState.WinningOverride);
    ...
    /// <summary>Verifies all native reference resolution outcomes remain explicit and only inspectable outcomes carry JSON.</summary>
    ...
    public async Task WorkspaceReplacement_CancelsAndSuppressesStaleLoad()
    {
        ...
        var firstWorkspace = new RecordingFormListBrowserWorkspace(
            ...
            async token =>
            {
                ...
                throw new InvalidOperationException("Canceled native state read resumed unexpectedly.");
            },
    ...
    public async Task WorkspaceClose_ClearsEveryDerivedRecordAndJsonValue()
    {
        ...
        viewModel.StatusText.ShouldBe("No native workspace is open.");
    }
    ...
    public async Task Refresh_CanceledBeforeQueuedPublication_DoesNotPublishSnapshotOrReselect()
    {
        ...
        viewModel.StatusText.ShouldBe("Native FormList refresh canceled.");
    }
    ...
    public async Task ContextChange_CancelsAndSuppressesPriorComparisonGeneration()
    {
        ...
        viewModel.AfterFields.ShouldHaveSingleItem()
            ...
            .ComparisonState.ShouldBe(NativeComparisonFieldState.Conflict);
    }

    /// <summary>Verifies a changed live revision rejects comparison before native record traversal.</summary>
    ...
    public async Task SelectRecord_WhenLiveRevisionChanged_ReportsTypedConflictBeforeComparison()
    {
        ...
        var workspace = new RecordingFormListBrowserWorkspace(
            ...
            {
                compareCalled = true;
                throw new InvalidOperationException("Revision guard failed to stop native comparison traversal.");
            });
        var coordinator = new RecordingNativeWorkspaceCoordinator();
    ...
    public async Task PickReference_ExactCurrentSelection_DisplaysAnyFamilyAndNavigatesOnlyFormList()
    {
        ...
        var workspace = CreateWorkspace(
            ...
            request => SuccessfulComparison(
                ...
                sourcePath,
                []));
        NativeReferencePickerSelection? nextSelection = new(
            workspaceId,
            revision,
            isNull: false,
            match: new ReferenceSearchMatch(
                ...
                0,
                PluginRole.Source));
        var picker = new RecordingNativeReferencePickerService((_, _) =>
            Task.FromResult<NativeReferencePickerSelection?>(nextSelection));
        var coordinator = new RecordingNativeWorkspaceCoordinator();
        coordinator.Publish(CreateDescriptor(workspaceId, revision), workspace);
        ...
        nextSelection = new NativeReferencePickerSelection(
            workspaceId,
            revision,
            isNull: false,
            match: new ReferenceSearchMatch(
    ...
    public async Task PickReference_StaleRevision_IsIgnored()
    {
        ...
        var workspace = CreateWorkspace(
            ...
            _ => throw new NotSupportedException());
        var picker = new RecordingNativeReferencePickerService((_, _) => Task.FromResult<NativeReferencePickerSelection?>(new(
            workspaceId,
            revision.Next(),
            isNull: false,
    ...
    /// <param name="overrideCount">The native override count.</param>
    ...
    private static NativeWorkspaceDescriptor CreateDescriptor(Guid workspaceId, WorkspaceRevision revision)
    {
    ...
    private static NativeFormListBrowserViewModel CreateViewModel(RecordingNativeWorkspaceCoordinator coordinator)
    {
        var picker = new RecordingNativeReferencePickerService();
        var dispatcher = new InlineUiDispatcher();
        var operationArbiter = new NativeWorkspacePresentationOperationArbiter();
        return new NativeFormListBrowserViewModel(
            coordinator,
            operationArbiter,
            new NativeJsonTreeProjectionService(),
            picker,
