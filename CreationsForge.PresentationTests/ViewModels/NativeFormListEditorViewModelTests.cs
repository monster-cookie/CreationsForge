...
public sealed class NativeFormListEditorViewModelTests
{
    ...
    public async Task BeginOverride_ForwardsCapturedSelectionRevisionAndCapturesReceiptSeed()
    {
        ...
        fixture.Host.PublishSelection(new NativeFormListEditorSelection(
            fixture.WorkspaceId,
            selectedRevision,
            fixture.FormKey,
            exactSelection,
    ...
    public async Task RetryPendingBegin_KnownSuccessCreatesInitialDraft(FormListEditRole role)
    {
        ...
        editor.OperationState.ShouldBe(NativeFormListEditorOperationState.PendingOutcome);
        ...
        editor.OperationState.ShouldBe(NativeFormListEditorOperationState.Idle);
    ...
    public async Task Apply_KnownSuccessWithPostReadFailures_AdvancesWithoutReplay()
    {
        ...
        var staleSeedAttempt = editor.CreateDraft(seededCommand, NativeFormListDraftSeedSelection.CurrentValue());
    ...
    public async Task ChangedDraft_BlocksEveryBeginAction()
    {
        ...
        var draftResult = editor.CreateDraft(command, NativeFormListDraftSeedSelection.CurrentValue());
        draftResult.Succeeded.ShouldBeTrue();
        var root = draftResult.Value!.Root.ShouldBeOfType<NativeWireObjectDraftNode>();
    ...
    /// <summary>Verifies same-command and different-command draft replacement require explicit local discard without changing native session or staged state.</summary>
    ...
    public async Task CreateDraft_ChangedDraftRequiresExplicitDiscardBeforeReplacement()
    {
        ...
        var firstResult = editor.CreateDraft(firstCommand, NativeFormListDraftSeedSelection.CurrentValue());
        var firstDraft = firstResult.Value.ShouldNotBeNull();
        var receiptSeed = firstDraft.Seed.ShouldNotBeNull();
        var root = firstDraft.Root.ShouldBeOfType<NativeWireObjectDraftNode>();
        var editorId = root.FindProperty("editorId").ShouldBeOfType<NativeWireStringDraftNode>();
        editorId.Value = "PreserveThisValue";

        var sameCommandAttempt = editor.CreateDraft(firstCommand, NativeFormListDraftSeedSelection.CurrentValue());
        AssertChangedDraftReplacementRejected(editor, sameCommandAttempt, firstDraft, firstCommand, editorId, session, revision);

        var differentCommandAttempt = editor.CreateDraft(differentCommand, NativeFormListDraftSeedSelection.CurrentValue());
        ...
        editor.HasError.ShouldBeFalse();
        editor.StatusText.ShouldContain("Native staged changes remain unchanged");

        var nextResult = editor.CreateDraft(differentCommand, NativeFormListDraftSeedSelection.CurrentValue());
    ...
    public async Task DiscardFormChanges_RunningApplyIsDisabledAndPreservesDraft()
    {
        ...
        var root = draft.Root.ShouldBeOfType<NativeWireObjectDraftNode>();
        root.FindProperty("editorId").ShouldBeOfType<NativeWireStringDraftNode>().Value = "BusyValue";
        var session = editor.Session.ShouldNotBeNull();
        var revision = session.ExpectedRevision;
    ...
    public async Task WorkspaceTransition_WaitsForKnownApplyPublicationAndCoordinatorExit()
    {
        ...
        draft.Root.ShouldBeOfType<NativeWireObjectDraftNode>()
            .FindProperty("editorId").ShouldBeOfType<NativeWireStringDraftNode>().Value = "DrainKnownApply";
        var applyTask = editor.ApplyAsync();
        await applyStarted.Task.WaitAsync(TimeSpan.FromSeconds(5));
    ...
    public async Task WorkspaceTransition_CancelAndWaitObservesUncertainApplyBeforeAcquisition()
    {
        ...
        var draft = editor.CreateDraft(command, NativeFormListDraftSeedSelection.CurrentValue()).Value.ShouldNotBeNull();
        draft.Root.ShouldBeOfType<NativeWireObjectDraftNode>()
            .FindProperty("editorId").ShouldBeOfType<NativeWireStringDraftNode>().Value = "DrainUncertainApply";
        var applyTask = editor.ApplyAsync();
        await applyStarted.Task.WaitAsync(TimeSpan.FromSeconds(5));
    ...
    public async Task RetryPendingApply_ReusesExactImmutableOperation()
    {
        ...
        await editor.BeginNewAsync();
        var command = editor.AvailableCommands.Single(candidate => candidate.CommandName == "form-list.set-editor-id");
        var pendingDraft = editor.CreateDraft(command, NativeFormListDraftSeedSelection.CurrentValue()).Value.ShouldNotBeNull();
        var pendingRoot = pendingDraft.Root.ShouldBeOfType<NativeWireObjectDraftNode>();
        pendingRoot.FindProperty("editorId").ShouldBeOfType<NativeWireStringDraftNode>().Value = "PendingValue";
        var expectedRevision = editor.Session!.ExpectedRevision;
    ...
    private static void AssertChangedDraftReplacementRejected(
        NativeFormListEditorViewModel editor,
        EngineResult<NativeFormListDraft> attempt,
        NativeFormListDraft expectedDraft,
        NativeFormListCommandPresentation expectedCommand,
        NativeWireStringDraftNode editorId,
    ...
    private sealed class EditorFixture
    {
        ...
        public EditorFixture()
        {
            ...
            Coordinator = new RecordingNativeWorkspaceCoordinator();
            Coordinator.Publish(CreateDescriptor(WorkspaceId, InitialRevision), Workspace);
            Host = new EditorTestHost();
            OperationArbiter = new NativeWorkspacePresentationOperationArbiter();
        }
        ...
        public RecordingNativeWorkspaceCoordinator Coordinator { get; }
        ...
        public NativeFormListEditorViewModel CreateEditor()
        {
            ...
            return new NativeFormListEditorViewModel(
                ...
                new RecordingNativeReferencePickerService(),
                new InlineUiDispatcher());
        }

        /// <summary>Creates a current native desktop descriptor for a fixture workspace.</summary>
        ...
        public NativeWorkspaceDescriptor CreateDescriptor(Guid workspaceId, WorkspaceRevision revision)
        {
    ...
    private sealed class EditorTestHost : INativeFormListEditorHost
    {
        /// <inheritdoc />
        public event PropertyChangedEventHandler? PropertyChanged;

        /// <inheritdoc />
        public NativeFormListEditorSelection? Selection { get; private set; }
        ...
        public void PublishSelection(NativeFormListEditorSelection? selection)
        {
            Selection = selection;
    ...
    /// <summary>Implements only the native workspace operations exercised by the editor lifecycle.</summary>
    private sealed class EditorTestWorkspace : IFormListWorkspace
    {
        ...
        /// <summary>Initializes one deterministic ready native workspace.</summary>
        /// <param name="workspaceId">The workspace identity.</param>
        ...
        public ValueTask<EngineResult<WorkspacePreview>> PreviewAsync(CancellationToken cancellationToken = default)
        {
            ...
            if (FailPreview)
            {
                return ValueTask.FromResult(EngineResult<WorkspacePreview>.Failure(
                    new EngineError(EngineErrorCode.UnexpectedFailure, "The native preview failed."),
                    WorkspaceId,
                    resultRevision: CurrentRevision));
            }
