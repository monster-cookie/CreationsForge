...
/// <summary>Verifies bounded reference search, exact resolution, cancellation, and stale-state suppression.</summary>
public sealed class ReferencePickerViewModelTests
{
    ...
    public async Task SearchAndNextPage_WithContinuation_ReplacesBoundedVisiblePage()
    {
        ...
        var firstPage = Enumerable.Range(0, ReferencePickerViewModel.PageSize)
        ...
        workspace.SearchRequests[0].MaximumResults.ShouldBe(ReferencePickerViewModel.PageSize);
        workspace.SearchRequests[0].ContinuationToken.ShouldBeNull();
        viewModel.Matches.Count.ShouldBe(ReferencePickerViewModel.PageSize);
    ...
    /// <summary>Verifies each unavailable plugin resolution status is preserved instead of returning a record.</summary>
    ...
    /// <summary>Verifies query invalidation cancels and drains the active plugin search before clearing busy state.</summary>
    [Fact]
    ...
    public async Task SearchAsync_WhenEngineExceedsPageBound_RejectsThePage()
    {
        ...
        var oversizedPage = Enumerable.Range(0, ReferencePickerViewModel.PageSize + 1)
    ...
    /// <param name="recordScope">The record contexts used by search and resolution.</param>
    /// <param name="containingModKey">The optional containing-plugin filter.</param>
    /// <returns>The configured picker view model.</returns>
    private static ReferencePickerViewModel CreateViewModel(
        ...
        ModKey? containingModKey = null)
    {
        ...
        return new ReferencePickerViewModel(
            coordinator,
            dispatcher,
            new ReferencePickerRequest(
    ...
    internal static WorkspaceDescriptor CreateDescriptor(Guid workspaceId, WorkspaceRevision revision)
    {
    ...
    /// <param name="formId">The local plugin form identifier.</param>
    /// <param name="editorId">The plugin EditorID.</param>
    /// <param name="recordType">The stable record type.</param>
    ...
    /// <summary>Creates a successful contextualized plugin search page.</summary>
