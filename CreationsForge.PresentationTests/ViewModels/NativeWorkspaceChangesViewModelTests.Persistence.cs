...
public sealed partial class NativeWorkspaceChangesViewModelTests
{
    ...
    public async Task SaveChangesAsync_ParticipantRefreshFailure_RetryDoesNotReplaySaveOrStateRead()
    {
        ...
        {
            await viewModel.SaveChangesAsync();
            viewModel.PendingRefreshEnvelope!.Step.ShouldBe(NativeWorkspaceRefreshStep.RefreshParticipant);
    ...
    {
        await using var context = NativeWorkspaceChangesTestContext.CreateReady(hasDraftChanges: true);
        context.DialogService.OnShowAsync = async (viewModel, _) =>
        {
    ...
    private static void ConfigureSuccessfulParticipantRefresh(NativeWorkspaceChangesTestContext context)
    {
