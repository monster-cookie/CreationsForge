...
public sealed partial class FormListBrowserViewModelTests
{
    ...
    public async Task EditParticipant_DiscardLocalDraft_RetainsSessionRevisionAndStagedState()
    {
        ...
        draft.Root.ShouldBeOfType<RecordWireObjectDraftNode>()
            .FindProperty("editorId").ShouldBeOfType<RecordWireStringDraftNode>().Value = "ParticipantDraft";

        viewModel.HasDraftChanges.ShouldBeTrue();
        changedProperties.ShouldContain(nameof(IWorkspaceEditParticipant.HasDraftChanges));
        ...
        changedProperties.Count(property => property == nameof(IWorkspaceEditParticipant.HasDraftChanges)).ShouldBeGreaterThanOrEqualTo(2);
    }
    ...
    /// <summary>Verifies a successful plugin read carrying another workspace identity is rejected without clearing stale editor state.</summary>
    /// <returns>A task that completes after the mismatched refresh is rejected.</returns>
    [Fact]
    ...
    private static async Task<ParticipantTestContext> CreateParticipantContextAsync()
    {
        ...
        var editorFactory = new FormListEditorViewModelFactory(
            coordinator,
            operationArbiter,
    ...
    /// <summary>Groups the browser, mutable plugin fake, and exact shared arbiter used by participant tests.</summary>
    /// <param name="viewModel">The browser that owns the editor participant.</param>
    /// <param name="workspace">The deterministic workspace.</param>
    ...
    private sealed class ParticipantTestContext
    {
        ...
        public ParticipantTestContext(
            FormListBrowserViewModel viewModel,
            ParticipantTestWorkspace workspace,
            WorkspacePresentationOperationArbiter operationArbiter)
        {
            ViewModel = viewModel;
            Workspace = workspace;
        ...
        public FormListBrowserViewModel ViewModel { get; }
    ...
    private sealed class ParticipantTestWorkspace : IPluginWorkspace
    {
        ...
        /// <summary>Gets the revision currently returned by every exact plugin read.</summary>
        ...
        /// <summary>Advances the plugin revision by one deterministic sequence.</summary>
        ...
        /// <summary>Creates the deterministic exception for plugin operations outside participant-test scope.</summary>
