...
/// <summary>
/// Verifies picker projection, game capabilities, validation, progress, and engine request forwarding for workspace selection.
/// </summary>
public sealed class WorkspaceSelectionViewModelTests
{
    /// <summary>Verifies the persisted active game selects the exact plugin release and valid output styles.</summary>
    ...
    public async Task BrowseStringDirectoriesAsync_WhenPickerIsCanceled_PreservesExistingDirectories()
    {
        ...
        var picker = new FakeWorkspacePathPicker { StringDirectoryPaths = [] };
        var dispatcher = new InlineUiDispatcher();
        var viewModel = CreateViewModel(
            CreateCoordinator(CreateFactory(CreateSuccessfulWorkspace()), dispatcher),
            picker,
    ...
    public async Task OpenWorkspaceAsync_WithValidSelection_ForwardsAllInputsReportsProgressAndPersistsGame()
    {
        ...
        var viewModel = CreateViewModel(coordinator, new FakeWorkspacePathPicker(), gameSelection, dispatcher);
    ...
    public async Task OpenWorkspaceAsync_WhenEngineRejectsSelection_ReportsErrorAndKeepsSettings()
    {
        ...
        viewModel.StatusText.ShouldBe("Workspace opening failed.");
    ...
    /// <summary>Creates a workspace selection view model with deterministic test dependencies.</summary>
