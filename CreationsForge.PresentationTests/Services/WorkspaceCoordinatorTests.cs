...
/// <summary>
/// Verifies transactional workspace ownership and serialized borrowing at the presentation root lifetime.
/// </summary>
public sealed class WorkspaceCoordinatorTests
{
    ...
    /// <param name="release">The exact plugin release expected by the engine factory.</param>
    [Theory]
    ...
    public async Task OpenAsync_WithSourceOnlyRequest_PublishesReadOnlyDescriptor()
    {
        ...
        var result = await coordinator.OpenAsync(new WorkspaceLaunchRequest(editableRequest.Sources));
    ...
    /// <summary>Verifies close releases plugin ownership even when bound state cannot be published.</summary>
    [Fact]
    ...
    /// <summary>Verifies shutdown disposal releases plugin ownership even when bound state cannot be published.</summary>
    [Fact]
    ...
    public async Task CloseAndDisposeAsync_WhenRepeated_DisposesWorkspaceOnceAndClearsObservableState()
    {
        ...
        {
            if (args.PropertyName == nameof(WorkspaceCoordinator.CurrentWorkspace))
            {
                propertyChanges++;
            }
        };
    ...
    private static WorkspaceCoordinator CreateCoordinator(
        FakeFormListWorkspaceFactory factory,
        InlineUiDispatcher dispatcher)
    {
        return new WorkspaceCoordinator(factory, dispatcher, new LoggerConfiguration().CreateLogger());
    }
        ...
        OutputMasterStyle masterStyle = OutputMasterStyle.Full)
    {
        ...
        var output = new OutputAssociation(
            ...
            masterStyle);
        return new WorkspaceLaunchRequest(sources, OutputSelectionMode.CreateNew, output);
    }
}
