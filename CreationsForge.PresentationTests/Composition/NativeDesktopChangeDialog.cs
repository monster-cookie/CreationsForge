...
internal sealed class NativeDesktopChangeDialog : INativeWorkspaceChangesDialogService
{
    /// <summary>The expected modal purposes and deterministic user interactions in invocation order.</summary>
    private readonly Queue<(NativeWorkspaceChangesDialogPurpose Purpose,
        Func<NativeWorkspaceChangesViewModel, CancellationToken, Task<NativeWorkspaceChangesDialogResult>> Callback)> Interactions = new();
    ...
    public void Enqueue(NativeWorkspaceChangesDialogPurpose purpose,
        Func<NativeWorkspaceChangesViewModel, CancellationToken, Task<NativeWorkspaceChangesDialogResult>> callback)
    {
    ...
    public void AssertDrained()
    {
        if (Failures.Count > 0)
        {
            throw new AggregateException("A native changes dialog interaction failed.", Failures);
        }
