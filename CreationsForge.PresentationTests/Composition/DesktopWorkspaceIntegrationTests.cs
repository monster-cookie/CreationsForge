...
public sealed class DesktopWorkspaceIntegrationTests
{
    ...
    /// <returns>A task that completes after both containers release independent plugin state.</returns>
    [AvaloniaFact]
    public async Task ProductionCoordinator_TwoContainers_OwnIndependentStarfieldWorkspaces()
    {
        ...
        var firstCoordinator = firstContainer.Resolve<IWorkspaceCoordinator>();
        var secondCoordinator = secondContainer.Resolve<IWorkspaceCoordinator>();

        var firstOpen = await firstCoordinator.OpenAsync(new WorkspaceLaunchRequest(
        ...
        var secondOpen = await secondCoordinator.OpenAsync(new WorkspaceLaunchRequest(
            fixture.CreateSourceRequest(),
            OutputSelectionMode.CreateNew,
    ...
    private static async Task VerifyGameAsync(SupportedGame game)
    {
        ...
        var readOnlyResult = await coordinator.OpenAsync(
            new WorkspaceLaunchRequest(fixture.CreateSourceRequest()),
        ...
        var createResult = await coordinator.OpenAsync(new WorkspaceLaunchRequest(
            fixture.CreateSourceRequest(),
            OutputSelectionMode.CreateNew,
        ...
        {
            var browser = browserScope.Resolve<FormListBrowserViewModel>();
        ...
        var existingResult = await coordinator.OpenAsync(new WorkspaceLaunchRequest(
            fixture.CreateSourceRequest(),
    ...
    /// <param name="scope">The record scope to enumerate.</param>
    ...
    /// <summary>Asserts one exact FormList can be listed and read as a detached record through the coordinator.</summary>
    /// <param name="coordinator">The production coordinator that owns the workspace.</param>
    /// <param name="formKey">The FormList identity to read.</param>
        ...
        RecordScope scope)
    {
        ...
        var readFailure = read.Error is null
            ? "The plugin read operation failed without a typed engine error."
    ...
    private static void AssertProjection(
        JsonElement element,
        RecordJsonFieldNodeViewModel node,
        string expectedName)
    {
