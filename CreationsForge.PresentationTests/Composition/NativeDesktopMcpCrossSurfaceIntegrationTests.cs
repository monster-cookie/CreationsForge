...
public sealed class NativeDesktopMcpCrossSurfaceIntegrationTests
{
    /// <summary>The finite deadline shared by every native and protocol operation in one case.</summary>
    private static readonly TimeSpan CaseTimeout = TimeSpan.FromSeconds(60);

    /// <summary>Verifies independent application and MCP owners cannot overwrite the first committed native output.</summary>
    /// <param name="game">The generated native adapter fixture.</param>
    /// <param name="saveOrder">Which production surface commits first.</param>
    /// <returns>A task that completes after both owners, a replacement, and a third native reader are released.</returns>
    [AvaloniaTheory]
    [InlineData(SupportedGame.Starfield, NativeCrossSurfaceSaveOrder.ApplicationFirst)]
    [InlineData(SupportedGame.Starfield, NativeCrossSurfaceSaveOrder.McpFirst)]
    [InlineData(SupportedGame.Fallout4, NativeCrossSurfaceSaveOrder.ApplicationFirst)]
    [InlineData(SupportedGame.Fallout4, NativeCrossSurfaceSaveOrder.McpFirst)]
    [InlineData(SupportedGame.Skyrim, NativeCrossSurfaceSaveOrder.ApplicationFirst)]
    [InlineData(SupportedGame.Skyrim, NativeCrossSurfaceSaveOrder.McpFirst)]
    public async Task ProductionDesktopAndPhysicalMcp_FirstWriterWinsAndStaleOwnerReopens(
        SupportedGame game,
        NativeCrossSurfaceSaveOrder saveOrder)
    {
        ...
        var changesDialog = new NativeDesktopChangeDialog();
        var selectionDialog = new FakeNativeWorkspaceSelectionDialogService();
        await using var viewScope = container.BeginLifetimeScope(builder =>
        {
            builder.RegisterInstance(changesDialog).As<INativeWorkspaceChangesDialogService>();
            builder.RegisterInstance(selectionDialog).As<INativeWorkspaceSelectionDialogService>();
        });
        ...
        var shell = viewScope.Resolve<NativeWorkspaceShellViewModel>();
        ...
        var orderName = saveOrder == NativeCrossSurfaceSaveOrder.ApplicationFirst ? "UiFirst" : "McpFirst";
        ...
        await using var process = await McpStdioProcessFixture.StartAsync(
            ...
            cancellationToken);
        var activeBeforeOpen = await NativeMcpWorkspaceSession.ReadActiveWorkspaceIdsAsync(process.Client, cancellationToken);
        activeBeforeOpen.ShouldBeEmpty();
        var mcp = await NativeMcpWorkspaceSession.OpenAsync(process.Client, fixture, cancellationToken);
        mcp.WorkspaceId.ShouldNotBe(applicationWorkspaceId);
        (await NativeMcpWorkspaceSession.ReadActiveWorkspaceIdsAsync(process.Client, cancellationToken))
        ...
        NativeMcpSaveRequest? mcpSaveRequest = null;
        NativeMcpSaveOutcome? mcpSave = null;
        IReadOnlyDictionary<string, byte[]>? winnerArtifacts = null;
        changesDialog.Enqueue(NativeWorkspaceChangesDialogPurpose.Save, async (model, dialogToken) =>
        {
            model.ShouldBeSameAs(changes);
            model.CurrentReview.ShouldNotBeNull().WorkspaceId.ShouldBe(applicationWorkspaceId);
            using var attached = AttachChangesView(model, NativeWorkspaceChangesDialogRequest.ForSave());
            AssertReviewView(attached.View, model, fixture.ExistingOutputFormKey);
            if (saveOrder == NativeCrossSurfaceSaveOrder.ApplicationFirst)
            {
        ...
        if (saveOrder == NativeCrossSurfaceSaveOrder.ApplicationFirst)
        {
            ...
            mcp = await NativeMcpWorkspaceSession.OpenAsync(process.Client, fixture, cancellationToken);
        ...
        NativeMcpStagedEdit? guardMcpEdit = null;
        ...
        if (saveOrder == NativeCrossSurfaceSaveOrder.McpFirst)
        {
            changesDialog.Enqueue(NativeWorkspaceChangesDialogPurpose.Leave, async (model, dialogToken) =>
            {
                using var attached = AttachChangesView(
                    model,
                    NativeWorkspaceChangesDialogRequest.ForLeave(NativeWorkspaceLeaveReason.OpenWorkspace));
                await model.SaveChangesAsync(dialogToken);
                ...
                var abandon = ControlFinder.FindByAutomationId<Button>(
                    attached.View,
                    "NativeWorkspaceConfirmAbandonmentButton").ShouldNotBeNull();
                abandon.IsEffectivelyVisible.ShouldBeTrue();
                abandon.IsEnabled.ShouldBeTrue();
                ControlFinder.FindByAutomationId<TextBlock>(
                    attached.View,
                    "NativeWorkspaceAbandonmentExplanation").ShouldNotBeNull().Text.ShouldNotBeNull().ShouldContain(
        ...
        replacementRecord.GetProperty("EditorID").GetString().ShouldBe(
            saveOrder == NativeCrossSurfaceSaveOrder.ApplicationFirst ? applicationEditorId : mcpEditorId);
        ...
        await AssertFreshNativeOutputAsync(
            fixture,
            saveOrder == NativeCrossSurfaceSaveOrder.ApplicationFirst ? applicationEditorId : mcpEditorId,
            cancellationToken);
        fixture.SnapshotSourceBytes().ShouldBe(sourceBefore);
    ...
    private static void SetEditorIdDraft(NativeFormListEditorViewModel editor, string editorId)
    {
        ...
        created.Value.ShouldNotBeNull().Root.ShouldBeOfType<NativeWireObjectDraftNode>()
            .FindProperty("editorId").ShouldBeOfType<NativeWireStringDraftNode>().Value = editorId;
    }
    ...
    /// <param name="cancellationToken">The finite case token.</param>
    /// <returns>The detached native record JSON.</returns>
    ...
    private static AttachedChangesView AttachChangesView(
        NativeWorkspaceChangesViewModel model,
        NativeWorkspaceChangesDialogRequest request)
    {
        var view = new NativeWorkspaceChangesView(model, request, _ => Task.CompletedTask);
    ...
    private static void AssertReviewView(
        NativeWorkspaceChangesView view,
        NativeWorkspaceChangesViewModel model,
        FormKey formKey)
    {
    ...
    private static void AssertBoundStatus(NativeWorkspaceChangesView view, string status, bool stale)
    {
        Dispatcher.UIThread.RunJobs();
        ControlFinder.FindByAutomationId<TextBlock>(view, "NativeWorkspaceChangesStatusText")!
            .Text.ShouldBe(status);
        ControlFinder.FindByAutomationId<TextBlock>(view, "NativeWorkspaceStaleReviewWarning")!
        ...
        WorkspaceState before)
    {
        ...
        var plugin = committed.Artifacts.Single(artifact => artifact.Role == NativeArtifactRole.Plugin);
    ...
    private static void AssertCommittedMcpSave(NativeMcpSaveOutcome result, NativeMcpSaveRequest request)
    {
    ...
    private static void AssertRejectedMcpSave(NativeMcpSaveOutcome result, NativeMcpSaveRequest request)
    {
        result.WorkspaceId.ShouldBe(request.WorkspaceId);
        result.OperationId.ShouldBe(request.OperationId);
        result.BaseRevision.ShouldBe(request.ExpectedRevision);
    ...
    private static void AssertSameMcpSave(NativeMcpSaveOutcome expected, NativeMcpSaveOutcome actual)
    {
    ...
    private static void ConfigureReplacement(
        NativeWorkspaceSelectionViewModel selection,
    ...
    /// <summary>Uses a third independent production native lifetime to verify the committed disk record and clean state.</summary>
    /// <param name="fixture">The generated native fixture.</param>
    ...
    private static async Task AssertFreshNativeOutputAsync(
        NativeDesktopWorkspaceFixture fixture,
    ...
    private sealed class AttachedChangesView : IDisposable
    {
        ...
        public AttachedChangesView(Window window, NativeWorkspaceChangesView view)
        {
            Window = window;
            View = view;
        }
        ...
        public NativeWorkspaceChangesView View { get; }
...
public enum NativeCrossSurfaceSaveOrder
{
