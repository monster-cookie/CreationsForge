...
/// <summary>
/// Owns minimal generated source and output plugins for exercising the production plugin desktop container without installed-game state.
/// </summary>
internal sealed class DesktopWorkspaceFixture : IDisposable
{
    ...
    /// <param name="dataDirectory">The explicit plugin source data directory.</param>
    ...
    private DesktopWorkspaceFixture(
    ...
    /// <param name="game">The game whose plugin adapters must open the generated plugins.</param>
    ...
    public static DesktopWorkspaceFixture Create(SupportedGame game)
    {
        var rootDirectory = Directory.CreateTempSubdirectory("CreationsForge-DesktopWorkspace-");
        try
        {
            ...
            var records = game switch
            {
                ...
                _ => throw new ArgumentOutOfRangeException(nameof(game), game, "A plugin desktop fixture requires a supported game."),
            };
            ...
            return new DesktopWorkspaceFixture(
                rootDirectory,
                dataDirectory,
                outputDirectory,
                game,
    ...
    public IContainer CreateContainer(string ownerName)
    {
        ...
        return DesktopComposition.Create(configurationStore, new LoggerConfiguration().CreateLogger());
    }

    /// <summary>Creates a complete source-open request with a fresh workspace identity.</summary>
    /// <returns>The explicit plugin request containing no installed-game defaults.</returns>
    ...
    /// <returns>The plugin source and output FormList identities.</returns>
    ...
    /// <summary>Writes one complete plugin using Mutagen's ordinary binary writer.</summary>
    ...
    /// <returns>The complete plugin output association.</returns>
    ...
    private static void DeleteOwnedDirectory(DirectoryInfo directory)
    {
        ...
        if (!rootPath.StartsWith(temporaryPath, StringComparison.OrdinalIgnoreCase) ||
            !directory.Name.StartsWith("CreationsForge-DesktopWorkspace-", StringComparison.Ordinal))
        {
            throw new InvalidOperationException($"Refusing to delete an unrecognized plugin desktop fixture root: {rootPath}");
        }
    ...
    /// <summary>Stores the generated record identities shared by game-specific fixture writers.</summary>
    private sealed class FixtureRecords
    {
        /// <summary>Initializes a pair of generated FormList identities.</summary>
