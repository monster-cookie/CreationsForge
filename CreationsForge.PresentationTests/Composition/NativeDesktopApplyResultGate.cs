...
/// <summary>Delays delivery of one real successful native Apply result without altering the engine's mutation or receipt.</summary>
internal sealed class NativeDesktopApplyResultGate : INativeWorkspaceCoordinator
{
    /// <summary>The production coordinator whose workspace ownership remains with the root container.</summary>
    private readonly INativeWorkspaceCoordinator Inner;

    /// <summary>Signals that the native operation succeeded and its exact receipt is waiting for presentation.</summary>
    ...
    /// <param name="inner">The root-owned coordinator that performs every native operation unchanged.</param>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="inner"/> is <see langword="null"/>.</exception>
    internal NativeDesktopApplyResultGate(INativeWorkspaceCoordinator inner)
    {
    ...
    /// <inheritdoc />
    public NativeWorkspaceDescriptor? CurrentWorkspace => Inner.CurrentWorkspace;

    /// <summary>Gets the exact successful native receipt retained before presentation publication.</summary>
    ...
    /// <param name="cancellationToken">The real caller token forwarded to native execution and observed during the response delay.</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (result.Succeeded
            && result.Value is NativeFormListEditorApplyOutcome outcome
