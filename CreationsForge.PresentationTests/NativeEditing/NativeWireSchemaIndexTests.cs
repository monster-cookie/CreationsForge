...
public sealed class NativeWireSchemaIndexTests
{
    ...
    public void Resolve_AllGeneratedConcreteTypes_PreservesDefaultAvailability()
    {
        ...
        var index = new NativeWireSchemaIndex(context);
        foreach (var node in concrete)
        {
            var result = index.Resolve(node.Key, NativeWireReadLimits.Default);
    ...
    public void Resolve_LargeGeneratedUnions_IndexesExpectedAlternativesLazily()
    {
        var context = NativeFormListCatalogTests.CreateContexts()[0];
        var index = new NativeWireSchemaIndex(context);
        var componentKey = context.SchemaCatalog.Nodes.Single(key => key.Kind == NativeWireSchemaNodeKind.Type && key.Name == "native.component");
        var conditionDataKey = context.SchemaCatalog.Nodes.Single(key => key.Kind == NativeWireSchemaNodeKind.Type && key.Name == "native.condition-data");

        var components = index.Resolve(componentKey, NativeWireReadLimits.Default).Value!;
        var conditionData = index.Resolve(conditionDataKey, NativeWireReadLimits.Default).Value!;

        components.Kind.ShouldBe(NativeWireSchemaValueKind.Union);
        components.UnionOptions.Count.ShouldBe(66);
        conditionData.Kind.ShouldBe(NativeWireSchemaValueKind.Union);
    ...
    public void Resolve_Canceled_ThrowsOperationCanceledException()
    {
        ...
        Should.Throw<OperationCanceledException>(() => new NativeWireSchemaIndex(context).Resolve(key, NativeWireReadLimits.Default, cancellation.Token));
    }
}
